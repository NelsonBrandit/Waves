...
inTask(docker)(
  Seq(
    dockerfile := {
      ...
      val withAspectJ =
        Option(System.getenv("WITH_ASPECTJ")).fold(false)(_.toBoolean)
      val aspectjAgentUrl =
        "http://search.maven.org/remotecontent?filepath=org/aspectj/aspectjweaver/1.9.1/aspectjweaver-1.9.1.jar"
...
lazy val itTestsCommonSettings: Seq[Def.Setting[_]] = Seq(
  testOptions += Tests.Argument(TestFrameworks.ScalaTest,
                                "-fFW",
                                (logDirectory.value / "summary.log").toString),
  testGrouping := {
    // ffs, sbt!
        ...
        Tests.SubProcess(
          ForkOptions(
            ...
            runJVMOptions = Vector(
              ...
              s"-Dwaves.it.logging.dir=${logDirectoryValue / suite.name
                .replaceAll("""(\w)\w*\.""", "$1.")}"
            ) ++ javaOptionsValue,
            connectInput = false,
...
inConfig(Test) {
  ...
  ) ++ inTask(test)(itTestsCommonSettings) ++ inTask(testOnly)(
    itTestsCommonSettings)
}
