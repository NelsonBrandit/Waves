    ...
    mappings := {
      ...
      mappings.value.map {
        case m @ (file, batScriptPattern(script)) =>
          if (script.endsWith(scriptSuffix)) m
          else (file, s"bin/$script$scriptSuffix.bat")
        case m @ (file, linuxScriptPattern(script)) =>
          if (script.endsWith(scriptSuffix)) m
          else (file, s"bin/$script$scriptSuffix")
        case other => other
      }
    },
    javaOptions ++= Seq(
...
checkPRRaw in Global := {
  try {
    ...
    test
      .all(
        ScopeFilter(inProjects(langJVM, node, commonJVM),
                    inConfigurations(Test)))
      .value
    (langJS / Compile / fastOptJS).value
    compile
      .all(
        ScopeFilter(inProjects(generator, benchmark), inConfigurations(Test)))
      .value
  }
}
...
lazy val lang =
  crossProject(JSPlatform, JVMPlatform)
    ...
    .jvmSettings(
      ...
      scmInfo := Some(
        ScmInfo(url("https://github.com/wavesplatform/Waves"),
                "git@github.com:wavesplatform/Waves.git",
                None)),
      developers := List(
        Developer("petermz",
                  "Peter Zhelezniakov",
...
lazy val benchmark = project
  ...
  .dependsOn(node % "compile->compile;test->test",
             langJVM % "compile->compile;test->test")
