...
package object state {
  def safeSum(x: Long, y: Long): Long =
    Try(Math.addExact(x, y)).getOrElse(Long.MinValue)
  ...
  def addressTransactionsFromDiff(b: Blockchain, d: Option[Diff])(
    ...
    def transactionsFromDiff(d: Diff): Seq[(Int, Transaction, Set[Address])] =
      d.transactions.values.view.toSeq.reverse

    def withPagination(s: Seq[(Int, Transaction, Set[Address])])
      : Seq[(Int, Transaction, Set[Address])] =
      fromId match {
        case None     => s
        case Some(id) => s.dropWhile(_._2.id() != id).drop(1)
      }

    def withFilterAndLimit(
        txs: Seq[(Int, Transaction, Set[Address])]): Seq[(Int, Transaction)] =
      txs
        .collect {
          case (height, tx, addresses)
              if addresses(address) && (types.isEmpty || types.contains(
                tx.builder.typeId)) =>
            (height, tx)
        }
        .take(count)

    def withRestFromBlockchain(
        s: Seq[(Int, Transaction)]): Either[String, Seq[(Int, Transaction)]] =
      s.length match {
        case `count` => Right(s)
        case l if l < count =>
          b.addressTransactions(address, types, count - l, None).map(s ++ _)
  ...
  implicit class EitherExt[L <: ValidationError, R](ei: Either[L, R]) {
    def liftValidationError[T <: Transaction](
        t: T): Either[ValidationError, R] = {
  ...
  implicit class BlockchainExt(blockchain: Blockchain) {
    ...
    def contains(signature: ByteStr): Boolean =
      blockchain.heightOf(signature).isDefined

    def blockById(blockId: ByteStr): Option[Block] =
      blockchain
        .blockBytes(blockId)
        .flatMap(bb => Block.parseBytes(bb).toOption)
    def blockAt(height: Int): Option[Block] =
      blockchain.blockBytes(height).flatMap(bb => Block.parseBytes(bb).toOption)

    def lastBlockHeaderAndSize: Option[(Block, Int)] =
      blockchain.lastBlock.map(b => (b, b.bytes().length))
    def lastBlockId: Option[ByteStr] =
      blockchain.lastBlockHeaderAndSize.map(_._1.uniqueId)
    def lastBlockTimestamp: Option[Long] =
      blockchain.lastBlockHeaderAndSize.map(_._1.timestamp)

    def lastBlocks(howMany: Int): Seq[Block] = {
      (Math.max(1, blockchain.height - howMany + 1) to blockchain.height)
        .flatMap(blockchain.blockAt)
        .reverse
    }
    ...
    def effectiveBalance(address: Address,
                         confirmations: Int,
                         block: BlockId = blockchain.lastBlockId.getOrElse(
                           ByteStr.empty)): Long = {
      val blockHeight = blockchain.heightOf(block).getOrElse(blockchain.height)
      val bottomLimit =
        (blockHeight - confirmations + 1).max(1).min(blockHeight)
      ...
      val block = blockchain
        ...
        .getOrElse(
          throw new IllegalArgumentException(
            s"Invalid block height: $atHeight"))
      val balances =
        blockchain.balanceSnapshots(address, bottomLimit, block.uniqueId)
    ...
    def aliasesOfAddress(address: Address): Seq[Alias] =
      blockchain
        .addressTransactions(address,
                             Set(CreateAliasTransactionV1.typeId),
                             Int.MaxValue,
                             None)
        .explicitGet()
    ...
    def activeLeases(address: Address): Seq[(Int, LeaseTransaction)] =
      blockchain
        .addressTransactions(address,
                             Set(LeaseTransactionV1.typeId),
                             Int.MaxValue,
                             None)
        .explicitGet()
        .collect {
          case (h, l: LeaseTransaction)
              if blockchain.leaseDetails(l.id()).exists(_.isActive) =>
            h -> l
        }
  ...
  implicit val dstMonoid: Monoid[AssetDistribution] =
    new Monoid[AssetDistribution] {
      override def empty: AssetDistribution =
        AssetDistribution(Map.empty[Address, Long])

      override def combine(x: AssetDistribution,
                           y: AssetDistribution): AssetDistribution = {
  ...
  object AssetDistributionPage
      extends TaggedType[Paged[Address, AssetDistribution]]
