...
class NgState(val base: Block,
  ...
  private val microDiffs
    : MMap[BlockId, (Diff, Long, Long)] = MMap.empty // microDiff, carryFee, timestamp
  ...
  def totalDiffOf(
      id: BlockId): Option[(Block, Diff, Long, DiscardedMicroBlocks)] =
    forgeBlock(id).map {
      case (b, txs) =>
        val (d, c) = diffFor(id)
        (b, d, c, txs)
