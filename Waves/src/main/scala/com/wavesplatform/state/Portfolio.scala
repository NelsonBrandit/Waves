...
object Portfolio {
  ...
  def build(a: Asset, amount: Long): Portfolio = a match {
    ...
    case t @ IssuedAsset(_) =>
      Portfolio(0L, LeaseBalance.empty, Map(t -> amount))
  }
  ...
  implicit class PortfolioExt(self: Portfolio) {
    def spendableBalanceOf(assetId: Asset): Long =
      assetId.fold(self.spendableBalance)(self.assets.getOrElse(_, 0L))
    ...
    def changedAssetIds(that: Portfolio): Set[Asset] = {
      ...
      intersection.filter(x =>
        spendableBalanceOf(x) != that.spendableBalanceOf(x)) ++ sureChanged
    }
  }
  ...
  implicit val portfolioJsonFormat: Format[Portfolio] =
    Format(portfolioJsonReads, portfolioJsonWrites)
}
