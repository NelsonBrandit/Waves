...
object BlockDiffer extends ScorexLogging with Instrumented {
  ...
  def fromBlock[Constraint <: MiningConstraint](settings: FunctionalitySettings,
    ...
    val ngHeight = blockchain
      .featureActivationHeight(BlockchainFeatures.NG.id)
      .getOrElse(Int.MaxValue)
    val sponsorshipHeight =
      Sponsorship.sponsoredFeesSwitchHeight(blockchain, settings)
  ...
  private def apply[Constraint <: MiningConstraint](
    ...
    val txDiffer = TransactionDiffer(settings,
                                     ...
                                     currentBlockHeight,
                                     verify) _
    val initDiff = Diff.empty.copy(portfolios = Map(
      blockGenerator -> currentBlockFeeDistr.orElse(prevBlockFeeDistr).orEmpty))
    val hasNg = currentBlockFeeDistr.isEmpty
    val hasSponsorship = currentBlockHeight >= Sponsorship
      .sponsoredFeesSwitchHeight(blockchain, settings)

    def clearSponsorship(blockchain: Blockchain,
                         portfolio: Portfolio): (Portfolio, Long) = {
      val spPf =
        if (hasSponsorship)
            ...
            balance = portfolio.balance +
              portfolio.assets.map {
                case (assetId, assetFee) =>
                  val baseFee =
                    blockchain.assetDescription(assetId).get.sponsorship
        ...
        case (Right((currDiff, carryFee, currConstraint)), tx) =>
          ...
          if (updatedConstraint.isOverfilled)
            ...
            txDiffer(updatedBlockchain, tx).map { newDiff =>
              ...
              if (hasNg) {
                val (curBlockFees, nextBlockFee) =
                  clearSponsorship(updatedBlockchain, tx.feeDiff())
      ...
      .map {
        case (d, carry, constraint) =>
          ...
          val diffWithCancelledLeaseIns =
            if (blockchain
                  .featureActivationHeight(
                    BlockchainFeatures.DataTransaction.id)
                  .contains(currentBlockHeight))
              Monoid.combine(diffWithLeasePatches,
                             CancelInvalidLeaseIn(
                               composite(blockchain, diffWithLeasePatches)))
