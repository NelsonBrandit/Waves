...
object MassTransferTransactionDiff {
  ...
  def apply(blockchain: Blockchain, blockTime: Long, height: Int)(
      tx: MassTransferTransaction): Either[ValidationError, Diff] = {
    def parseTransfer(
        xfer: ParsedTransfer): Validation[(Map[Address, Portfolio], Long)] = {
      for {
        recipientAddr <- blockchain.resolveAlias(xfer.address)
        portfolio = tx.assetId
          .fold(
            Map(
              recipientAddr -> Portfolio(xfer.amount,
                                         LeaseBalance.empty,
                                         Map.empty))) { asset =>
            Map(
              recipientAddr -> Portfolio(0,
                                         LeaseBalance.empty,
                                         Map(asset -> xfer.amount)))
          }
    ...
    portfoliosEi.flatMap { list: List[(Map[Address, Portfolio], Long)] =>
      ...
      val foldInit =
        (Map(sender -> Portfolio(-tx.fee, LeaseBalance.empty, Map.empty)), 0L)
        ...
        recipientPortfolios
          .combine(
            tx.assetId
              .fold(
                Map(
                  sender -> Portfolio(-totalAmount,
                                      LeaseBalance.empty,
                                      Map.empty))) { asset =>
                Map(
                  sender -> Portfolio(0,
                                      LeaseBalance.empty,
                                      Map(asset -> -totalAmount)))
              }
          )
      ...
      val assetIssued = tx.assetId match {
        ...
        case asset @ IssuedAsset(_) =>
          blockchain.assetDescription(asset).isDefined
      }
