...
object InvokeScriptTransactionDiff {
  def apply(blockchain: Blockchain, height: Int)(
      tx: InvokeScriptTransaction): Either[ValidationError, Diff] = {
    ...
    def evalContract(contract: Contract) = {
      val ctx = Monoid
        .combineAll(
          Seq(
            ...
            WavesContext.build(
              DirectiveSet(StdLibVersion.V3,
                           ScriptType.Account,
                           ContentType.Contract),
              new WavesEnvironment(AddressScheme.current.chainId,
      ...
      val maybePayment: Option[(Long, Option[ByteStr])] =
        tx.payment.headOption.map(p => (p.amount, p.assetId.compatId))
      ContractEvaluator.apply(
        ctx,
        contract,
        ContractEvaluator.Invocation(tx.fc,
                                     invoker,
                                     maybePayment,
                                     tx.contractAddress.bytes))
    }
    ...
    sc match {
      case Some(ContractScriptImpl(_, contract, _)) =>
        ...
        contract.cfs.find(_.u.name == functionName) match {
          case None =>
            Left(GenericError(
              s"No function '$functionName' at address ${tx.contractAddress}"))
          case Some(_) =>
            evalContract(contract).left
              ...
              .flatMap {
                case ContractResult(ds, ps) =>
                  ...
                  val pmts: List[Map[Address, Map[Option[ByteStr], Long]]] =
                    ps.map {
                      case (Recipient.Address(addrBytes), amt, maybeAsset) =>
                        Map(
                          Address.fromBytes(addrBytes.arr).explicitGet() -> Map(
                            maybeAsset -> amt))
                    }
                  for {
                        ...
                        for {
                          assetInfo <- blockchain
                            ...
                            .toRight(GenericError(
                              s"Asset $asset does not exist, cannot be used to pay fees"))
                          wavesFee <- Either.cond(
                            assetInfo.sponsorship > 0,
                            Sponsorship.toWaves(tx.fee, assetInfo.sponsorship),
                            GenericError(
                              s"Asset $asset is not sponsored, cannot be used to pay fees")
                          )
                        } yield {
                          (wavesFee,
                           Map(
                             tx.sender.toAddress -> Portfolio(
                               ...
                               LeaseBalance.empty,
                               Map(asset -> -tx.fee)),
                             assetInfo.issuer.toAddress -> Portfolio(
                               -wavesFee,
                               LeaseBalance.empty,
                               Map(asset -> tx.fee))
                           ))
                    ...
                    wavesFee = feeInfo._1
                    dataAndPaymentDiff <- payableAndDataPart(height,
                                                             tx,
                                                             ds,
                                                             feeInfo._2)
                    _ <- Either.cond(
                      pmts.flatMap(_.values).flatMap(_.values).forall(_ >= 0),
                      (),
                      ValidationError.NegativeAmount(-42, ""))
                    _ <- validateOverflow(
                      pmts.flatMap(_.values).flatMap(_.values),
                      "Attempt to transfer unavailable funds in contract payment")
                    _ <- Either.cond(
                      pmts
                        .flatMap(_.values)
                        .flatMap(_.keys)
                        ...
                        .forall(id =>
                          blockchain
                            .assetDescription(IssuedAsset(id))
                            .isDefined),
                      (),
                    ...
                    _ <- {
                      val totalScriptsInvoked =
                        tx.checkedAssets()
                          ...
                          ps.count(_._3.fold(false)(id =>
                            blockchain.hasAssetScript(IssuedAsset(id))))
                      val minWaves = totalScriptsInvoked * ScriptExtraFee + FeeConstants(
                        InvokeScriptTransaction.typeId) * FeeUnit
                      Either.cond(
                        minWaves <= wavesFee,
                        (),
                    ...
                    _ <- foldContractTransfers(blockchain, tx)(
                      ps,
                      dataAndPaymentDiff)
                  } yield {
                    val paymentReceiversMap: Map[Address, Portfolio] = Monoid
                      ...
                      .mapValues(mp =>
                        mp.toList.map(x =>
                          Portfolio.build(Asset.fromCompatId(x._1), x._2)))
                      .mapValues(l => Monoid.combineAll(l))
                    val paymentFromContractMap = Map(
                      tx.contractAddress -> Monoid
                        .combineAll(paymentReceiversMap.values)
                        .negate)
                    val transfers = Monoid.combineAll(
                      Seq(paymentReceiversMap, paymentFromContractMap))
                    dataAndPaymentDiff.combine(
                      Diff.stateOps(portfolios = transfers))
                  }
              }
        }
      case _ =>
        Left(GenericError(s"No contract at address ${tx.contractAddress}"))
    }
  ...
  private def payableAndDataPart(height: Int,
    ...
    if (r.length > ContractLimits.MaxWriteSetSize) {
      Left(GenericError(
        s"WriteSec can't contain more than ${ContractLimits.MaxWriteSetSize} entries"))
    } else if (r.exists(
                 _.key.getBytes().length > ContractLimits.MaxKeySizeInBytes)) {
      Left(
        GenericError(
        ...
        .map {
          case InvokeScriptTransaction.Payment(amt, assetId) =>
            assetId match {
              case asset @ IssuedAsset(_) =>
                Map(
                  tx.sender.toAddress -> Portfolio(0,
                                                   LeaseBalance.empty,
                                                   Map(asset -> -amt))).combine(
                  Map(
                    tx.contractAddress -> Portfolio(0,
                                                    LeaseBalance.empty,
                                                    Map(asset -> amt)))
                )
              case Waves =>
                Map(
                  tx.sender.toAddress -> Portfolio(-amt,
                                                   LeaseBalance.empty,
                                                   Map.empty))
                  .combine(
                    Map(
                      tx.contractAddress -> Portfolio(amt,
                                                      LeaseBalance.empty,
                                                      Map.empty)))
            }
        })
      ...
      if (totalDataBytes <= ContractLimits.MaxWriteSetSizeInBytes)
        Right(
          Diff(
            ...
            accountData = Map(tx.contractAddress -> AccountDataInfo(
              r.map(d => d.key -> d).toMap))
          ))
      else
        Left(GenericError(
  ...
  private def foldContractTransfers(blockchain: Blockchain,
                                    tx: InvokeScriptTransaction)(
    ...
    if (ps.length <= ContractLimits.MaxPaymentAmount)
      ps.foldLeft(Either.right[ValidationError, Diff](dataDiff)) {
        (diffEi, payment) =>
              ...
              diffEi combine Right(
                Diff.stateOps(
                  portfolios = Map(
                    ...
                    tx.contractAddress -> Portfolio(-amount,
                                                    LeaseBalance.empty,
                                                    Map.empty)
                  )))
            case a @ IssuedAsset(_) =>
              diffEi combine {
                ...
                blockchain.assetScript(a) match {
                  ...
                  case Some(script) =>
                    diffEi flatMap (d =>
                      validateContractTransferWithSmartAssetScript(
                        blockchain,
                        tx)(d, addressRepr, amount, asset, nextDiff, script))
                }
              }
          }
      } else
      Left(GenericError(
        s"Too many ContractTransfers: max: ${ContractLimits.MaxPaymentAmount}, actual: ${ps.length}"))
  }

  private def validateContractTransferWithSmartAssetScript(
    ...
    Try {
      ScriptRunner(
        ...
        case (log, Left(execError)) =>
          Left(ScriptExecutionError(execError, log, true))
        case (log, Right(FALSE)) =>
          Left(TransactionNotAllowedByScript(log, isTokenScript = true))
        case (_, Right(TRUE)) => Right(nextDiff)
        case (_, Right(x)) =>
          Left(GenericError(s"Script returned not a boolean result, but $x"))
      }
    } match {
      case Failure(e) =>
        Left(
          ScriptExecutionError(
            s"Uncaught execution error: ${Throwables.getStackTraceAsString(e)}",
            List.empty,
            true))
      case Success(s) => s
