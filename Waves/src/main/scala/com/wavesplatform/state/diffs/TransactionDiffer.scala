...
object TransactionDiffer extends Instrumented with ScorexLogging {
  ...
  def verified(settings: FunctionalitySettings,
               prevBlockTimestamp: Option[Long],
               currentBlockTimestamp: Long,
               currentBlockHeight: Int)(
      blockchain: Blockchain,
    ...
    for {
        ...
        .measureForType(tx.builder.typeId) {
          for {
            _ <- CommonValidation.disallowTxFromFuture(settings,
                                                       currentBlockTimestamp,
                                                       tx)
            _ <- CommonValidation.disallowTxFromPast(settings,
                                                     prevBlockTimestamp,
                                                     tx)
            _ <- CommonValidation.disallowBeforeActivationTime(
              blockchain,
              currentBlockHeight,
              tx)
      ...
      diff <- unverified(settings, currentBlockTimestamp, currentBlockHeight)(
        blockchain,
        tx)
  ...
  def unverified(settings: FunctionalitySettings,
                 currentBlockTimestamp: Long,
                 currentBlockHeight: Int)(
      blockchain: Blockchain,
      tx: Transaction): Either[ValidationError, Diff] = {
    stats.transactionDiffValidation.measureForType(tx.builder.typeId) {
      tx match {
        ...
        case sstx: SetScriptTransaction =>
          SetScriptTransactionDiff(blockchain, currentBlockHeight)(sstx)
        case sstx: SetAssetScriptTransaction =>
          AssetTransactionsDiff.setAssetScript(blockchain,
                                               settings,
                                               currentBlockTimestamp,
                                               currentBlockHeight)(sstx)
        ...
        case ci: InvokeScriptTransaction =>
          InvokeScriptTransactionDiff.apply(blockchain, currentBlockHeight)(ci)
