...
object CommonValidation {
    ...
    if (blockTime >= settings.allowTemporaryNegativeUntil) {
      def checkTransfer(sender: Address,
        ...
        if (newWavesBalance < 0) {
          ...
          val balanceError = spendings.assets.collectFirst {
            case (aid, delta)
                if delta < 0 && blockchain.balance(sender, aid) + delta < 0 =>
      ...
      tx match {
        case ptx: PaymentTransaction
            if blockchain.balance(ptx.sender, Waves) < (ptx.amount + ptx.fee) =>
  ...
  def disallowDuplicateIds[T <: Transaction](
    ...
    case _ =>
      val id = tx.id()
      Either.cond(!blockchain.containsTransaction(tx),
                  tx,
                  AlreadyInTheState(id, blockchain.transactionInfo(id).get._1))
  }
  ...
  def disallowBeforeActivationTime[T <: Transaction](
    ...
    def activationBarrier(
        b: BlockchainFeature,
        msg: Option[String] = None): Either[ActivationError, T] =
      Either.cond(
        blockchain.isFeatureActivated(b, height),
        tx,
        ValidationError.ActivationError(
          msg.getOrElse(b.description + " feature has not been activated yet"))
      )
    ...
    def scriptActivation(sc: Script): Either[ActivationError, T] = {
      ...
      def scriptVersionActivation(sc: Script): Either[ActivationError, T] =
        sc.stdLibVersion match {
          case V1 | V2 if sc.containsBlockV2.value => ab
          case V1 | V2                             => Right(tx)
          case V3                                  => ab
        }

      def scriptTypeActivation(sc: Script): Either[ActivationError, T] =
        sc match {
    ...
    tx match {
      ...
      case exv2: ExchangeTransactionV2 =>
        activationBarrier(BlockchainFeatures.SmartAccountTrading).flatMap {
          tx =>
            (exv2.buyOrder, exv2.sellOrder) match {
              case (_: OrderV3, _: Order) | (_: Order, _: OrderV3) =>
                activationBarrier(BlockchainFeatures.OrderV3)
              case _ => Right(tx)
            }
        }
  ...
  def disallowTxFromPast[T <: Transaction](settings: FunctionalitySettings,
                                           prevBlockTime: Option[Long],
                                           tx: T): Either[ValidationError, T] =
    prevBlockTime match {
      case Some(t)
          if (t - tx.timestamp) > settings.maxTransactionTimeBackOffset.toMillis =>
        Left(
          Mistiming(
  ...
  private def feeInUnits(blockchain: Blockchain,
                         height: Int,
                         tx: Transaction): Either[ValidationError, Long] = {
    FeeConstants
  ...
  def getMinFee(
      ...
      height: Int,
      tx: Transaction): Either[ValidationError, (Asset, Long, Long)] = {
              ...
              for {
                assetInfo <- blockchain
                  ...
                  .toRight(GenericError(
                    s"Asset ${assetId.id.base58} does not exist, cannot be used to pay fees"))
                wavesFee <- Either.cond(
                  assetInfo.sponsorship > 0,
                  feeInUnits * FeeUnit,
                  GenericError(
                    s"Asset ${assetId.id.base58} is not sponsored, cannot be used to pay fees")
                )
    ...
    def feeAfterSmartTokens(
        inputFee: FeeInfo): Either[ValidationError, FeeInfo] = {
      val (feeAssetInfo, feeAmount) = inputFee
      val assetsCount = tx match {
        case tx: ExchangeTransaction =>
          tx.checkedAssets()
            .collect { case a @ IssuedAsset(_) => a }
            .count(blockchain.hasAssetScript) /* *3 if we deside to check orders and transaction */
        case _ =>
          tx.checkedAssets()
            .collect { case a @ IssuedAsset(_) => a }
    ...
    def smartAccountScriptsCount: Int = tx match {
      case tx: Transaction with Authorized =>
        cond(blockchain.hasScript(tx.sender))(1, 0)
      case _ => 0
    }

    def feeAfterSmartAccounts(
        inputFee: FeeInfo): Either[ValidationError, FeeInfo] = Right {
    ...
    feeAfterSponsorship(tx.assetFee._1)
      ...
      .map {
        case (Some((assetId, assetInfo)), amountInWaves) =>
          (assetId,
           Sponsorship.fromWaves(amountInWaves, assetInfo.sponsorship),
           amountInWaves)
  ...
  def checkFee(blockchain: Blockchain,
    ...
    if (height >= Sponsorship.sponsoredFeesSwitchHeight(blockchain, fs)) {
      ...
      Either.cond(tx.assetFee._2 > 0 || !tx.isInstanceOf[Authorized],
                  (),
                  GenericError(s"Fee must be positive."))
    }
  }

  def cond[A](c: Boolean)(a: A, b: A): A = if (c) a else b

  def validateOverflow(dataList: Traversable[Long],
                       errMsg: String): Either[ValidationError, Unit] = {
