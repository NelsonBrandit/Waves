...
object BalanceDiffValidation extends ScorexLogging with Instrumented {
  ...
  def apply(b: Blockchain, currentHeight: Int, fs: FunctionalitySettings)(
      d: Diff): Either[AccountBalanceError, Diff] = {
    ...
    def check(acc: Address): Option[(Address, String)] = {
      ...
      (if (balance < 0) {
         ...
         if (newB < 0) {
           Some(
             acc -> s"negative waves balance: $acc, old: $oldWaves, new: $newB")
         } else if (newB < lease.out && currentHeight > fs.allowLeasedBalanceTransferUntilHeight) {
           Some(
             acc -> (if (newB + lease.in - lease.out < 0) {
        ...
        acc -> s"negative asset balance: $acc, new portfolio: ${negativeAssetsInfo(
          b.portfolio(acc).combine(portfolioDiff))}"
      })
    }

    val positiveBalanceErrors: Map[Address, String] =
      changedAccounts.flatMap(check).toMap
