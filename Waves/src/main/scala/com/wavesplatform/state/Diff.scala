...
object Sponsorship {
  ...
  def toWaves(assetFee: Long, sponsorship: Long): Long = {
    val waves = (BigDecimal(assetFee) * BigDecimal(CommonValidation.FeeUnit)) / BigDecimal(
      sponsorship)
  ...
  def fromWaves(wavesFee: Long, sponsorship: Long): Long = {
    val assetFee = (BigDecimal(wavesFee) / BigDecimal(CommonValidation.FeeUnit)) * BigDecimal(
      sponsorship)
