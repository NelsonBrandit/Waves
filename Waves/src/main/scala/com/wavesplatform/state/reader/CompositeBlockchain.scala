...
class CompositeBlockchain(inner: Blockchain,
  ...
  override def portfolio(a: Address): Portfolio =
    inner.portfolio(a).combine(diff.portfolios.getOrElse(a, Portfolio.empty))

  override def balance(address: Address, assetId: Asset): Long =
    inner.balance(address, assetId) + diff.portfolios
      .getOrElse(address, Portfolio.empty)
      .balanceOf(assetId)

  override def leaseBalance(address: Address): LeaseBalance = {
    cats.Monoid.combine(
      inner.leaseBalance(address),
      diff.portfolios.getOrElse(address, Portfolio.empty).lease)
  }
  ...
  override def hasAssetScript(asset: IssuedAsset): Boolean =
    maybeDiff.flatMap(_.assetScripts.get(asset)) match {
      case Some(s) => s.nonEmpty
      case None    => inner.hasAssetScript(asset)
    }

  override def assetDescription(
      asset: IssuedAsset): Option[AssetDescription] = {
      ...
      case Some(ad) =>
        diff.issuedAssets
          ...
          .map { newAssetInfo =>
            ...
            ad.copy(reissuable = combination.isReissuable,
                    totalVolume = combination.volume,
                    script = script)
          }
          ...
          .map(
            z =>
              diff.issuedAssets
                .get(asset)
                .fold(z)(r =>
                  z.copy(reissuable = r.isReissuable,
  ...
  override def leaseDetails(leaseId: ByteStr): Option[LeaseDetails] = {
    inner
      ...
      .map(ld =>
        ld.copy(isActive = diff.leaseState.getOrElse(leaseId, ld.isActive))) orElse
  ...
  override def resolveAlias(alias: Alias): Either[ValidationError, Address] =
    inner.resolveAlias(alias) match {
  ...
  override def allActiveLeases: Set[LeaseTransaction] = {
    ...
    val fromDiff = active.keys
      ...
      .toSet
    val fromInner =
      inner.allActiveLeases.filterNot(ltx => canceled.keySet.contains(ltx.id()))
    fromDiff ++ fromInner
  }
  ...
  override def containsTransaction(tx: Transaction): Boolean =
    diff.transactions.contains(tx.id()) || inner.containsTransaction(tx)

  override def filledVolumeAndFee(orderId: ByteStr): VolumeAndFee =
    diff.orderFills
      .get(orderId)
      .orEmpty
      .combine(inner.filledVolumeAndFee(orderId))

  override def balanceSnapshots(address: Address,
                                from: Int,
                                to: BlockId): Seq[BalanceSnapshot] = {
    if (inner.heightOf(to).isDefined || maybeDiff.isEmpty) {
      ...
      if (inner.height > 0 && from < this.height)
  ...
  override def assetDistribution(assetId: IssuedAsset): AssetDistribution = {
    ...
    val fromDiff = AssetDistribution(
  ...
  override def wavesDistribution(
      height: Int): Either[ValidationError, Map[Address, Long]] = {
  ...
  override def blockHeaderAndSize(
    ...
    inner.blockHeaderAndSize(blockId)
  ...
  override def blockIdsAfter(parentSignature: ByteStr,
    ...
    inner.blockIdsAfter(parentSignature, howMany)
  ...
  override def featureVotes(height: Int): Map[Short, Int] =
    inner.featureVotes(height)

  override def append(diff: Diff, carryFee: Long, block: Block): Unit =
    inner.append(diff, carryFee, block)

  override def rollbackTo(targetBlockId: ByteStr): Either[String, Seq[Block]] =
    inner.rollbackTo(targetBlockId)
}

object CompositeBlockchain {
  def composite(inner: Blockchain, diff: => Option[Diff]): Blockchain =
    new CompositeBlockchain(inner, diff)
  def composite(inner: Blockchain, diff: Diff, carryFee: Long = 0): Blockchain =
