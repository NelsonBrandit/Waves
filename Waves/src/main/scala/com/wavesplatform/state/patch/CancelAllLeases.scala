...
object CancelAllLeases extends ScorexLogging {
  private def invertLeaseInfo(p: Portfolio) =
    Portfolio(0, LeaseBalance(-p.lease.in, -p.lease.out), Map.empty)
  ...
  def apply(s: Blockchain): Diff = {
    ...
    val portfolios = s.collectLposPortfolios {
      case (_, p) if p.lease != LeaseBalance.empty => invertLeaseInfo(p)
    }
    portfolios.keys.foreach(addr =>
      log.info(s"Resetting lease balance for $addr"))
