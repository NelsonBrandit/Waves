...
object BlockAppender extends ScorexLogging with Instrumented {
      ...
      measureSuccessful(
        blockProcessingTimeStats, {
          if (blockchainUpdater.isLastBlockId(newBlock.reference)) {
            appendBlock(blockchainUpdater,
                        utxStorage,
                        pos,
                        time,
                        settings,
                        verify)(newBlock).map(_ =>
  ...
  def apply(blockchainUpdater: BlockchainUpdater with Blockchain,
    ...
    (for {
      ...
      validApplication <- EitherT(
        apply(blockchainUpdater, time, utxStorage, pos, settings, scheduler)(
          newBlock))
  ...
  private val blockProcessingTimeStats =
    Kamon.histogram("single-block-processing-time")
