...
package object appender extends ScorexLogging {
  ...
  private[appender] def appendBlock(
    ...
    val append: Block => Either[ValidationError, Option[Int]] =
      if (verify)
        appendBlock(blockchainUpdater, utxStorage, pos, time, settings) _
  ...
  private[appender] def appendBlock(
    ...
    for {
      ...
      baseHeight <- appendBlock(blockchainUpdater, utxStorage, verify = true)(
        block)
    } yield baseHeight
  ...
  private def blockConsensusValidation(
      ...
      pos: PoSSelector,
      currentTs: Long,
      block: Block)(genBalance: (Int, BlockId) => Either[String, Long])
    ...
    for {
      ...
      effectiveBalance <- genBalance(height, block.reference).left.map(
        GenericError(_))
