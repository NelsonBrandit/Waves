...
object MatcherTool extends ScorexLogging {
  private def collectStats(db: DB): Unit = {
    ...
    def add(prefix: Short,
            e: java.util.Map.Entry[Array[Byte], Array[Byte]]): Unit = {
      result.compute(
        prefix,
        (_, maybePrev) =>
          maybePrev match {
            ...
            case prev =>
              Stats(prev.entryCount + 1,
                    prev.totalKeySize + e.getKey.length,
                    prev.totalValueSize + e.getValue.length)
        }
      )
    }
  ...
  private def deleteLegacyEntries(db: DB): Unit = {
    ...
    db.readWrite(rw =>
      keysToDelete.result().foreach(rw.delete(_, "matcher-legacy-entries")))
  }

  def main(args: Array[String]): Unit = {
    log.info(s"OK, engine start")

    val userConfig = args.headOption.fold(ConfigFactory.empty())(f =>
      ...
      case "ob" =>
        ...
        try {
          ...
          history.headOption.foreach { lastSnapshotNr =>
            val lastSnapshotKey =
              MatcherSnapshotStore.kSnapshot(persistenceId, lastSnapshotNr)
            val lastSnapshotRawData =
              lastSnapshotKey.parse(snapshotDB.get(lastSnapshotKey))
            val lastSnapshot = se
              .deserialize(lastSnapshotRawData, classOf[Snapshot])
              .get
              .data
              .asInstanceOf[OrderBookActor.Snapshot]
              .orderBook
        ...
        try {
          ...
          if (history.isEmpty) log.warn("History is empty")
            ...
            val xs = history.map { seqNr =>
              ...
              val snapshotKey =
                MatcherSnapshotStore.kSnapshot(persistenceId, seqNr)
            ...
            log.info(
              s"Writing the record with seqNr=$firstSeqNr at seqNr=1, sm was $firstSm, will be $updatedSm")
  ...
  object SK {
    def apply[A](suffix: String, extractor: Array[Byte] => Option[A]) =
      new SK(suffix, extractor)

    private def byteStr(b: Array[Byte]) =
      ByteStr.decodeBase58(new String(b, UTF_8)).toOption
    private def addr(b: Array[Byte]) =
      Address.fromString(new String(b, UTF_8)).toOption
