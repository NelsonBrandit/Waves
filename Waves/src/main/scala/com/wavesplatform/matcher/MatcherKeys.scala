...
object MatcherKeys {
  ...
  def orderTxIdsSeqNr(orderId: ByteStr): Key[Int] =
    bytesSeqNr("matcher-order-tx-ids-seq-nr", 9, orderId.arr)
  def orderTxId(orderId: ByteStr, seqNr: Int): Key[ByteStr] =
    Key("matcher-order-tx-id",
        hBytes(10, seqNr, orderId.arr),
        ByteStr(_),
        _.arr)

  def exchangeTransaction(txId: ByteStr): Key[Option[ExchangeTransaction]] =
    Key.opt("matcher-exchange-transaction",
            bytes(11, txId.arr),
            ExchangeTransaction.parse(_).get,
            _.bytes())
  ...
  def finalizedCommon(address: Address, seqNr: Int): Key[Option[Order.Id]] =
    Key.opt("matcher-finalized-common",
            bytes(15, address.bytes.arr ++ Ints.toByteArray(seqNr)),
            ByteStr(_),
            _.arr)

  def finalizedPairSeqNr(address: Address, pair: AssetPair): Key[Int] =
    bytesSeqNr("matcher-finalized-pair-seq-nr",
               16,
               address.bytes.arr ++ pair.bytes)

  def finalizedPair(address: Address,
                    pair: AssetPair,
                    seqNr: Int): Key[Option[Order.Id]] =
    Key.opt(
      "matcher-finalized-pair",
      bytes(17, address.bytes.arr ++ pair.bytes ++ Ints.toByteArray(seqNr)),
      ByteStr(_),
      _.arr)

  def lastCommandTimestamp(address: Address): Key[Option[Long]] =
    Key.opt("matcher-last-command-timestamp",
            bytes(18, address.bytes.arr),
            Longs.fromByteArray,
            Longs.toByteArray)
  ...
  def lpqElement(idx: Long): Key[Option[QueueEventWithMeta]] =
    Key.opt(
      ...
      xs =>
        QueueEventWithMeta(idx,
                           Longs.fromByteArray(xs.take(8)),
                           QueueEvent.fromBytes(xs.drop(8))),
