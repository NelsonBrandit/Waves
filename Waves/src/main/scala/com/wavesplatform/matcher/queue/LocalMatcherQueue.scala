...
class LocalMatcherQueue(settings: Settings, store: LocalQueueStore, time: Time)(
    ...
    timer.schedule(
      new TimerTask {
        override def run(): Unit = {
          val requests =
            store.getFrom(lastUnreadOffset, settings.maxElementsPerPoll)
  ...
  override def lastEventOffset: Future[QueueEventWithMeta.Offset] =
    Future.successful(store.newestOffset.getOrElse(-1L))
...
object LocalMatcherQueue {
  case class Settings(pollingInterval: FiniteDuration,
                      maxElementsPerPoll: Int,
                      cleanBeforeConsume: Boolean)
}
