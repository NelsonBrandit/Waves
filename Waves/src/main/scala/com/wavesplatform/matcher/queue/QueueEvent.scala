...
object QueueEvent {
  ...
  case class Canceled(assetPair: AssetPair, orderId: Order.Id)
      extends QueueEvent
  ...
  def toBytes(x: QueueEvent): Array[Byte] = x match {
    ...
    case Canceled(assetPair, orderId) =>
      (2: Byte) +: (assetPair.bytes ++ orderId.arr)
    case OrderBookDeleted(assetPair) => (3: Byte) +: assetPair.bytes
  }

  def fromBytes(xs: Array[Byte]): QueueEvent = xs.head match {
    case 1 =>
      QueueEvent.Placed(Order.fromBytes(xs(1), xs.slice(2, Int.MaxValue)))
    case 2 =>
