...
sealed class MatcherError(val code: Int, val message: MatcherErrorMessage) {
  ...
  def this(obj: Entity,
           part: Entity,
           klass: Class,
           message: MatcherErrorMessage) = this(
  ...
  override def toString: String =
    s"${getClass.getCanonicalName}(error=$code, message=$text)"
}

case class MatcherErrorMessage(text: String,
                               template: String,
...
object MatcherError {
  ...
  case object MatcherIsStarting
      extends MatcherError(commonEntity,
                           commonEntity,
                           starting,
                           e"System is starting")
  case object MatcherIsStopping
      extends MatcherError(commonEntity,
                           commonEntity,
                           stopping,
                           e"System is shutting down")
  case object OperationTimedOut
      extends MatcherError(commonEntity,
                           commonEntity,
                           timedOut,
                           e"Operation is timed out, please try later")
  case object FeatureNotImplemented
      extends MatcherError(commonEntity,
                           feature,
                           unsupported,
                           e"This feature is not implemented")
  ...
  case object CanNotPersistEvent
      extends MatcherError(
        ...
        commonEntity,
        broken,
        e"Can not persist event, please retry later or contact with the administrator")

  case object CancelRequestIsIncomplete
      extends MatcherError(request,
                           commonEntity,
                           unexpected,
                           e"Either timestamp or orderId must be specified")

  case class UnexpectedMatcherPublicKey(required: PublicKeyAccount,
                                        given: PublicKeyAccount)
      extends MatcherError(
        order,
        pubKey,
        unexpected,
        e"The required matcher public key for this DEX is ${'required -> Base58.encode(
          required.publicKey)}, but given ${'given -> Base58.encode(given.publicKey)}"
      )
  case class UnexpectedSender(required: Address, given: Address)
      extends MatcherError(
        ...
        pubKey,
        unexpected,
        e"The sender ${'given -> given} does not match expected ${'required -> required}")
  case class OrderInvalidSignature(id: Order.Id, details: String)
      extends MatcherError(
        order,
        signature,
        commonClass,
        e"The signature of order ${'id -> id} is invalid: ${'details -> details}")
  ...
  case class UnexpectedFeeAsset(required: Asset, given: Asset)
      extends MatcherError(order, fee, unexpected, e"Required ${'required -> assetIdStr(
        required)} as asset fee, but given ${'given -> assetIdStr(given)}")
  ...
  case class AssetNotFound(assetId: Asset)
      extends MatcherError(asset,
                           commonEntity,
                           notFound,
                           e"The asset ${'assetId -> assetId} not found")
  ...
  case class WrongExpiration(currentTs: Long,
                             minExpirationOffset: Long,
                             givenExpiration: Long)
      extends MatcherError(
        order,
        expiration,
        notEnough,
  ...
  case class OrderCommonValidationFailed(details: String)
      extends MatcherError(order,
                           commonEntity,
                           commonClass,
                           e"The order is invalid: ${'details -> details}")
  case class AssetPairCommonValidationFailed(details: String)
      extends MatcherError(assetPair,
                           commonEntity,
                           commonClass,
                           e"The asset pair is invalid: ${'details -> details}")
  ...
  case class AmountAssetBlacklisted(assetId: Asset)
      extends MatcherError(
        ...
        amount,
        blacklisted,
        e"The amount asset ${'assetId -> assetId} is blacklisted")
  case class PriceAssetBlacklisted(assetId: Asset)
      extends MatcherError(
        asset,
        price,
        blacklisted,
        e"The price asset ${'assetId -> assetId} is blacklisted")
  case class FeeAssetBlacklisted(assetId: Asset)
      extends MatcherError(
        asset,
        fee,
        blacklisted,
        e"The fee asset ${'assetId -> assetId} is blacklisted")
  case class AddressIsBlacklisted(address: Address)
      extends MatcherError(account,
                           commonEntity,
                           blacklisted,
                           e"The account ${'address -> address} is blacklisted")
  case class BalanceNotEnough(required: Map[Asset, Long],
                              actual: Map[Asset, Long])
      extends MatcherError(
        account,
        balance,
        notEnough,
        e"Not enough tradable balance. The order requires ${'required -> formatBalance(
          required)}, but only ${'actual -> formatBalance(actual)} is available"
      )
  ...
  case class OrderDuplicate(id: Order.Id)
      extends MatcherError(account,
                           order,
                           duplicate,
                           e"The order ${'id -> id} has already been placed")
  case class OrderNotFound(id: Order.Id)
      extends MatcherError(order,
                           commonEntity,
                           notFound,
                           e"The order ${'id -> id} not found")
  case class OrderCanceled(id: Order.Id)
      extends MatcherError(order,
                           commonEntity,
                           canceled,
                           e"The order ${'id -> id} is canceled")
  case class OrderFull(id: Order.Id)
      extends MatcherError(order,
                           commonEntity,
                           limitReached,
                           e"The order ${'id -> id} is filled")
  case class OrderFinalized(id: Order.Id)
      extends MatcherError(order,
                           commonEntity,
                           immutable,
                           e"The order ${'id -> id} is finalized")

  case class OrderVersionUnsupported(version: Byte,
                                     requiredFeature: BlockchainFeature)
      extends MatcherError(
        feature,
        order,
        unsupported,
  ...
  case object CancelRequestInvalidSignature
      extends MatcherError(request,
                           signature,
                           commonClass,
                           e"The cancel request has an invalid signature")
  ...
  case class AccountScriptUnexpectResult(address: Address,
  ...
  case class AccountScriptException(address: Address,
                                    errorName: String,
                                    errorText: String)
      extends MatcherError(
        account,
  ...
  case class AssetScriptReturnedError(assetId: IssuedAsset,
  ...
  case class AssetScriptDeniedOrder(assetId: IssuedAsset)
      extends MatcherError(
        ...
        script,
        denied,
        e"The asset's script of ${'assetId -> assetId} rejected the order")
  case class AssetScriptUnexpectResult(assetId: IssuedAsset,
                                       returnedObject: String)
      extends MatcherError(
        account,
        script,
  ...
  case class AssetScriptException(assetId: IssuedAsset,
                                  errorName: String,
                                  errorText: String)
      extends MatcherError(
        account,
