...
case class OrderBookResult(timestamp: Long,
                           pair: AssetPair,
                           bids: Seq[LevelAgg],
                           asks: Seq[LevelAgg])
...
object OrderBookResult {
  ...
  def empty(pair: AssetPair, timestamp: Long) =
    OrderBookResult(timestamp, pair, Seq.empty, Seq.empty)

  def toJson(x: OrderBookResult): String = JsonSerializer.serialize(x)

  class Serializer
      extends StdSerializer[OrderBookResult](classOf[OrderBookResult]) {
    override def serialize(x: OrderBookResult,
                           j: JsonGenerator,
