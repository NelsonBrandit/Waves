...
class ExchangeTransactionCreator(blockchain: Blockchain,
                                 matcherPrivateKey: PrivateKeyAccount,
                                 matcherOrderFeeSettings: OrderFeeSettings) {

  private def calculateMatcherFee(buy: Order,
                                  sell: Order,
                                  executedAmount: Long,
                                  executedPrice: Long): (Long, Long) = {
    ...
    def getNomDenomByAssetType(assetType: AssetType,
                               buyAmount: Long,
                               buyPrice: Long,
                               sellAmount: Long,
                               sellPrice: Long): (Long, Long) = {
      val (buyND, sellND) = assetType match {
        case AssetType.AMOUNT =>
          buy.getReceiveAmount(buyAmount, buyPrice) -> sell.getSpendAmount(
            sellAmount,
            sellPrice)
        case AssetType.PRICE =>
          buy.getSpendAmount(buyAmount, buyPrice) -> sell.getReceiveAmount(
            sellAmount,
            sellPrice)
        case AssetType.RECEIVING =>
          buy.getReceiveAmount(buyAmount, buyPrice) -> sell.getReceiveAmount(
            sellAmount,
            sellPrice)
        case AssetType.SPENDING =>
          buy.getSpendAmount(buyAmount, buyPrice) -> sell.getSpendAmount(
            sellAmount,
            sellPrice)
      }
    ...
    matcherOrderFeeSettings match {
      case PercentSettings(assetType, _) =>
        val (buyFeeNominator, sellFeeNominator) = getNomDenomByAssetType(
          assetType,
          executedAmount,
          executedPrice,
          executedAmount,
          executedPrice)
        val (buyFeeDenominator, sellFeeDenominator) = getNomDenomByAssetType(
          assetType,
          buy.amount,
          buy.price,
          sell.amount,
        ...
        (
          Math.min(buy.matcherFee,
                   calcFee(buy, buyFeeNominator, buyFeeDenominator)),
          Math.min(sell.matcherFee,
                   calcFee(sell, sellFeeNominator, sellFeeDenominator))
        )

      case _ =>
        calcFee(buy, executedAmount, buy.amount) -> calcFee(sell,
                                                            executedAmount,
                                                            sell.amount)
    }
  }
  ...
  def createTransaction(
      ...
      counter: LimitOrder,
      timestamp: Long): Either[ValidationError, ExchangeTransaction] = {
    val executedAmount = LimitOrder.executedAmount(submitted, counter)
    val price = counter.price
    val (buy, sell) = Order.splitByType(submitted.order, counter.order)
    val (buyFee, sellFee) =
      calculateMatcherFee(buy, sell, executedAmount, price)

    val txFee = minFee(blockchain,
                       matcherPrivateKey,
                       counter.order.assetPair,
                       OrderValidator.exchangeTransactionCreationFee)
    if (blockchain.isFeatureActivated(BlockchainFeatures.SmartAccountTrading,
      ...
      for {
        buyV1 <- toV1(buy)
        sellV1 <- toV1(sell)
        tx <- ExchangeTransactionV1.create(matcherPrivateKey,
                                           buyV1,
                                           sellV1,
                                           executedAmount,
                                           price,
  ...
  private def toV1(order: Order): Either[ValidationError, OrderV1] =
    order match {
