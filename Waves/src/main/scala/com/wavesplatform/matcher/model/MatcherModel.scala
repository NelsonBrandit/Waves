...
sealed trait LimitOrder {
  ...
  def requiredBalance: Map[Asset, Long] = Monoid.combine(
    ...
    Map(
      feeAsset -> (if (feeAsset != rcvAsset) fee
                   else (fee - receiveAmount).max(0L)))
  )

  def amountOfPriceAsset: Long =
    (BigDecimal(amount) * price / Order.PriceConstant)
      .setScale(0, RoundingMode.FLOOR)
      .toLong
  def amountOfAmountAsset: Long = correctedAmountOfAmountAsset(amount, price)
  private def executionAmount(counterPrice: Price): Long =
    correctedAmountOfAmountAsset(amount, counterPrice)
  ...
  private def minimalAmountOfAmountAssetByPrice(p: Long): Long =
    (BigDecimal(Order.PriceConstant) / p)
      .setScale(0, RoundingMode.CEILING)
      .toLong
  private def correctedAmountOfAmountAsset(a: Long, p: Long): Long = {
    val settledTotal = (BigDecimal(p) * a / Order.PriceConstant)
      .setScale(0, RoundingMode.FLOOR)
      .toLong
    (BigDecimal(settledTotal) / p * Order.PriceConstant)
      .setScale(0, RoundingMode.CEILING)
      .toLong
  }
}

case class BuyLimitOrder(amount: Long, fee: Long, order: Order)
    extends LimitOrder {
  def partial(amount: Long, fee: Long): BuyLimitOrder =
    copy(amount = amount, fee = fee)
...
case class SellLimitOrder(amount: Long, fee: Long, order: Order)
    extends LimitOrder {
  def partial(amount: Long, fee: Long): SellLimitOrder =
    copy(amount = amount, fee = fee)
...
object LimitOrder {
  ...
  def partialFee(matcherFee: Long,
                 totalAmount: Long,
                 partialAmount: Long): Long = {
  ...
  def executedAmount(submitted: LimitOrder, counter: LimitOrder): Long =
    math.min(submitted.executionAmount(counter.price),
             counter.amountOfAmountAsset)
}
...
object OrderStatus {
  ...
  case object NotFound extends Final {
    val name = "NotFound"
    def json: JsObject =
      Json.obj("status" -> name, "message" -> "The limit order is not found")
  }
  ...
  def finalStatus(lo: LimitOrder, unmatchable: Boolean): Final = {
    ...
    if (unmatchable && filledAmount > 0) Filled(filledAmount)
    else Cancelled(filledAmount)
  }
}
...
object Events {
  ...
  case class OrderExecuted(submitted: LimitOrder,
                           counter: LimitOrder,
                           timestamp: Long)
      extends Event {
    lazy val executedAmount: Long =
      LimitOrder.executedAmount(submitted, counter)

    def counterRemainingAmount: Long =
      math.max(counter.amount - executedAmount, 0)
    def counterExecutedFee: Long =
      LimitOrder.partialFee(counter.order.matcherFee,
                            counter.order.amount,
                            executedAmount)
    def counterRemainingFee: Long =
      math.max(counter.fee - counterExecutedFee, 0)
    def counterRemaining: LimitOrder =
      counter.partial(amount = counterRemainingAmount,
                      fee = counterRemainingFee)

    def submittedRemainingAmount: Long =
      math.max(submitted.amount - executedAmount, 0)
    def submittedExecutedFee: Long =
      LimitOrder.partialFee(submitted.order.matcherFee,
                            submitted.order.amount,
                            executedAmount)
    def submittedRemainingFee: Long =
      math.max(submitted.fee - submittedExecutedFee, 0)
    def submittedRemaining: LimitOrder =
      submitted.partial(amount = submittedRemainingAmount,
                        fee = submittedRemainingFee)
  }

  case class OrderAdded(order: LimitOrder) extends Event

  case class OrderCanceled(limitOrder: LimitOrder, unmatchable: Boolean)
      extends Event
  ...
  object BalanceChanged {
    ...
    case class Changes(updatedPortfolio: Portfolio,
                       changedAssets: Set[Option[Asset]])
  }
}
