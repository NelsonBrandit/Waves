...
class OrderBook private (private[OrderBook] val bids: OrderBook.Side,
    ...
    allOrders.collectFirst {
      case lo if lo.order.id() == orderId =>
        (if (lo.order.orderType == OrderType.BUY) bids else asks)
          .remove(lo.order.price, lo.order.id())
  ...
  def cancelAll(): Seq[OrderCanceled] = {
    val canceledOrders =
      allOrders.map(lo => OrderCanceled(lo, unmatchable = false)).toSeq
    bids.clear()
    asks.clear()
    canceledOrders
  }
  ...
  def add(o: Order, ts: Long): Seq[Event] =
    (o.orderType match {
      case OrderType.BUY =>
        doMatch(ts, buy, LimitOrder(o), Seq.empty, bids, asks)
      case OrderType.SELL =>
        doMatch(ts, sell, LimitOrder(o), Seq.empty, asks, bids)
    }).reverse

  def snapshot: Snapshot = Snapshot(bids.toMap, asks.toMap)
  def aggregatedSnapshot: AggregatedSnapshot =
    AggregatedSnapshot(bids.aggregated.toSeq, asks.aggregated.toSeq)

  override def toString =
    s"""{"bids":${formatSide(bids)},"asks":${formatSide(asks)}}"""
}
...
object OrderBook {
  ...
  case class AggregatedSnapshot(bids: Seq[LevelAgg] = Seq.empty,
                                asks: Seq[LevelAgg] = Seq.empty)
  ...
  implicit class SideExt(val side: Side) extends AnyVal {
    ...
    final def removeBest(): LimitOrder = side.headOption match {
      case l if l.forall(_._2.isEmpty) =>
        throw new IllegalArgumentException(
          "Cannot remove the best element from an empty level")
    ...
    def replaceBest(newBestAsk: LimitOrder): Side = {
      ...
      require(level.nonEmpty,
              "Cannot replace the best element of an empty level")
      side += (price -> (newBestAsk +: level.tail))
    }

    def remove(price: Price, orderId: ByteStr): LimitOrder = {
      val (toRemove, toKeep) =
        side.getOrElse(price, Vector.empty).partition(_.order.id() == orderId)
  ...
  private object buy extends ((Long, Long) => Boolean) {
    def apply(submittedPrice: Long, counterPrice: Long): Boolean =
      submittedPrice >= counterPrice
  ...
  private def doMatch(
    ...
    if (!submitted.order.isValid(eventTs))
      OrderCanceled(submitted, false) +: prevEvents
    else
      counterSide.best match {
        case counter
            if counter.forall(c => !canMatch(submitted.price, c.price)) =>
          submittedSide += submitted.price -> (submittedSide.getOrElse(
            submitted.price,
            Vector.empty) :+ submitted)
        ...
        case Some(counter) =>
          if (!submitted.isValid(counter.price)) {
            ...
            doMatch(eventTs,
                    canMatch,
                    submitted,
                    OrderCanceled(counter, false) +: prevEvents,
                    submittedSide,
                    counterSide)
          } else {
            ...
            if (x.counterRemaining.isValid) {
              ...
              if (x.submittedRemaining.isValid)
                OrderCanceled(x.submittedRemaining, true) +: newEvents
              else newEvents
            } else {
              counterSide.removeBest()
              if (x.submittedRemaining.isValid) {
  ...
  private def formatSide(side: Side) =
    side
      .map {
        case (price, level) =>
          s""""$price":${level
            .map(lo => s""""${lo.order.id()}"""")
            .mkString("[", ",", "]")}"""
      }
      .mkString("{", ",", "}")

  val bidsOrdering: Ordering[Long] = (x: Long, y: Long) =>
    -Ordering.Long.compare(x, y)
  val asksOrdering: Ordering[Long] = (x: Long, y: Long) =>
    Ordering.Long.compare(x, y)

  import com.wavesplatform.transaction.assets.exchange.OrderJson.orderFormat

  private def limitOrder(remainingAmount: Long,
                         remainingFee: Long,
                         o: Order): LimitOrder = o.orderType match {
  ...
  def empty: OrderBook =
    new OrderBook(mutable.TreeMap.empty(bidsOrdering),
                  mutable.TreeMap.empty(asksOrdering))

  private def transformSide(side: SideSnapshot,
                            expectedSide: OrderType,
                            ordering: Ordering[Long]): Side = {
    ...
    for ((p, level) <- side) {
      ...
      for (lo <- level) {
        require(
          ...
          s"Expecting $expectedSide only orders in bid list, but ${lo.order
            .id()} is a ${lo.order.orderType} order")
        v += lo
      }
      bidMap += p -> v.result()
    }
