...
class EventSerializers extends SerializerWithStringManifest {
  ...
  private val bytesWritten =
    Kamon.histogram("matcher.serialization.bytes-written")
  ...
  private def encodeAndMeasure(manifest: String,
  ...
  override def toBinary(o: AnyRef): Array[Byte] = o match {
    case s: OrderBookActor.Snapshot =>
      encodeAndMeasure("order-book-snapshot", s)
    case obc: MatcherActor.OrderBookCreated =>
      encodeAndMeasure("order-book-created", obc)
    case x: MatcherActor.Snapshot =>
      encodeAndMeasure("matcher-actor-snapshot", x)
  }
...
object EventSerializers {
  ...
  private def mkOrderBookCreated(a1: String, a2: String) =
    OrderBookCreated(AssetPair.createAssetPair(a1, a2).get)
  private def orderBookToPair(obc: OrderBookCreated) =
    (obc.pair.amountAssetStr, obc.pair.priceAssetStr)
  ...
  implicit val assetPair: Format[AssetPair] = ((__ \ "a1").format[String] and
    (__ \ "a2").format[String])((a, b) => AssetPair.createAssetPair(a, b).get,
                                x => (x.amountAssetStr, x.priceAssetStr))

  implicit val matcherSnapshot: Format[MatcherActor.Snapshot] =
    Json.format[MatcherActor.Snapshot]

  implicit val cacheFormat: Format[Map[String, (Price, Price)]] =
    new Format[Map[String, (Long, Long)]] {
  ...
  implicit val snapshotFormat: Format[Snapshot] = Format(
    ((JsPath \ "n").readNullable[Long].map(_.getOrElse(-1L)) and (JsPath \ "o")
      .read[OrderBook.Snapshot])(Snapshot),
