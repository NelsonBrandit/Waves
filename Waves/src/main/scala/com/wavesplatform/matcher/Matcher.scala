...
import com.wavesplatform.matcher.api.{
  MatcherApiRoute,
...
class Matcher(actorSystem: ActorSystem,
  ...
  private val status: AtomicReference[Status] = new AtomicReference(
    Status.Starting)
  private var currentOffset = -1L // Used only for REST API

  private val blacklistedAssets: Set[IssuedAsset] =
    matcherSettings.blacklistedAssets
      .map { assetName =>
        val asset = AssetPair
        ...
        asset match {
          case Waves =>
            throw new IllegalArgumentException("Can't blacklist the main coin")
          case a @ IssuedAsset(_) => a
        }
      }

  private val pairBuilder =
    new AssetPairBuilder(settings.matcherSettings, blockchain)
  private val orderBookCache =
    new ConcurrentHashMap[AssetPair, OrderBook.AggregatedSnapshot](1000,
                                                                   0.9f,
                                                                   10)
  ...
  private val orderBooks = new AtomicReference(
    Map.empty[AssetPair, Either[Unit, ActorRef]])
  ...
  private val marketStatuses =
    new ConcurrentHashMap[AssetPair, MarketStatus](1000, 0.9f, 10)

  private def updateOrderBookCache(assetPair: AssetPair)(
      newSnapshot: OrderBook.AggregatedSnapshot): Unit = {
    ...
    OrderBookActor.props(
      ...
      tx =>
        exchangeTxPool.execute(() =>
          if (utx.putIfNew(tx).isRight) allChannels.broadcastTx(tx)),
      matcherSettings,
      transactionCreator.createTransaction,
      time
    )

  private val matcherQueue: MatcherQueue =
    settings.matcherSettings.eventsQueue.tpe match {
      case "local" =>
        log.info("Events will be stored locally")
        new LocalMatcherQueue(settings.matcherSettings.eventsQueue.local,
                              new LocalQueueStore(db),
                              time)(actorSystem.dispatcher)

      case "kafka" =>
        log.info("Events will be stored in Kafka")
        new KafkaMatcherQueue(settings.matcherSettings.eventsQueue.kafka)(
          materializer)
  ...
  private def validateOrder(o: Order) = {
    ...
    for {
      ...
      _ <- OrderValidator.blockchainAware(blockchain,
                                          ...
                                          matcherSettings.orderFee)(o)
      _ <- pairBuilder
        .validateAssetPair(o.assetPair)
        .left
        .map(x => MatcherError.AssetPairCommonValidationFailed(x))
    } yield o
  }
  ...
  lazy val matcherApiRoutes: Seq[MatcherApiRoute] = Seq(
    MatcherApiRoute(
      ...
      time,
      () => currentOffset,
      ExchangeTransactionCreator.minAccountFee(blockchain,
                                               matcherPublicKey.toAddress)
    )
  )
      ...
      matcherSettings, {
        ...
        case Right((self, oldestSnapshotOffset)) =>
          ...
          matcherQueue.startConsume(
            ...
            eventWithMeta => {
              log.debug(
                s"[offset=${eventWithMeta.offset}, ts=${eventWithMeta.timestamp}] Consumed ${eventWithMeta.event}")
  ...
  private lazy val blacklistedAddresses =
    settings.matcherSettings.blacklistedAddresses
      .map(Address.fromString(_).explicitGet())
  private lazy val matcherPublicKey = PublicKeyAccount(
    matcherPrivateKey.publicKey)
  ...
  def shutdown(): Unit = {
    ...
    Await.result(
      gracefulStop(matcher, stopMatcherTimeout, MatcherActor.Shutdown),
      stopMatcherTimeout)
  ...
  def runMatcher(): Unit = {
    ...
    log.info(
      s"Starting matcher on: ${matcherSettings.bindAddress}:${matcherSettings.port} ...")

    val combinedRoute = CompositeHttpService(matcherApiTypes,
                                             matcherApiRoutes,
                                             restAPISettings).compositeRoute
    matcherServerBinding = Await.result(
    ...
    actorSystem.actorOf(MatcherTransactionWriter.props(db, matcherSettings),
                        MatcherTransactionWriter.name)

    val startGuard = for {
      _ <- waitSnapshotsRestored(
        settings.matcherSettings.snapshotsLoadingTimeout)
  ...
  private def getLastOffset(
      deadline: Deadline): Future[QueueEventWithMeta.Offset] =
    matcherQueue.lastEventOffset.recoverWith {
      case _: AskTimeoutException =>
        if (deadline.isOverdue())
          Future.failed(
            new TimeoutException("Can't get last offset from queue"))
        else getLastOffset(deadline)
    }

  private def waitOffsetReached(lastQueueOffset: QueueEventWithMeta.Offset,
    ...
    def loop(): Unit = {
      if (currentOffset >= lastQueueOffset) p.trySuccess(())
        ...
        p.tryFailure(new TimeoutException("Can't process all events in time"))
...
object Matcher extends ScorexLogging {
  ...
  def apply(actorSystem: ActorSystem,
    ...
    try {
      ...
      val matcher = new Matcher(actorSystem,
                                time,
                                utx,
                                allChannels,
                                blockchain,
