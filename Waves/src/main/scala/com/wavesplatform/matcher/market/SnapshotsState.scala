...
/**
  ...
  */
case class SnapshotsState private (
    startOffsetToSnapshot: EventOffset,
    snapshotOffsets: Map[AssetPair, EventOffset],
    nearestSnapshotOffsets: SortedSet[(AssetPair, EventOffset)]) {
  import SnapshotsState._

  val nearestSnapshotOffset: Option[(AssetPair, EventOffset)] =
    nearestSnapshotOffsets.headOption // Caching log(N) operations

  def requiredSnapshot(
      offset: EventOffset): Option[(AssetPair, SnapshotsState)] =
    if (offset > startOffsetToSnapshot) nearestSnapshotOffset.collect {
      case (assetPair, nearestSnapshotNr) if offset >= nearestSnapshotNr =>
        assetPair -> copy(
          nearestSnapshotOffsets = this.nearestSnapshotOffsets.tail)
    } else None

  def updated(assetPair: AssetPair,
              snapshotEventNr: EventOffset,
              lastGlobalEventNr: EventOffset,
              interval: EventOffset): SnapshotsState = {
    val nextSnapshotOffset = {
      val z = (snapshotEventNr / interval) * interval + snapshotOffset(
        assetPair,
        interval)
...
object SnapshotsState {
  val empty = SnapshotsState(
    ...
    nearestSnapshotOffsets = SortedSet.empty(
      Ordering.by[(AssetPair, EventOffset), (EventOffset, String)] {
  ...
  def apply(startOffsetToSnapshot: Long,
    ...
    nearestSnapshotOffsets = empty.nearestSnapshotOffsets ++ currentOffsets
      .map { // ++ to preserve ordering
        case (assetPair, o) =>
          val nextSnapshotOffset = {
            val z = (o / interval) * interval + snapshotOffset(assetPair,
                                                               interval)
  ...
  private def snapshotOffset(assetPair: AssetPair,
                             interval: EventOffset): Long =
    math.abs(assetPair.key.hashCode % interval)
}
