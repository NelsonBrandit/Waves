...
class MatcherTransactionWriter(db: DB) extends Actor with ScorexLogging {
  ...
  override def preStart(): Unit = {
    context.system.eventStream
      .subscribe(self, classOf[ExchangeTransactionCreated])
  }
  ...
  private def saveExchangeTx(tx: ExchangeTransaction): Unit = db.readWrite {
    rw =>
      log.trace(
        s"Appending ${tx.id()} to orders [${tx.buyOrder.idStr()}, ${tx.sellOrder.idStr()}]")
...
object MatcherTransactionWriter {
  ...
  def props(db: DB, settings: MatcherSettings): Props =
    Props(new MatcherTransactionWriter(db))
