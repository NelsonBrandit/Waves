...
class MatcherActor(
  ...
  private def orderBook(pair: AssetPair) =
    Option(orderBooks.get()).flatMap(_.get(pair))

  private def getAssetName(asset: Asset,
                           desc: Option[AssetDescription]): String =
    asset match {
      case Waves => AssetPair.WavesName
      case _     => desc.fold("Unknown")(d => new String(d.name, Charsets.UTF_8))
    }

  private def getAssetInfo(asset: Asset,
                           desc: Option[AssetDescription]): Option[AssetInfo] =
    asset.fold(Option(8))(_ => desc.map(_.decimals)).map(AssetInfo)

  private def getAssetDescriptionByAssetId(
      assetId: Asset): Option[AssetDescription] = assetId match {
    case Waves                  => None
  ...
  private def createOrderBook(pair: AssetPair): ActorRef = {
    ...
    val orderBook = context.watch(
      context.actorOf(orderBookActorProps(pair, self),
  ...
  /**
    ...
    */
  private def runFor(eventWithMeta: QueueEventWithMeta)(
          ...
          snapshotsState.requiredSnapshot(offset).foreach {
            case (assetPair, updatedSnapshotState) =>
              ...
              orderBooks.get
                .get(assetPair)
                .flatMap(_.toOption)
                .foreach(_ ! SaveSnapshot(offset))
              snapshotsState = updatedSnapshotState
          }
        case Some(Left(_)) =>
          s ! OrderBookUnavailable(MatcherError.OrderBookUnavailable(assetPair))
        case None =>
  ...
  private def forwardToOrderBook: Receive = {
    ...
    case request: QueueEventWithMeta =>
      request.event match {
        ...
        case _ =>
          runFor(request)(
            (sender, orderBook) => orderBook.tell(request, sender))
      }
    ...
    case Shutdown =>
      shutdownStatus = shutdownStatus.copy(
        initiated = true,
        onComplete = () => context.stop(self))

      context.children.foreach(context.unwatch)
      context.become(snapshotsCommands orElse shutdownFallback)

      if (lastSnapshotSequenceNr < lastSequenceNr)
        saveSnapshot(Snapshot(tradedPairs.keySet))
      else {
        log.debug(
          s"No changes, lastSnapshotSequenceNr = $lastSnapshotSequenceNr, lastSequenceNr = $lastSequenceNr")
        shutdownStatus = shutdownStatus.copy(
    ...
    case OrderBookSnapshotUpdated(assetPair, eventNr) =>
      snapshotsState = snapshotsState.updated(assetPair,
                                              eventNr,
                                              lastProcessedNr,
                                              settings.snapshotsInterval)
  }
  ...
  override def receiveRecover: Receive = {
    ...
    case RecoveryCompleted =>
      if (orderBooks.get().isEmpty) {
        ...
        log.info(
          s"Recovery completed, waiting order books to restore: ${obs.keys.mkString(", ")}")
        context.become(
          collectOrderBooks(obs.size, Long.MaxValue, Long.MinValue, Map.empty))
      }
  }
  ...
  private def collectOrderBooks(
    ...
    case OrderBookSnapshotUpdated(assetPair, snapshotEventNr) =>
      ...
      val updatedCurrentOffsets =
        currentOffsets.updated(assetPair, snapshotEventNr)

      if (updatedRestOrderBooksNumber > 0)
        context.become(
          collectOrderBooks(updatedRestOrderBooksNumber,
                            updatedOldestEventNr,
                            updatedNewestEventNr,
                            updatedCurrentOffsets))
      else
        becomeWorking(updatedOldestEventNr,
                      updatedNewestEventNr,
                      updatedCurrentOffsets)
  ...
  private def becomeWorking(
      oldestEventNr: EventOffset,
      newestEventNr: EventOffset,
      currentOffsets: Map[AssetPair, EventOffset]): Unit = {
    ...
    log.info(
      s"All snapshots are loaded, oldestEventNr: $oldestEventNr, newestEventNr: $newestEventNr")
    log.trace(
      s"Expecting snapshots at: ${snapshotsState.nearestSnapshotOffsets}")
  ...
  private def snapshotsCommands: Receive = {
    case SaveSnapshotSuccess(metadata) =>
      ...
      deleteSnapshots(
        SnapshotSelectionCriteria.Latest.copy(
          maxSequenceNr = metadata.sequenceNr - 1))
  ...
  override def receiveCommand: Receive =
    forwardToOrderBook orElse snapshotsCommands
...
object MatcherActor {
  ...
  private case class ShutdownStatus(initiated: Boolean,
    ...
    def isCompleted: Boolean =
      initiated && oldMessagesDeleted && oldSnapshotsDeleted
  ...
  def compare(buffer1: Option[Array[Byte]],
              buffer2: Option[Array[Byte]]): Int = {
