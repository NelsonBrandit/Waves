...
class AssetPairBuilder(settings: MatcherSettings, blockchain: Blockchain) {
  ...
  private def isNotBlacklisted(asset: IssuedAsset): Boolean =
    blockchain.assetDescription(asset).exists { d =>
      settings.blacklistedNames.forall(
        _.findFirstIn(new String(d.name, UTF_8)).isEmpty)
    }
  ...
  def validateAssetPair(pair: AssetPair): Either[String, AssetPair] =
    validate.measure(for {
      _ <- cond(pair.amountAsset != pair.priceAsset,
                (),
