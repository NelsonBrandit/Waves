...
object JsonSerializer {
  ...
  coreTypeSerializers.addDeserializer(classOf[AssetPair],
                                      new AssetPairDeserializer)
  ...
  private class AssetPairDeserializer
      extends StdDeserializer[AssetPair](classOf[AssetPair]) {
    override def deserialize(p: JsonParser,
                             ctxt: DeserializationContext): AssetPair = {
      val node = p.getCodec.readTree[JsonNode](p)
      def readAssetId(fieldName: String) = {
        val x = node.get(fieldName).asText(AssetPair.WavesName)
        if (x == AssetPair.WavesName) Waves
        else IssuedAsset(ByteStr.decodeBase58(x).get)
      }
