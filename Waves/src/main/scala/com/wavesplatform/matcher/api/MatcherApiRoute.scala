...
case class MatcherApiRoute(
  ...
  private implicit val timeout: Timeout =
    wavesSettings.matcherSettings.actorResponseTimeout
  ...
  private def unavailableOrderBookBarrier(p: AssetPair): Directive0 =
    orderBook(p) match {
      case Some(Left(_)) =>
        complete(OrderBookUnavailable(MatcherError.OrderBookUnavailable(p)))
      case _ => pass
    }

  private def withAssetPair(p: AssetPair,
                            redirectToInverse: Boolean = false,
                            suffix: String = ""): Directive1[AssetPair] =
      ...
      case Left(e) if redirectToInverse =>
        assetPairBuilder
          ...
          .fold(
            ...
            _ =>
              redirect(
                s"/matcher/orderbook/${p.priceAssetStr}/${p.amountAssetStr}$suffix",
  ...
  private def withCancelRequest(f: CancelOrderRequest => Route): Route =
    post {
      entity(as[CancelOrderRequest]) { req =>
        if (req.isSignatureValid()) f(req)
        else complete(CancelRequestInvalidSignature)
    ...
    (headerValueByName("Timestamp") & headerValueByName("Signature")).tflatMap {
      case (timestamp, sig) =>
        require(crypto.verify(
                  Base58.decode(sig).get,
                  publicKey.publicKey ++ Longs.toByteArray(timestamp.toLong),
                  publicKey.publicKey),
                "Incorrect signature")
        pass
    }
  ...
  @inline
  private def askAddressActor[A: ClassTag](
      sender: Address,
      msg: AddressActor.Command): Future[A] = {
  ...
  @ApiImplicitParams(
    Array(
      new ApiImplicitParam(name = "amountAsset",
                           value = "Amount Asset Id in Pair, or 'WAVES'",
                           dataType = "string",
                           paramType = "path"),
      new ApiImplicitParam(name = "priceAsset",
                           value = "Price Asset Id in Pair, or 'WAVES'",
                           dataType = "string",
                           paramType = "path"),
      new ApiImplicitParam(name = "depth",
                           value =
                             "Limit the number of bid/ask records returned",
                           required = false,
                           dataType = "integer",
                           paramType = "query")
    ))
  ...
  def marketStatus: Route = (path("orderbook" / AssetPairPM / "status") & get) {
    p =>
      withAssetPair(p, redirectToInverse = true) { pair =>
        getMarketStatus(pair).fold(
          complete(StatusCodes.NotFound -> Json.obj(
            "message" -> "There is no information about this asset pair"))) {
          ms =>
          ...
          complete {
            placeTimer.measureFuture {
              orderValidator(order) match {
                case Right(_) =>
                  placeTimer.measureFuture(
                    askAddressActor[MatcherResponse](
                      order.sender,
                      AddressActor.PlaceOrder(order)))
                case Left(error) =>
                  Future.successful[MatcherResponse](OrderRejected(error))
              }
            }
          }
        }
  ...
  private def handleCancelRequest(assetPair: Option[AssetPair],
                                  sender: Address,
                                  orderId: Option[ByteStr],
                                  timestamp: Option[Long]): Route =
    complete((timestamp, orderId) match {
      case (Some(ts), None) =>
        askAddressActor[MatcherResponse](
          sender,
          AddressActor.CancelAllOrders(assetPair, ts))
      case (None, Some(oid)) =>
        askAddressActor[MatcherResponse](sender, AddressActor.CancelOrder(oid))
  ...
  @ApiOperation(
    ...
    notes =
      "This method is deprecated and doesn't work anymore. Please don't use it.",
    httpMethod = "POST",
    produces = "application/json",
    consumes = "application/json"
  )
  ...
  def historyDelete: Route =
    (path("orderbook" / AssetPairPM / "delete") & post) { _ =>
      json[CancelOrderRequest] { req =>
        req.orderId.fold[MatcherResponse](
          NotImplemented(MatcherError.FeatureNotImplemented))(OrderDeleted)
      }
    }

  private def loadOrders(address: Address,
                         pair: Option[AssetPair],
                         activeOnly: Boolean): Route = complete {
    askAddressActor[Seq[(ByteStr, OrderInfo[OrderStatus])]](
      address,
      AddressActor.GetOrders(pair, activeOnly))
      .map(orders =>
  ...
  @ApiImplicitParams(
    Array(
      ...
      new ApiImplicitParam(name = "priceAsset",
                           ...
                           dataType = "string",
                           paramType = "path"),
      new ApiImplicitParam(name = "publicKey",
                           value = "Public Key",
                           required = true,
                           dataType = "string",
                           paramType = "path"),
      ...
      new ApiImplicitParam(name = "Timestamp",
                           value = "Timestamp",
                           required = true,
                           dataType = "integer",
                           paramType = "header"),
      new ApiImplicitParam(name = "Signature",
                           value =
                             "Signature of [Public Key ++ Timestamp] bytes",
                           required = true,
                           dataType = "string",
                           paramType = "header")
    ))
  def getAssetPairAndPublicKeyOrderHistory: Route =
    (path("orderbook" / AssetPairPM / "publicKey" / PublicKeyPM) & get) {
      (p, publicKey) =>
        withAssetPair(p, redirectToInverse = true, s"/publicKey/$publicKey") {
          pair =>
  ...
  def getPublicKeyOrderHistory: Route =
    (path("orderbook" / PublicKeyPM) & get) { publicKey =>
      parameters('activeOnly.as[Boolean].?) { activeOnly =>
        signedGet(publicKey) {
  ...
  @ApiImplicitParams(
    Array(
      new ApiImplicitParam(name = "orderId",
                           value = "Order Id",
                           required = true,
                           dataType = "string",
                           paramType = "path")
    ))
  def forceCancelOrder: Route =
    (path("orders" / "cancel" / ByteStrPM) & post & withAuth) { orderId =>
      DBUtils.order(db, orderId) match {
        case Some(order) =>
          handleCancelRequest(None, order.sender, Some(orderId), None)
        case None =>
          complete(OrderCancelRejected(MatcherError.OrderNotFound(orderId)))
      }
    }
  ...
  def getAllOrderHistory: Route =
    (path("orders" / AddressPM) & get & withAuth) { address =>
      parameters('activeOnly.as[Boolean].?) { activeOnly =>
  ...
  def tradableBalance: Route =
    (path("orderbook" / AssetPairPM / "tradableBalance" / AddressPM) & get) {
      (pair, address) =>
        withAssetPair(pair,
                      redirectToInverse = true,
                      s"/tradableBalance/$address") { pair =>
          complete {
            askAddressActor[Map[Asset, Long]](
              address,
              AddressActor.GetTradableBalance(pair))
  ...
  @ApiOperation(value = "Reserved Balance",
  ...
  def reservedBalance: Route =
    (path("balance" / "reserved" / PublicKeyPM) & get) { publicKey =>
      signedGet(publicKey) {
        complete {
          askAddressActor[Map[Asset, Long]](publicKey,
                                            AddressActor.GetReservedBalance)
            .map(stringifyAssetIds)
        }
      }
    }
  ...
  def orderStatus: Route = (path("orderbook" / AssetPairPM / ByteStrPM) & get) {
    (p, orderId) =>
      withAssetPair(p, redirectToInverse = true, s"/$orderId") { _ =>
        complete(DBUtils
          .order(db, orderId)
          .fold[Future[OrderStatus]](Future.successful(OrderStatus.NotFound)) {
            order =>
              askAddressActor[OrderStatus](order.sender,
  ...
  @ApiOperation(value = "Remove Order Book for a given Asset Pair",
                notes = "Remove Order Book for a given Asset Pair",
                httpMethod = "DELETE")
  @ApiImplicitParams(
    Array(
      new ApiImplicitParam(name = "amountAsset",
  ...
  def orderBookDelete: Route =
    (path("orderbook" / AssetPairPM) & delete & withAuth) { p =>
      withAssetPair(p) { pair =>
        complete(storeEvent(QueueEvent.OrderBookDeleted(pair)).map(_ =>
          SimpleResponse(StatusCodes.Accepted, "Deleting order book")))
      }
    }
  ...
  @ApiOperation(
    ...
    notes =
      "Get all exchange transactions created by DEX on execution of the given order",
    httpMethod = "GET")
  @ApiImplicitParams(
    Array(
      new ApiImplicitParam(name = "orderId",
  ...
  def getTransactionsByOrder: Route = (path("transactions" / ByteStrPM) & get) {
    orderId =>
      complete(
        StatusCodes.OK -> Json.toJson(
          DBUtils.transactionsForOrder(db, orderId)))
  }

  @Path("/debug/currentOffset")
  @ApiOperation(value = "Get a current offset in the queue",
                notes = "",
                httpMethod = "GET")
  def getCurrentOffset: Route =
    (path("debug" / "currentOffset") & get & withAuth) {
      complete(StatusCodes.OK -> currentOffset())
    }

  @Path("/debug/oldestSnapshotOffset")
  @ApiOperation(value = "Get the oldest snapshot's offset in the queue",
                notes = "",
                httpMethod = "GET")
  def getOldestSnapshotOffset: Route =
    (path("debug" / "oldestSnapshotOffset") & get & withAuth) {
      complete {
  ...
  @ApiOperation(value = "Get all snapshots' offsets in the queue",
                notes = "",
                httpMethod = "GET")
  def getAllSnapshotOffsets: Route =
    (path("debug" / "allSnapshotOffsets") & get & withAuth) {
      complete {
        (matcher ? GetSnapshotOffsets).mapTo[SnapshotOffsetsResponse].map { x =>
          val js = Json.obj(
            x.offsets.map {
