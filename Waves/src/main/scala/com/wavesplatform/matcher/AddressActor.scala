...
import com.wavesplatform.matcher.model.{
  LimitOrder,
  OrderInfo,
  OrderStatus,
  OrderValidator
...
class AddressActor(
  ...
  protected override def log =
    LoggerFacade(LoggerFactory.getLogger(s"AddressActor[$owner]"))
  ...
  private def reserve(limitOrder: LimitOrder): Unit = {
    ...
    for ((id, b) <- limitOrder.requiredBalance if b != 0) {
      ...
      log.trace(
        s"${limitOrder.order.id()}, ${assetIdStr(id)}: $prevBalance + $b = $newBalance")
      openVolume += id -> newBalance
    }
  }

  private def release(orderId: ByteStr): Unit =
    for (limitOrder <- activeOrders.get(orderId);
         (id, b) <- limitOrder.requiredBalance if b != 0) {
      val prevBalance = openVolume(id)
      val newBalance = prevBalance - b
      log.trace(
        s"${limitOrder.order.id()}, ${assetIdStr(id)}: $prevBalance - $b = $newBalance")
  ...
  private def updateTimestamp(newTimestamp: Long): Unit =
    if (newTimestamp > latestOrderTs) {
      latestOrderTs = newTimestamp
    }

  private def tradableBalance(assetId: Asset): Long =
    spendableBalance(assetId) - openVolume(assetId)
  ...
  private val validator =
    OrderValidator.accountStateAware(
      ...
      activeOrders.size,
      id =>
        activeOrders.contains(id) || orderDB.containsInfo(id) || hasOrder(id)) _
  ...
  private def handleCommands: Receive = {
    ...
    case CancelAllOrders(maybePair, _) =>
      val batchCancelFutures = for {
        ...
        storeCanceled(lo.order.assetPair, lo.order.id()).map(lo.order.id() -> _)

      Future
        .sequence(batchCancelFutures)
        .map(_.toMap)
        .map(api.BatchCancelCompleted)
        .pipeTo(sender())

    case CancelExpiredOrder(id) =>
      expiration.remove(id)
      for (lo <- activeOrders.get(id)) {
        if ((lo.order.expiration - time
              .correctedTime()).max(0L).millis <= ExpirationThreshold) {
  ...
  private def store(id: ByteStr,
                    event: QueueEvent,
                    eventCache: MutableMap[ByteStr, Promise[Resp]],
                    error: Resp): Future[Resp] = {
  ...
  private def storeCanceled(assetPair: AssetPair, id: ByteStr): Future[Resp] =
    store(id,
          QueueEvent.Canceled(assetPair, id),
          pendingCancellation,
          api.OrderCancelRejected(MatcherError.CanNotPersistEvent))
  private def storePlaced(order: Order): Future[Resp] = {
    import com.wavesplatform.matcher.error._
    store(order.id(),
          QueueEvent.Placed(order),
          pendingPlacement,
          api.OrderRejected(MatcherError.CanNotPersistEvent))
  }

  private def confirmPlacement(order: Order): Unit =
    for (p <- pendingPlacement.remove(order.id())) {
  ...
  private def handleStatusRequests: Receive = {
    case GetOrderStatus(orderId) =>
      sender() ! activeOrders
        .get(orderId)
        .fold[OrderStatus](orderDB.status(orderId))(activeStatus)
    case GetOrders(maybePair, onlyActive) =>
      log.trace(
        s"Loading ${if (onlyActive) "active" else "all"} ${maybePair.fold("")(_.toString + " ")}orders")
      val matchingActiveOrders = (for {
        lo <- activeOrders.values
        if maybePair.forall(_ == lo.order.assetPair)
      } yield
        lo.order
          .id() -> OrderInfo(lo.order.orderType,
                             lo.order.amount,
                             lo.order.price,
                             lo.order.timestamp,
                             activeStatus(lo),
                             lo.order.assetPair)).toSeq.sorted

      log.trace(s"Collected ${matchingActiveOrders.length} active orders")

      sender() ! (if (onlyActive) matchingActiveOrders
                  else
                    orderDB.loadRemainingOrders(owner,
                                                maybePair,
                                                matchingActiveOrders))
    case GetTradableBalance(pair) =>
      sender() ! Set(pair.amountAsset, pair.priceAsset)
        .map(id => id -> tradableBalance(id))
        .toMap
  ...
  private def handleExecutionEvents: Receive = {
    ...
    case e @ OrderExecuted(submitted, counter, _) =>
      log.trace(s"OrderExecuted(${submitted.order.id()}, ${counter.order
        .id()}), amount = ${e.executedAmount}")
    ...
    case OrderCanceled(lo, unmatchable) =>
      ...
      pendingCancellation
        .remove(lo.order.id())
        .foreach(_.success(api.OrderCanceled(lo.order.id())))
  ...
  private def scheduleExpiration(order: Order): Unit = {
    ...
    log.trace(s"Order ${order.id()} will expire in ${JDuration.ofMillis(
      timeToExpiration)}, at ${Instant.ofEpochMilli(order.expiration)}")
    expiration +=
      order.id() -> context.system.scheduler.scheduleOnce(
        timeToExpiration.millis,
        self,
  ...
  private def handleOrderExecuted(remaining: LimitOrder): Unit =
    if (remaining.order.sender.toAddress == owner) {
  ...
  private def handleOrderTerminated(lo: LimitOrder,
                                    status: OrderStatus.Final): Unit = {
    log.trace(s"Order ${lo.order.id()} terminated: $status")
    orderDB.saveOrder(lo.order)
    pendingCancellation
      .remove(lo.order.id())
      .foreach(_.success(
        api.OrderCancelRejected(MatcherError.OrderFinalized(lo.order.id()))))
    expiration.remove(lo.order.id()).foreach(_.cancel())
    activeOrders.remove(lo.order.id())
    orderDB.saveOrderInfo(
      lo.order.id(),
      owner,
      OrderInfo(lo.order.orderType,
                lo.order.amount,
                lo.order.price,
                lo.order.timestamp,
                status,
  ...
  def receive: Receive =
    handleCommands orElse handleExecutionEvents orElse handleStatusRequests
  ...
  /**
    ...
    */
  private def ordersToDelete(
      initBalance: SpendableBalance): Queue[QueueEvent.Canceled] = {
    def keepChanged(requiredBalance: Map[Asset, Long]) =
      requiredBalance.filter {
  ...
  private def toSpendable(event: BalanceUpdated): SpendableBalance = {
    val r: SpendableBalance = event.changedAssets.map(x =>
      x -> spendableBalance(x))(collection.breakOut)
    r.withDefaultValue(0)
  }

  private def remove(from: SpendableBalance,
                     xs: SpendableBalance): Option[SpendableBalance] =
...
object AddressActor {
  ...
  private def activeStatus(lo: LimitOrder): OrderStatus =
    if (lo.amount == lo.order.amount) OrderStatus.Accepted
    else OrderStatus.PartiallyFilled(lo.order.amount - lo.amount)

  sealed trait Command

  case class GetOrderStatus(orderId: ByteStr) extends Command
  case class GetOrders(assetPair: Option[AssetPair], onlyActive: Boolean)
      extends Command
  ...
  case class PlaceOrder(order: Order) extends Command {
    override lazy val toString = s"PlaceOrder(${order
  ...
  case class CancelAllOrders(pair: Option[AssetPair], timestamp: Long)
      extends Command
