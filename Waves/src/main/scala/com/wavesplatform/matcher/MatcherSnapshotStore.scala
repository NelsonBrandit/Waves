...
class MatcherSnapshotStore(config: Config) extends SnapshotStore {
      ...
      readOnly { db =>
        db.get(kSMHistory(persistenceId))
          ...
          .map { seqNr =>
            ...
            SelectedSnapshot(
              SnapshotMetadata(persistenceId, sm.seqNr, sm.ts),
              deserialize(db.get(kSnapshot(persistenceId, seqNr))).data)
          }
      }
    )
  ...
  override def deleteAsync(metadata: SnapshotMetadata): Future[Unit] =
    nextAction(
      ...
      readWrite { db =>
        ...
        history
          .find(seqNr =>
            db.get(kSM(metadata.persistenceId, seqNr)).matches(metadata))
          .foreach { seqNr =>
  ...
  override def deleteAsync(persistenceId: String,
    ...
    nextAction(
      ...
      readWrite { db =>
        ...
        val (toDelete, toKeep) = history
          .map(seqNr => seqNr -> db.get(kSM(persistenceId, seqNr)))
          .partition { case (_, sm) => sm.matches(criteria) }
  ...
  protected def save(metadata: SnapshotMetadata, snapshot: Any): Unit =
    saveTimer.refine("id" -> metadata.persistenceId).measure {
      readWrite { rw =>
        val historyKey = kSMHistory(metadata.persistenceId)
        ...
        rw.put(kSM(metadata.persistenceId, nextId),
               SM(metadata.sequenceNr, metadata.timestamp))

        val serialized = serializeTimer
          .refine("id" -> metadata.persistenceId)
  ...
  private def serialize(s: Any) =
    serializationExtension.serialize(Snapshot(s)).get
}
...
object MatcherSnapshotStore extends ScorexLogging {
  ...
  def kSMHistory(persistenceId: String): Key[Seq[Int]] =
    Key[Seq[Int]]("matcher-sm-history",
                  Bytes.concat(Array(1: Byte), persistenceId.getBytes(UTF_8)),
                  readIntSeq,
                  writeIntSeq)
  def kSM(persistenceId: String, seqNr: Int): Key[SM] =
    Key[SM]("matcher-sm",
            Bytes.concat(Array(2: Byte),
                         persistenceId.getBytes(UTF_8),
                         Ints.toByteArray(seqNr)),
            readSnapshotMetadata,
            writeSnapshotMetadata)
  def kSnapshot(persistenceId: String, seqNr: Int): Key[Array[Byte]] =
    Key[Array[Byte]]("matcher-snapshot",
                     Bytes.concat(Array(3: Byte),
                                  persistenceId.getBytes(UTF_8),
                                  Ints.toByteArray(seqNr)),
                     identity,
                     identity)
