...
package object utils extends ScorexLogging {
  ...
  private val lazyFunctionCosts
    : Map[StdLibVersion, Coeval[Map[FunctionHeader, Coeval[Long]]]] =
    lazyContexts.map(
      el =>
        (el._1.stdLibVersion,
         el._2.map(ctx =>
           estimate(el._1.stdLibVersion, ctx.evaluationContext))))

  def functionCosts(version: StdLibVersion): Map[FunctionHeader, Coeval[Long]] =
    lazyFunctionCosts(version)()

  def estimate(version: StdLibVersion,
               ctx: EvaluationContext): Map[FunctionHeader, Coeval[Long]] = {
    val costs: mutable.Map[FunctionHeader, Coeval[Long]] =
      ctx.typeDefs.collect {
        case (typeName, CaseType(_, fields)) =>
  ...
  def compilerContext(version: StdLibVersion,
                      cType: ContentType,
                      isAssetScript: Boolean): CompilerContext = {
    val ds =
      DirectiveSet(version, ScriptType.isAssetScript(isAssetScript), cType)
    lazyContexts(ds)().compilerContext
  }

  val defaultDecompilerContext: DecompilerContext = lazyContexts(DirectiveSet(
    V3,
    ScriptType.Account,
    ContentType.Contract))().decompilerContext
  ...
  @tailrec def doWhile[T](z: T)(cond: T => Boolean)(f: T => T): T =
    if (cond(z)) doWhile(f(z))(cond)(f) else z
  ...
  implicit val byteStrWrites: Format[ByteStr] = new Format[ByteStr] {
    ...
    override def reads(json: JsValue): JsResult[ByteStr] = json match {
      case JsString(v) =>
        decodeBase58(v).fold(
          e => JsError(s"Error parsing base58: ${e.getMessage}"),
          b => JsSuccess(b))
