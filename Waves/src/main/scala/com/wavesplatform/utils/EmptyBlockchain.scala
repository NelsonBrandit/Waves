...
object EmptyBlockchain extends Blockchain {
  ...
  override def blockHeaderAndSize(
      blockId: ByteStr): Option[(BlockHeader, Int)] = None
  ...
  override def blockIdsAfter(parentSignature: ByteStr,
                             howMany: Int): Option[Seq[ByteStr]] = None
  ...
  override def resolveAlias(a: Alias): Either[ValidationError, Address] =
    Left(GenericError("Empty blockchain"))
  ...
  override def balanceSnapshots(address: Address,
                                from: Int,
                                to: ByteStr): Seq[BalanceSnapshot] = Seq.empty
  ...
  override def assetDistribution(assetId: IssuedAsset): AssetDistribution =
    Monoid.empty[AssetDistribution]
  ...
  override def assetDistributionAtHeight(assetId: IssuedAsset,
    ...
    Right(
      AssetDistributionPage(
        Paged[Address, AssetDistribution](false,
                                          None,
                                          Monoid.empty[AssetDistribution])))
  ...
  override def collectLposPortfolios[A](
      pf: PartialFunction[(Address, Portfolio), A]): Map[Address, A] = Map.empty
