...
object JsonFileStorage {
  ...
  def prepareKey(key: String): SecretKeySpec =
    new SecretKeySpec(hashPassword(key.toCharArray,
                                   keySalt.getBytes(encoding),
                                   hashingIterations,
                                   keyLength),
                      aes)
  ...
  def save[T](value: T, path: String, key: Option[SecretKeySpec])(
  ...
  def load[T](path: String, key: Option[SecretKeySpec] = None)(
      implicit r: Reads[T]): T = {
