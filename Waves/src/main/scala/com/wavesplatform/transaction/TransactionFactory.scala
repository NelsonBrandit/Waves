...
import com.wavesplatform.api.http.leasing.{
...
object TransactionFactory {
  ...
  def transferAssetV1(request: TransferV1Request, sender: PublicKeyAccount)
  ...
  def transferAssetV2(request: TransferV2Request, sender: PublicKeyAccount)
    : Either[ValidationError, TransferTransactionV2] =
    for {
      recipientAcc <- AddressOrAlias.fromString(request.recipient)
  ...
  def massTransferAsset(request: MassTransferRequest, sender: PublicKeyAccount)
  ...
  def setScript(
      ...
      sender: PublicKeyAccount): Either[ValidationError, SetScriptTransaction] =
    for {
      script <- request.script match {
        case None | Some("") => Right(None)
        case Some(s)         => Script.fromBase64String(s).map(Some(_))
  ...
  def setAssetScript(request: SetAssetScriptRequest, sender: PublicKeyAccount)
  ...
  def issueAssetV2(
      ...
      sender: PublicKeyAccount): Either[ValidationError, IssueTransactionV2] =
    for {
      s <- request.script match {
        case None    => Right(None)
        case Some(s) => Script.fromBase64String(s).map(Some(_))
  ...
  def issueAssetV1(
      request: IssueV1Request,
      sender: PublicKeyAccount): Either[ValidationError, IssueTransactionV1] =
  ...
  def leaseV1(
      request: LeaseV1Request,
      sender: PublicKeyAccount): Either[ValidationError, LeaseTransactionV1] =
    for {
      recipientAcc <- AddressOrAlias.fromString(request.recipient)
  ...
  def leaseV2(
      request: LeaseV2Request,
      sender: PublicKeyAccount): Either[ValidationError, LeaseTransactionV2] =
    for {
      recipientAcc <- AddressOrAlias.fromString(request.recipient)
  ...
  def leaseCancelV1(request: LeaseCancelV1Request, sender: PublicKeyAccount)
  ...
  def leaseCancelV2(request: LeaseCancelV2Request, sender: PublicKeyAccount)
  ...
  def aliasV1(request: CreateAliasV1Request, sender: PublicKeyAccount)
    : Either[ValidationError, CreateAliasTransactionV1] =
    for {
      alias <- Alias.create(request.alias)
  ...
  def aliasV2(request: CreateAliasV2Request, sender: PublicKeyAccount)
    : Either[ValidationError, CreateAliasTransactionV2] =
    for {
      alias <- Alias.create(request.alias)
  ...
  def reissueAssetV1(
      request: ReissueV1Request,
      sender: PublicKeyAccount): Either[ValidationError, ReissueTransactionV1] =
  ...
  def reissueAssetV2(
      request: ReissueV2Request,
      sender: PublicKeyAccount): Either[ValidationError, ReissueTransactionV2] =
  ...
  def burnAssetV1(
      request: BurnV1Request,
      sender: PublicKeyAccount): Either[ValidationError, BurnTransactionV1] =
  ...
  def burnAssetV2(
      request: BurnV2Request,
      sender: PublicKeyAccount): Either[ValidationError, BurnTransactionV2] =
    BurnTransactionV2.create(
      AddressScheme.current.chainId,
      sender,
      IssuedAsset(ByteStr.decodeBase58(request.assetId).get),
      request.quantity,
  ...
  def data(request: DataRequest,
           sender: PublicKeyAccount): Either[ValidationError, DataTransaction] =
  ...
  def invokeScript(request: InvokeScriptRequest, sender: PublicKeyAccount)
  ...
  def sponsor(request: SponsorFeeRequest, sender: PublicKeyAccount)
  ...
  def exchangeV1(request: SignedExchangeRequest, sender: PublicKeyAccount)
    ...
    for {
      signature <- ByteStr
        .decodeBase58(request.signature)
        .toEither
        .left
        .map(_ => GenericError(s"Wrong Base58 string: ${request.signature}"))
  ...
  def fromSignedRequest(jsv: JsValue): Either[ValidationError, Transaction] = {
    ...
    val version =
      (jsv \ "version").asOpt[Byte](versionReads).getOrElse(1.toByte)
      ...
      case Some(x) =>
        x match {
          ...
          case LeaseCancelTransactionV1 =>
            jsv.as[SignedLeaseCancelV1Request].toTx
          case LeaseCancelTransactionV2 =>
            jsv.as[SignedLeaseCancelV2Request].toTx
          case CreateAliasTransactionV1 =>
            jsv.as[SignedCreateAliasV1Request].toTx
          case CreateAliasTransactionV2 =>
            jsv.as[SignedCreateAliasV2Request].toTx
          ...
          case SetAssetScriptTransaction =>
            jsv.as[SignedSetAssetScriptRequest].toTx
