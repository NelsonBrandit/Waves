...
final case class CreateAliasTransactionV2 private (sender: PublicKeyAccount,
                                                   alias: Alias,
                                                   fee: Long,
                                                   timestamp: Long,
                                                   proofs: Proofs)
  ...
  override val bytes: Coeval[Array[Byte]] = (bodyBytes, proofs.bytes).mapN {
    case (bb, pb) => Bytes.concat(Array(0: Byte), bb, pb)
  }
...
object CreateAliasTransactionV2
    extends TransactionParserFor[CreateAliasTransactionV2]
  ...
  override protected def parseTail(
      bytes: Array[Byte]): Try[CreateAliasTransactionV2] = {
