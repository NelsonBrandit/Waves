...
case class MassTransferTransaction private (assetId: Asset,
  ...
  override val bodyBytes: Coeval[Array[Byte]] = Coeval.evalOnce {
    ...
    val transferBytes = transfers
      .map {
        case ParsedTransfer(recipient, amount) =>
  ...
  def compactJson(recipients: Set[AddressOrAlias]): JsObject =
    jsonBase() ++ Json.obj("transfers" -> toJson(transfers.filter(t =>
      recipients.contains(t.address))))
...
object MassTransferTransaction
    extends TransactionParserFor[MassTransferTransaction]
    with TransactionParser.OneVersion {
  ...
  @ApiModel
  case class Transfer(@(ApiModelProperty @field)(
                        dataType = "string",
                        example = "3Mciuup51AxRrpSz7XhutnQYTkNT9691HAk",
                        required = true) recipient: String,
                      @(ApiModelProperty @field)(dataType = "long",
                                                 example = "3000000000",
                                                 required = true) amount: Long)
          ...
          totalAmount =>
            if (tx.transfers.lengthCompare(MaxTransferCount) > 0) {
              Left(ValidationError.GenericError(
                s"Number of transfers ${tx.transfers.length} is greater than $MaxTransferCount"))
    ...
    Try {
      ...
      totalAmount =>
        if (transfers.lengthCompare(MaxTransferCount) > 0) {
          Left(ValidationError.GenericError(
            s"Number of transfers ${transfers.length} is greater than $MaxTransferCount"))
  ...
  def parseTransfersList(
      transfers: List[Transfer]): Validation[List[ParsedTransfer]] = {
    transfers.traverse {
  ...
  private def toJson(transfers: List[ParsedTransfer]): JsValue = {
    Json.toJson(transfers.map {
      case ParsedTransfer(address, amount) =>
  ...
  val byteTailDescription: ByteEntity[MassTransferTransaction] = {
    (
      ...
      OptionBytes(index = tailIndex(2),
                  name = "Asset ID",
                  nestedByteEntity = AssetIdBytes(tailIndex(2), "Asset ID")),
