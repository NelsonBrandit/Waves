...
object TransferTransactionV2
  ...
  def create(assetId: Asset,
    ...
    for {
      _ <- TransferTransaction.validate(amount,
                                        assetId,
                                        feeAmount,
                                        feeAssetId,
                                        attachment)
    } yield
      TransferTransactionV2(sender,
                            recipient,
                            assetId,
                            amount,
                            timestamp,
  ...
  val byteTailDescription: ByteEntity[TransferTransactionV2] = {
    (
      ...
      OptionBytes(tailIndex(2),
                  "Asset ID",
                  AssetIdBytes(tailIndex(2), "Asset ID"),
                  "flag (1 - asset, 0 - Waves)"),
      OptionBytes(tailIndex(3),
                  "Fee's asset ID",
                  AssetIdBytes(tailIndex(3), "Fee's asset ID"),
                  "flag (1 - asset, 0 - Waves)"),
