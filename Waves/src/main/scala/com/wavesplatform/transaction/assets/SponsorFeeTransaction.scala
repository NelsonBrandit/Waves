...
object SponsorFeeTransaction
    ...
    byteTailDescription.deserializeFromByteArray(bytes).flatMap { tx =>
      (
        if (tx.minSponsoredAssetFee.exists(_ < 0)) {
          Left(
            ValidationError.NegativeMinFee(tx.minSponsoredAssetFee.get,
                                           "asset"))
        } else if (tx.fee <= 0) {
  ...
  def create(sender: PublicKeyAccount,
    ...
    if (minSponsoredAssetFee.exists(_ < 0)) {
      ...
      Right(
        SponsorFeeTransaction(sender,
                              asset,
                              minSponsoredAssetFee.filter(_ != 0),
                              fee,
                              timestamp,
                              proofs))
    }
  ...
  def signed(
    ...
    create(sender, asset, minSponsoredAssetFee, fee, timestamp, Proofs.empty).right
      .map { unsigned =>
        unsigned.copy(
          proofs = Proofs
