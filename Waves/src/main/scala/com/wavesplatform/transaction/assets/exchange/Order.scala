...
trait Order extends BytesSerializable with JsonSerializable with Proven {
  ...
  @ApiModelProperty(hidden = true)
  def getSpendAmount(matchAmount: Long,
                     matchPrice: Long): Either[ValidationError, Long] =
    Try {
  ...
  @ApiModelProperty(hidden = true)
  def getReceiveAmount(matchAmount: Long,
                       matchPrice: Long): Either[ValidationError, Long] =
    Try {
  ...
  override def toString: String = {
    val matcherFeeAssetIdStr =
      if (version == 3)
        s" matcherFeeAssetId=${matcherFeeAssetId.fold("Waves")(_.toString)},"
      else ""
...
object Order {
  ...
  def correctAmount(a: Long, price: Long): Long = {
    val settledTotal = (BigDecimal(price) * a / Order.PriceConstant)
      .setScale(0, RoundingMode.FLOOR)
      .toLong
    (BigDecimal(settledTotal) / price * Order.PriceConstant)
  ...
  def fromBytes(version: Byte, xs: Array[Byte]): Order = version match {
    ...
    case other =>
      throw new IllegalArgumentException(s"Unexpected order version: $other")
  }
