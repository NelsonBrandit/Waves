...
case class OrderV1(
  ...
  @ApiModelProperty(hidden = true)
  val signatureValid = Coeval.evalOnce(
    crypto.verify(signature, bodyBytes(), senderPublicKey.publicKey))

  @ApiModelProperty(hidden = true)
  override val bytes: Coeval[Array[Byte]] =
    Coeval.evalOnce(bodyBytes() ++ signature)
}
...
object OrderV1 {
  ...
  def parseBytes(bytes: Array[Byte]): Try[OrderV1] = Try {
    ...
    def parse[T](f: (Array[Byte], Int, Int) => (T, Int),
                 size: Int): State[Int, T] = State { from =>
