...
object OrderJson {
  ...
  implicit val optionByteArrayReads: Reads[Option[Array[Byte]]] = {
    ...
    case JsString(s) if s.nonEmpty =>
      Base58.decode(s) match {
        ...
        case Failure(_) =>
          JsError(
            Seq(JsPath() -> Seq(JsonValidationError("error.incorrect.base58"))))
      }
    case _ =>
      JsError(
  ...
  def readOrderV1V2(sender: PublicKeyAccount,
    ...
    val vrsn: Byte = version.getOrElse(
      if (eproofs.proofs.size == 1 && eproofs.proofs.head.arr.length == SignatureLength)
        1
      else 2)
    Order(
      sender,
