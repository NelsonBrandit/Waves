...
object ReissueTransactionV1
    extends TransactionParserFor[ReissueTransactionV1]
  ...
  def signed(
    ...
    create(sender, asset, quantity, reissuable, fee, timestamp, ByteStr.empty).right
      .map { unsigned =>
        unsigned.copy(
