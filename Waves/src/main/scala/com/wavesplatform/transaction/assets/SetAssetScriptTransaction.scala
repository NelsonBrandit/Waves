...
object SetAssetScriptTransaction
    extends TransactionParserFor[SetAssetScriptTransaction]
  ...
  def create(chainId: Byte,
    ...
    for {
      _ <- Either.cond(
        script.fold(true)(_.isInstanceOf[ExprScript]),
        (),
        ValidationError.GenericError(
          s"Asset can only be assigned with Expression script, not Contract"))
      _ <- Either.cond(
        chainId == currentChainId,
        (),
        ValidationError.GenericError(
          s"Wrong chainId actual: ${chainId.toInt}, expected: $currentChainId"))
    } yield
  ...
  def signed(
    ...
    create(chainId, sender, asset, script, fee, timestamp, Proofs.empty).right
      .map { unsigned =>
        unsigned.copy(
          proofs = Proofs
            .create(Seq(ByteStr(sign(signer, unsigned.bodyBytes()))))
            .explicitGet())
      }
  }
    ...
    byteTailDescription.deserializeFromByteArray(bytes).flatMap { tx =>
      Either
        .cond(
          ...
          (),
          ValidationError.GenericError(
            s"Wrong chainId actual: ${tx.chainId.toInt}, expected: $currentChainId"))
        .map(_ => tx)
        .foldToTry
    }
  }
  ...
  val byteTailDescription: ByteEntity[SetAssetScriptTransaction] = {
    (
      ...
      OptionBytes(index = tailIndex(6),
                  name = "Script",
                  nestedByteEntity = ScriptBytes(tailIndex(6), "Script")),
