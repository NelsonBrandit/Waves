...
object BurnTransactionV1
  ...
  def create(sender: PublicKeyAccount,
    ...
    BurnTransaction
      ...
      .map(_ =>
        BurnTransactionV1(sender, asset, quantity, fee, timestamp, signature))
  }
  ...
  def signed(
    ...
    create(sender, asset, quantity, fee, timestamp, ByteStr.empty).right.map {
      unverified =>
        unverified.copy(
          signature = ByteStr(crypto.sign(signer, unverified.bodyBytes())))
    }
  }
