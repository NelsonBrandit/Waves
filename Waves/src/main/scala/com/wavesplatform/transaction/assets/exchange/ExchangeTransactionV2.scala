...
case class ExchangeTransactionV2(buyOrder: Order,
  ...
  override val bodyBytes: Coeval[Array[Byte]] =
    Coeval.evalOnce(
      Array(0: Byte, builder.typeId, version) ++
        Ints.toByteArray(buyOrder.bytes().length) ++ orderMark(buyOrder.version) ++ buyOrder
        .bytes() ++
        Ints.toByteArray(sellOrder.bytes().length) ++ orderMark(
        sellOrder.version) ++ sellOrder.bytes() ++
  ...
  override val bytes: Coeval[Array[Byte]] =
    Coeval.evalOnce(bodyBytes() ++ proofs.bytes())
}

object ExchangeTransactionV2
    extends TransactionParserFor[ExchangeTransactionV2]
  ...
  def create(matcher: PrivateKeyAccount,
    ...
    create(buyOrder,
           ...
           fee,
           timestamp,
           Proofs.empty).map { unverified =>
      unverified.copy(proofs = Proofs(
        List(ByteStr(crypto.sign(matcher.privateKey, unverified.bodyBytes())))))
    }
  }
  ...
  def create(buyOrder: Order,
    ...
    validateExchangeParams(
      ...
      ExchangeTransactionV2(buyOrder,
                            sellOrder,
                            amount,
                            price,
                            buyMatcherFee,
