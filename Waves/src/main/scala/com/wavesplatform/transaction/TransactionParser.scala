...
trait TransactionParser {
  ...
  /**
    ...
    * }}}
    */
  lazy val byteDescription: ByteEntity[TransactionT] = (byteHeaderDescription,
                                                        byteTailDescription) mapN {
    case (_, tx) => tx
  }
}
...
object TransactionParser {
    ...
    override protected def parseHeader(bytes: Array[Byte]): Try[Int] = Try {
      ...
      if (bytes.length < 1)
        throw new IllegalArgumentException(
          s"The buffer is too small, it has ${bytes.length} elements")

      val parsedTypeId = bytes.head

      if (parsedTypeId != typeId)
        throw new IllegalArgumentException(
          s"Expected type of transaction '$typeId', but got '$parsedTypeId'")
  ...
  trait OneVersion extends TransactionParser {
    ...
    override protected def parseHeader(bytes: Array[Byte]): Try[Int] = Try {
      ...
      if (!supportedVersions.contains(parsedVersion))
        throw new IllegalArgumentException(
          s"Expected version of transaction: $version, but got '$parsedVersion'")
  ...
  trait MultipleVersions extends TransactionParser {
    ...
    override protected def parseHeader(bytes: Array[Byte]): Try[Int] = Try {
      ...
      if (parsedMark != 0)
        throw new IllegalArgumentException(
          s"Expected the '0' byte, but got '$parsedMark'")
      if (parsedTypeId != typeId)
        throw new IllegalArgumentException(
          s"Expected type of transaction '$typeId', but got '$parsedTypeId'")
      if (!supportedVersions.contains(parsedVersion))
        throw new IllegalArgumentException(
          s"Expected version of transaction ${supportedVersions.mkString(", ")}, but got '$parsedVersion'")
...
abstract class TransactionParserFor[T <: Transaction](
    implicit override val classTag: ClassTag[T])
    extends TransactionParser {
