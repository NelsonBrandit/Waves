...
case class GenesisTransaction private (recipient: Address,
  ...
  override val bytes: Coeval[Array[Byte]] = Coeval.evalOnce {
    ...
    val timestampBytes =
      Bytes.ensureCapacity(Longs.toByteArray(timestamp), TimestampLength, 0)
    val amountBytes =
      Bytes.ensureCapacity(Longs.toByteArray(amount), AmountLength, 0)
...
object GenesisTransaction
    extends TransactionParserFor[GenesisTransaction]
  ...
  def generateSignature(recipient: Address,
                        amount: Long,
                        timestamp: Long): Array[Byte] = {

    val typeBytes =
      Bytes.ensureCapacity(Ints.toByteArray(typeId), TypeLength, 0)
    val timestampBytes =
      Bytes.ensureCapacity(Longs.toByteArray(timestamp), TimestampLength, 0)
    ...
    Try {
      ...
      byteTailDescription.deserializeFromByteArray(bytes).flatMap { tx =>
        Either
          .cond(tx.amount >= 0,
                tx,
                ValidationError.NegativeAmount(tx.amount, "waves"))
          .foldToTry
      }
    }.flatten
  }
  ...
  def create(recipient: Address,
             amount: Long,
             timestamp: Long): Either[ValidationError, GenesisTransaction] = {
    if (amount < 0) {
