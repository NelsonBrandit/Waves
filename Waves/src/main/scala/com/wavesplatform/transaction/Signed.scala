...
trait Signed extends Authorized {
  ...
  protected val signedDescendants: Coeval[Seq[Signed]] =
    Coeval.evalOnce(Seq.empty)

  @ApiModelProperty(hidden = true)
  protected val signaturesValidMemoized
    : Task[Either[InvalidSignature, this.type]] =
    Signed.validateTask[this.type](this).memoize

  @ApiModelProperty(hidden = true)
  val signaturesValid: Coeval[Either[InvalidSignature, this.type]] =
    Coeval.evalOnce(
      Await.result(signaturesValidMemoized.runAsync(Signed.scheduler),
                   Duration.Inf))
}
  ...
  private def validateTask[S <: Signed](s: S): Task[E[S]] =
    Task {
      if (!s.signatureValid()) Task.now(Left(InvalidSignature(s, None)))
        ...
        Task.wanderUnordered(s.signedDescendants())(s =>
          s.signaturesValidMemoized) map { l =>
