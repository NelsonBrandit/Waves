...
case class SetScriptTransaction private (chainId: Byte,
  ...
  override val json = Coeval.evalOnce(
    jsonBase() ++ Json.obj("chainId" -> chainId,
                           "version" -> version,
...
object SetScriptTransaction
    extends TransactionParserFor[SetScriptTransaction]
  ...
  override protected def parseTail(bytes: Array[Byte]): Try[TransactionT] = {
    byteTailDescription.deserializeFromByteArray(bytes).flatMap { tx =>
      Either
        .cond(tx.chainId == chainId,
              (),
              GenericError(s"Wrong chainId ${tx.chainId.toInt}"))
        .flatMap(
          _ =>
            Either.cond(
              tx.fee > 0,
              (),
              ValidationError.InsufficientFee(s"insufficient fee: ${tx.fee}")))
        .map(_ => tx)
        .foldToTry
    }
  }
  ...
  def create(sender: PublicKeyAccount,
    ...
    for {
      _ <- Either.cond(
        fee > 0,
        (),
        ValidationError.InsufficientFee(s"insufficient fee: $fee"))
    } yield
      new SetScriptTransaction(chainId, sender, script, fee, timestamp, proofs)
  }
  ...
  val byteTailDescription: ByteEntity[SetScriptTransaction] = {
    (
      ...
      OptionBytes(index = tailIndex(3),
                  name = "Script",
                  nestedByteEntity = ScriptBytes(tailIndex(3), "Script")),
