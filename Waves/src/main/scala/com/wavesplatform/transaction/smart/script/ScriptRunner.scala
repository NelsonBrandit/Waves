    ...
    script match {
      ...
      case ContractScript.ContractScriptImpl(_, Contract(_, _, Some(vf)), _) =>
        ...
        val evalContract = in.eliminate(
          t => ContractEvaluator.verify(vf, RealTransactionWrapper.apply(t)),
          _.eliminate(
            t => ContractEvaluator.verify(vf, RealTransactionWrapper.ord(t)),
            _ => ???))
      ...
      case ContractScript.ContractScriptImpl(_, Contract(_, _, None), _) =>
        val t: Proven with Authorized =
          in.eliminate(
            ...
            _.eliminate(_.asInstanceOf[Proven with Authorized], _ => ???))
        (List.empty,
         Verifier.verifyAsEllipticCurveSignature[Proven with Authorized](t) match {
