...
class WavesEnvironment(
  ...
  override def data(recipient: Recipient,
                    key: String,
    ...
    for {
      ...
      data <- blockchain
        ...
        .flatMap {
          ...
          case (BinaryDataEntry(_, value), DataType.ByteArray) =>
            Some(ByteStr(value.arr))
  ...
  override def accountBalanceOf(
      ...
      maybeAssetId: Option[Array[Byte]]): Either[String, Long] = {
    (for {
      aoa <- addressOrAlias match {
        case Address(bytes) =>
          AddressOrAlias.fromBytes(bytes.arr, position = 0).map(_._1)
        case Alias(name) => com.wavesplatform.account.Alias.create(name)
      }
      address <- blockchain.resolveAlias(aoa)
      balance = blockchain.balance(
        address,
        Asset.fromCompatId(maybeAssetId.map(ByteStr(_))))
