  ...
  def fromBytes(bytes: Array[Byte]): Either[ScriptParseError, Script] = {
    ...
    (for {
      a <- {
        if (versionByte == 0)
          Right(
            (ContentType.parseId(bytes(1)),
             StdLibVersion.parseVersion(bytes(2)),
             3))
          ...
          Left(
            ScriptParseError(s"Can't parse script bytes starting with [${bytes(
              0).toInt},${bytes(1).toInt},${bytes(2).toInt}]"))
      }
