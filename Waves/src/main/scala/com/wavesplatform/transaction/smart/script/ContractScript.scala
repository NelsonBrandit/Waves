...
object ContractScript {
  ...
  def apply(version: StdLibVersion,
            contract: Contract): Either[String, Script] = {
    for {
  ...
  case class ContractScriptImpl(stdLibVersion: StdLibVersion,
                                expr: Contract,
                                maxComplexity: Long)
      extends Script {
    ...
    override val bytes: Coeval[ByteStr] =
      Coeval.evalOnce(ByteStr(Global.serializeContract(expr, stdLibVersion)))
    override val containsBlockV2: Coeval[Boolean] = Coeval.evalOnce(true)
  }

  def estimateComplexity(version: StdLibVersion,
                         contract: Contract): Either[String, (String, Long)] = {
    import cats.implicits._
    type E[A] = Either[String, A]
    val funcsWithComplexity: Seq[E[(String, Long)]] =
      (contract.cfs.map(func => (func.annotation.invocationArgName, func.u)) ++ contract.vf
        .map(func => (func.annotation.invocationArgName, func.u)))
        .map {
          case (annotationArgName, funcExpr) =>
            ScriptEstimator(varNames(version, ContentType.Contract),
                            functionCosts(version),
                            constructExprFromFuncAndContext(contract.dec,
                                                            annotationArgName,
                                                            funcExpr))
              .map(complexity => (funcExpr.name, complexity))
        }
    val funcsWithComplexityEi: E[Vector[(String, Long)]] =
      funcsWithComplexity.toVector.sequence

    funcsWithComplexityEi.map(namesAndComp =>
      (("", 0L) +: namesAndComp).maxBy(_._2))
  }

  private def constructExprFromFuncAndContext(dec: List[DECLARATION],
    ...
    val funcWithAnnotationContext =
      BLOCK(
        ...
        BLOCK(
          funcExpr,
          FUNCTION_CALL(FunctionHeader.User(funcExpr.name),
                        List.fill(funcExpr.args.size)(TRUE))
        )
      )
