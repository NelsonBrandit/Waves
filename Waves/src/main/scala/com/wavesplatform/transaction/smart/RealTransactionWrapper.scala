...
object RealTransactionWrapper {
  ...
  implicit def assetPair(a: AssetPair): APair =
    APair(a.amountAsset.compatId, a.priceAsset.compatId)
  ...
  def apply(tx: Transaction): Tx = {
    tx match {
      ...
      case r: ReissueTransaction =>
        Tx.ReIssue(proven(r), r.quantity, r.asset.id, r.reissuable)
      ...
      case ms: MassTransferTransaction =>
        Tx.MassTransfer(
          ...
          transfers = ms.transfers
            .map(
              r =>
                com.wavesplatform.lang.v1.traits.domain.Tx
                  .TransferItem(r.address, r.amount))
            .toIndexedSeq,
          attachment = ByteStr(ms.attachment)
        )
      case ss: SetScriptTransaction =>
        Tx.SetScript(proven(ss), ss.script.map(_.bytes()))
      case ss: SetAssetScriptTransaction =>
        Tx.SetAssetScript(proven(ss), ss.asset.id, ss.script.map(_.bytes()))
      ...
      case s: SponsorFeeTransaction =>
        Tx.Sponsorship(proven(s), s.asset.id, s.minSponsoredAssetFee)
