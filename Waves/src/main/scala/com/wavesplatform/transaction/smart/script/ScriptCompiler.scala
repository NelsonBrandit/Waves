...
object ScriptCompiler extends ScorexLogging {
  ...
  @Deprecated
  def apply(scriptText: String,
            isAssetScript: Boolean): Either[String, (Script, Long)] = {
    val directives = DirectiveParser(scriptText)
    for {
      ver <- extractStdLibVersion(directives)
      tpe <- extractContentType(directives)
      _ <- directiveConsistency(
        DirectiveSet(ver,
                     if (isAssetScript) ScriptType.Asset
                     else ScriptType.Account,
                     tpe))
  ...
  private def tryCompile(src: String,
                         cType: ContentType,
                         version: StdLibVersion,
                         isAssetScript: Boolean): Either[String, Script] = {
    ...
    try {
      cType match {
        case ContentType.Expression =>
          ExpressionCompiler
            .compile(src, ctx)
            .flatMap(expr => ExprScript.apply(version, expr))
        case ContentType.Contract =>
          ContractCompiler
            .compile(src, ctx)
            .flatMap(expr => ContractScript.apply(version, expr))
      }
    } catch {
      case ex: Throwable =>
  ...
  def estimate(script: Script, version: StdLibVersion): Either[String, Long] =
    script match {
      case s: ExprScript =>
        ScriptEstimator(varNames(version, ContentType.Expression),
                        functionCosts(version),
                        s.expr)
      case s: ContractScriptImpl =>
        ContractScript.estimateComplexity(version, s.expr).map(_._2)
      case _ => ???
    }
