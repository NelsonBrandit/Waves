...
case class LeaseCancelTransactionV2 private (chainId: Byte,
  ...
  val bodyBytes: Coeval[Array[Byte]] =
    Coeval.evalOnce(
      Bytes.concat(Array(builder.typeId, version, chainId), bytesBase()))
...
object LeaseCancelTransactionV2
    extends TransactionParserFor[LeaseCancelTransactionV2]
  ...
  def create(chainId: Byte,
    ...
    for {
      ...
      LeaseCancelTransactionV2(chainId, sender, leaseId, fee, timestamp, proofs)
  }
  ...
  def signed(
    ...
    create(chainId, sender, leaseId, fee, timestamp, Proofs.empty).right.map {
      unsigned =>
        unsigned.copy(
          proofs = Proofs
