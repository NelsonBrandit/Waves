...
case class LeaseTransactionV2 private (sender: PublicKeyAccount,
                                       amount: Long,
                                       fee: Long,
                                       timestamp: Long,
...
object LeaseTransactionV2
    ...
    byteTailDescription.deserializeFromByteArray(bytes).flatMap { tx =>
      ...
      Either
        .cond(
          assetIdOpt.isEmpty,
          (),
          ValidationError.GenericError("Leasing assets is not supported yet"))
  ...
  def create(sender: PublicKeyAccount,
    ...
    for {
      ...
      LeaseTransactionV2(sender, amount, fee, timestamp, recipient, proofs)
  }
  ...
  val byteTailDescription: ByteEntity[LeaseTransactionV2] = {
    (
      OptionBytes(tailIndex(1),
                  "Leasing asset",
                  AssetIdBytes(tailIndex(1), "Leasing asset"),
                  "flag (1 - asset, 0 - Waves)"),
