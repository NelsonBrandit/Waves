...
object DataTransaction
    extends TransactionParserFor[DataTransaction]
  ...
  private def validateTxContent(tx: DataTransaction,
                                txByteCountOpt: Option[Int] = None)
    ...
    if (tx.data.lengthCompare(MaxEntryCount) > 0 || tx.data.exists(!_.valid)) {
      ...
      Either.cond(txByteCountOpt.getOrElse(tx.bytes().length) <= MaxBytes,
                  tx,
  ...
  def signed(
    ...
    create(sender, data, feeAmount, timestamp, Proofs.empty).right.map {
      unsigned =>
        unsigned.copy(
          proofs = Proofs
