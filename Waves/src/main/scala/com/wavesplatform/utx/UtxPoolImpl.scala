...
class UtxPoolImpl(time: Time,
  ...
  private[this] object PoolMetrics {
    private[this] val sizeStats = Kamon.rangeSampler(
      "utx-pool-size",
      MeasurementUnit.none,
      Duration.of(500, ChronoUnit.MILLIS))
    private[this] val bytesStats = Kamon.rangeSampler(
      "utx-pool-bytes",
      MeasurementUnit.information.bytes,
      Duration.of(500, ChronoUnit.MILLIS))
    val processingTimeStats = Kamon.histogram("utx-transaction-processing-time",
  ...
  override def putIfNew(
      tx: Transaction): Either[ValidationError, (Boolean, Diff)] = {
    def canReissue(blockchain: Blockchain, tx: Transaction) = tx match {
      case r: ReissueTransaction
          if blockchain.assetDescription(r.asset).exists(!_.reissuable) =>
    ...
    def checkAlias(blockchain: Blockchain, tx: Transaction) = tx match {
      case cat: CreateAliasTransaction
          if !blockchain.canCreateAlias(cat.alias) =>
    ...
    def checkScripted(blockchain: Blockchain, tx: Transaction) = tx match {
      ...
      case a: AuthorizedTransaction
          if blockchain.hasScript(a.sender.toAddress) =>
        Left(
          GenericError(
    ...
    def checkNotBlacklisted(tx: Transaction) = {
      if (utxSettings.blacklistSenderAddresses.isEmpty) {
        ...
        sender match {
          case Some(addr)
              if utxSettings.blacklistSenderAddresses.contains(addr) =>
            ...
            val allowed =
              recipients.nonEmpty &&
                recipients.forall(r =>
                  utxSettings.allowBlacklistedTransferTo.contains(r.stringRepr))
    ...
    val result = measureSuccessful(
      PoolMetrics.processingTimeStats, {
        for {
          _ <- Either.cond(
            transactions.size < utxSettings.maxSize,
            (),
          ...
          transactionsBytes = transactions.values.asScala // Bytes size of all transactions in pool
            ...
            .sum
          _ <- Either.cond(
            (transactionsBytes + tx.bytes().length) <= utxSettings.maxBytesSize,
            (),
          ...
          diff <- TransactionDiffer(fs,
                                    blockchain.lastBlockTimestamp,
                                    time.correctedTime(),
                                    blockchain.height)(blockchain, tx)
        } yield {
  ...
  override def packUnconfirmed(rest: MultiDimensionalMiningConstraint)
    ...
    val differ = TransactionDiffer(fs,
                                   blockchain.lastBlockTimestamp,
                                   time.correctedTime(),
                                   blockchain.height) _
    val (invalidTxs, reversedValidTxs, _, finalConstraint, _) =
        ...
        .scanLeft(
          ...
          case ((invalid, valid, diff, currRest, isEmpty), tx) =>
            ...
            if (updatedRest.isOverfilled) {
              log.trace(
                s"Mining constraints overfilled: ${updatedRest.constraints
  ...
  private[this] object TxCheck {
    ...
    def transactionIsValid(transaction: Transaction,
                           lastBlockTimestamp: Option[Long] =
                             blockchain.lastBlockTimestamp,
                           currentTime: Long = time.correctedTime(),
                           height: Int = blockchain.height) = {
      !transactionIsExpired(transaction) && TransactionDiffer(
        fs,
        lastBlockTimestamp,
        currentTime,
        height)(blockchain, transaction).isRight
    }
  }

  //noinspection ScalaStyle
  object cleanup {
    private[UtxPoolImpl] implicit val scheduler: SchedulerService =
      Scheduler.singleThread("utx-pool-cleanup")
    ...
    def runCleanupOn(observable: Observable[_]): Cancelable = {
      observable
        .whileBusyDropEventsAndSignal(dropped =>
          log.warn(s"UTX pool cleanup is too slow, $dropped cleanups skipped"))
...
object UtxPoolImpl {
  private class PessimisticPortfolios(
    ...
    private val transactionPortfolios =
      new ConcurrentHashMap[ByteStr, Portfolios]()
    private val transactions = new ConcurrentHashMap[Address, Set[ByteStr]]()

    def add(txId: ByteStr, txDiff: Diff): Unit = {
      val pessimisticPortfolios = txDiff.portfolios.map {
        case (addr, portfolio) => addr -> portfolio.pessimistic
      }
      val nonEmptyPessimisticPortfolios = pessimisticPortfolios.filterNot {
      ...
      if (nonEmptyPessimisticPortfolios.nonEmpty &&
        ...
        nonEmptyPessimisticPortfolios.keys.foreach { address =>
          transactions.put(address,
                           transactions.getOrDefault(address, Set.empty) + txId)
        }
      }
    ...
    def getAggregated(accountAddr: Address): Portfolio = {
      val portfolios = for {
        ...
        txPortfolios = transactionPortfolios.getOrDefault(
          txId,
      ...
      Option(transactionPortfolios.remove(txId)) match {
        case Some(txPortfolios) =>
          txPortfolios.foreach {
            case (addr, p) =>
              transactions.computeIfPresent(addr,
                                            (_, prevTxs) => prevTxs - txId)
              p.assetIds.foreach(assetId =>
                spendableBalanceChanged.onNext(addr -> assetId))
          }
        case None =>
