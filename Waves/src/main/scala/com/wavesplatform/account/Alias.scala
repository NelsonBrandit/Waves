...
sealed trait Alias extends AddressOrAlias {
  ...
  lazy val bytes: ByteStr = ByteStr(
    Alias.AddressVersion +: chainId +: Deser.serializeArray(
      name.getBytes("UTF-8")))
...
object Alias {
  ...
  private[this] val AliasPatternInfo =
    "Alias string pattern is 'alias:<chain-id>:<address-alias>"
  ...
  private[wavesplatform] def createWithChainId(
      name: String,
      chainId: Byte = currentChainId): Either[ValidationError, Alias] = {
    ...
    if (name.length < MinLength || MaxLength < name.length)
      ...
      Left(
        GenericError(
          s"Alias should contain only following characters: $aliasAlphabet"))
