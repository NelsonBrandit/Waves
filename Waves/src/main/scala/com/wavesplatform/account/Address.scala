...
object Address extends ScorexLogging {
  ...
  def fromPublicKey(publicKey: Array[Byte],
                    chainId: Byte = scheme.chainId): Address = {
    ...
    val bytes = ByteBuffer
      ...
      .put(withoutChecksum)
      .put(crypto.secureHash(withoutChecksum), 0, ChecksumLength)
      .array()
    createUnsafe(bytes)
  }
  ...
  def fromBytes(
      ...
      chainId: Byte = scheme.chainId): Either[InvalidAddress, Address] = {
    val version = addressBytes.head
    val network = addressBytes.tail.head
    (for {
      _ <- Either.cond(version == AddressVersion,
                       (),
      ...
      _ <- Either.cond(checkSum.sameElements(checkSumGenerated),
                       (),
  ...
  def fromString(addressStr: String): Either[ValidationError, Address] = {
    val base58String =
      if (addressStr.startsWith(Prefix)) addressStr.drop(Prefix.length)
      else addressStr
    for {
      _ <- Either.cond(
      ...
      byteArray <- Base58
        ...
        .toEither
        .left
        .map(ex => InvalidAddress(s"Unable to decode base58: ${ex.getMessage}"))
      address <- fromBytes(byteArray)
    } yield address
  }

  def calcCheckSum(withoutChecksum: Array[Byte]): Array[Byte] =
    crypto.secureHash(withoutChecksum).take(ChecksumLength)
}
