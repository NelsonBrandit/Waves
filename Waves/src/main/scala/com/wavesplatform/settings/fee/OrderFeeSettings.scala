...
object OrderFeeSettings {
  ...
  case class FixedSettings(defaultAssetId: Asset, minFee: Long)
      extends OrderFeeSettings
  case class PercentSettings(assetType: AssetType, minFee: Double)
      extends OrderFeeSettings

  implicit val orderFeeSettingsReader: ValueReader[OrderFeeSettings] = {
    (cfg, path) =>
      def getPrefixByMode(mode: Mode): String = s"$path.$mode"

      def validateSafe[T: ValueReader](settingName: String): ErrorsListOr[T] = {
        Validated
          .fromTry(Try(cfg.as[T](settingName)))
          .leftMap(_ =>
            List(
              s"Invalid setting $settingName value: ${cfg.getString(settingName)}"))
      }

      def validateByPredicate[T: ValueReader](settingName: String)(
          predicate: T => Boolean,
          errorMsg: String): ErrorsListOr[T] = {

        val settingValue = cfg.as[T](settingName)
        val additionalInfo =
          Option(errorMsg).filter(_.nonEmpty).fold("")(m => s", $m")

        Validated.cond(
          predicate(settingValue),
          settingValue,
          List(
            s"Invalid setting $settingName value: $settingValue$additionalInfo"))
      }

      def validateAssetId(settingName: String)(
          assetIdStr: String): ErrorsListOr[Asset] = {
        Validated
          .fromTry(AssetPair.extractAssetId(assetIdStr))
          .leftMap(_ =>
            List(s"Invalid setting $settingName value: $assetIdStr"))
          .ensure(List(
            s"Invalid setting $settingName value: $assetIdStr, asset must not be Waves"))(
            _ != Waves)
      }

      def validateFixedWavesSettings: ErrorsListOr[FixedWavesSettings] = {
        validateByPredicate[Long](
          s"${getPrefixByMode(Mode.FIXED_WAVES)}.base-fee")(
          _ > 0,
          "must be > 0") map FixedWavesSettings
      }
      ...
      def validateFixedSettings: ErrorsListOr[FixedSettings] = {
        ...
        (
          ...
          validateByPredicate[Long](fixedMinFeeSettingName)(_ > 0,
                                                            "must be > 0")
        ) mapN FixedSettings
      }
      ...
      def validatePercentSettings: ErrorsListOr[PercentSettings] = {
        ...
        (
          ...
          validateByPredicate[Double](percentSettingName)(
            percent => 0 < percent && percent <= 100,
            "required 0 < p <= 100")
        ) mapN PercentSettings
      }

      def getSettingsByMode(mode: Mode): ErrorsListOr[OrderFeeSettings] =
        mode match {
      ...
      validateSafe[Mode](s"$path.mode").toEither.flatMap(mode =>
        getSettingsByMode(mode).toEither) match {
