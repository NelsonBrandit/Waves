...
object RestAPISettings {
  ...
  def fromConfig(config: Config): RestAPISettings = {
    RestAPISettings(
      ...
      transactionByAddressLimit =
        config.as[Int](s"$configPath.transactions-by-address-limit"),
      distributionAddressLimit =
        config.as[Int](s"$configPath.distribution-address-limit")
    )
  }
}
