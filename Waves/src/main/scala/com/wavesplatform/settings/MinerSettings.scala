...
object MinerSettings {
  ...
  def fromConfig(c: Config): MinerSettings = {
    ...
    val noQuorumMiningDelay =
      config.as[FiniteDuration]("no-quorum-mining-delay")
    val offset = config
      .as[Option[FiniteDuration]]("minimal-block-generation-offset")
      .getOrElse(0.millis)
    val blockInterval = config.as[FiniteDuration](
      "interval-after-last-block-then-generation-is-allowed")
    val minMicroBlockAge = config.as[FiniteDuration]("min-micro-block-age")
    val maxTransactionsInKeyBlock =
      config.as[Int]("max-transactions-in-key-block")
    val maxTransactionsInMicroBlock =
      config.as[Int]("max-transactions-in-micro-block")
