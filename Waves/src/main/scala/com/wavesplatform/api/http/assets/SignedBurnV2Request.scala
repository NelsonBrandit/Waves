...
case class SignedBurnV2Request(
    @ApiModelProperty(value = "Base58 encoded Issuer public key",
                      required = true)
  ...
  def toTx: Either[ValidationError, BurnTransactionV2] =
    for {
      ...
      _t <- BurnTransactionV2.create(chainId,
                                     _sender,
                                     _assetId,
                                     quantity,
                                     fee,
...
object SignedBurnV2Request {
  implicit val reads: Reads[SignedBurnV2Request] = (
    (JsPath \ "senderPublicKey").read[String] and
      ...
      (JsPath \ "quantity")
        .read[Long]
        .orElse((JsPath \ "amount").read[Long]) and
