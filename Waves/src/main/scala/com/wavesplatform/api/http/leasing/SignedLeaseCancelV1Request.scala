...
case class SignedLeaseCancelV1Request(
    ...
    @ApiModelProperty(value = "Base58 encoded lease transaction id",
                      required = true)
    txId: String,
  ...
  def toTx: Either[ValidationError, LeaseCancelTransactionV1] =
    for {
      ...
      _t <- LeaseCancelTransactionV1.create(_sender,
                                            _leaseTx,
                                            fee,
                                            timestamp,
                                            _signature)
...
object SignedLeaseCancelV1Request {
  implicit val reads: Reads[SignedLeaseCancelV1Request] = (
    (JsPath \ "senderPublicKey").read[String] and
      (JsPath \ "txId")
        .read[String]
        .orElse((JsPath \ "leaseId").read[String]) and
      (JsPath \ "timestamp").read[Long] and
      (JsPath \ "signature").read[String] and
      (JsPath \ "fee").read[Long]
  )(SignedLeaseCancelV1Request.apply _)

  implicit val writes: Writes[SignedLeaseCancelV1Request] =
    Json.writes[SignedLeaseCancelV1Request]
}
