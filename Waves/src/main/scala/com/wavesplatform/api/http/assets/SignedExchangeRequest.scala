...
object SignedExchangeRequest {
  implicit val orderFormat: Format[Order] =
    com.wavesplatform.transaction.assets.exchange.OrderJson.orderFormat
  implicit val signedExchangeRequestFormat: Format[SignedExchangeRequest] =
    Json.format
}
...
case class SignedExchangeRequest(
  ...
  def toTx: Either[ValidationError, ExchangeTransaction] =
    for {
      ...
      o1 <- castOrder(order1)
      o2 <- castOrder(order2)
      _t <- ExchangeTransactionV1.create(o1,
                                         o2,
                                         amount,
                                         price,
                                         buyMatcherFee,
  ...
  def castOrder(o: Order): Either[ValidationError, OrderV1] = o match {
    ...
    case _ =>
      Left(GenericError(
        "ExchangeTransaction of version 1 can only contain orders of version 1"))
  }
}
