...
object SignedTransferV2Request {
  ...
  implicit val writes: Writes[SignedTransferV2Request] =
    Json
      .writes[SignedTransferV2Request]
      .transform((request: JsObject) => request + ("version" -> JsNumber(2)))
...
case class SignedTransferV2Request(
  ...
  def toTx: Either[ValidationError, TransferTransactionV2] =
    for {
      ...
      _assetId <- parseBase58ToAssetId(assetId.filter(_.length > 0),
                                       "invalid.assetId")
      ...
      _attachment <- parseBase58(attachment.filter(_.length > 0),
                                 "invalid.attachment",
                                 TransferTransaction.MaxAttachmentStringSize)
      t <- TransferTransactionV2.create(_assetId,
                                        _sender,
                                        _recipient,
                                        amount,
                                        timestamp,
