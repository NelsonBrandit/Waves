...
object InvokeScriptRequest {
            ...
            case JsDefined(JsString(n)) =>
              ByteStr
                ...
                .fold(ex => JsError(ex.getMessage),
                      bstr => JsSuccess(CONST_BYTESTR(bstr)))
            case _ =>
              JsError("value is missing or not an base64 encoded string")
          }
  ...
  implicit val unsignedInvokeScriptRequestReads =
    Json.reads[InvokeScriptRequest]
  implicit val signedInvokeScriptRequestReads =
    Json.reads[SignedInvokeScriptRequest]
...
case class InvokeScriptRequest(
    ...
    @(ApiModelProperty @field)(required = true) payment: Seq[
      InvokeScriptTransaction.Payment],
    @(ApiModelProperty @field)(
      dataType = "string",
      example = "3Mciuup51AxRrpSz7XhutnQYTkNT9691HAk") contractAddress: String,
    timestamp: Option[Long] = None)

@ApiModel(value = "Signed Data transaction")
case class SignedInvokeScriptRequest(
    @(ApiModelProperty @field)(value = "Base58 encoded sender public key",
                               required = true) senderPublicKey: String,
    ...
    @(ApiModelProperty @field)(
      ...
      example = "3Mciuup51AxRrpSz7XhutnQYTkNT9691HAk") contractAddress: String,
    @(ApiModelProperty @field)(required = true) call: InvokeScriptRequest.FunctionCallPart,
    @(ApiModelProperty @field)(required = true) payment: Option[
      Seq[InvokeScriptTransaction.Payment]],
  ...
  def toTx: Either[ValidationError, InvokeScriptTransaction] =
    for {
      ...
      _feeAssetId <- parseBase58ToAssetId(feeAssetId.filter(_.length > 0),
                                          "invalid.feeAssetId")
