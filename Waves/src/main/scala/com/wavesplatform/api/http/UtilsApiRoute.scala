...
case class UtilsApiRoute(timeService: Time, settings: RestAPISettings)
  ...
  @ApiOperation(
    value = "Decompile",
    notes = "Decompiles base64 script representation to string code",
    httpMethod = "POST")
  @ApiImplicitParams(
    Array(
  ...
  @Deprecated
  @Path("/script/compile")
  @ApiOperation(value = "Compile",
                notes = "Compiles string code to base64 script representation",
                httpMethod = "POST")
  @ApiImplicitParams(
    Array(
      new ApiImplicitParam(
  ...
  @ApiOperation(value = "Compile script",
                notes = "Compiles string code to base64 script representation",
                httpMethod = "POST")
  @ApiImplicitParams(
    Array(
  ...
  @ApiOperation(value = "Estimate",
                notes = "Estimates compiled code in Base64 representation",
                httpMethod = "POST")
  @ApiImplicitParams(
    Array(
      new ApiImplicitParam(
    ...
    (post & entity(as[String])) { code =>
      complete(
        Script
          ...
          .flatMap { script =>
            ScriptCompiler
              .estimate(script, script.stdLibVersion)
              .map((script, _))
          }
          .fold(
            e => ScriptCompilerError(e), {
  ...
  @ApiOperation(value = "Serialize transaction",
                notes = "Serialize transaction",
                httpMethod = "POST")
  @ApiImplicitParams(
    Array(
      new ApiImplicitParam(
  ...
  def transactionSerialize: Route =
    (pathPrefix("transactionSerialize") & post) {
      handleExceptions(jsonExceptionHandler) {
        json[JsObject] { jsv =>
          parseOrCreateTransaction(jsv)(tx =>
