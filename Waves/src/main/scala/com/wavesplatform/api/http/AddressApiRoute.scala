...
case class AddressApiRoute(settings: RestAPISettings,
  ...
  @ApiOperation(value = "Details for account",
                notes = "Account's script",
                httpMethod = "GET")
  @ApiImplicitParams(
    Array(
    ...
    (delete & withAuth) {
      if (Address.fromString(address).isLeft) {
        ...
        val deleted = wallet
          ...
          .exists(account => wallet.deleteAccount(account))
  ...
  @ApiImplicitParams(
    Array(
      new ApiImplicitParam(name = "message",
                           value = "Message to sign as a plain string",
                           required = true,
                           paramType = "body",
                           dataType = "string"),
      new ApiImplicitParam(name = "address",
  ...
  @ApiOperation(value = "Verify text",
                notes = "Check a signature of a message signed by an account",
                httpMethod = "POST")
  @ApiImplicitParams(
    Array(
  ...
  @ApiOperation(value = "Details for balance",
                notes = "Account's balances",
                httpMethod = "GET")
  @ApiImplicitParams(
    Array(
      new ApiImplicitParam(name = "address",
  ...
  def balanceDetails: Route = (path("balance" / "details" / Segment) & get) {
    address =>
      complete(
        Address
          .fromString(address)
  ...
  @ApiOperation(value = "Confirmed balance",
  ...
  @ApiImplicitParams(
    Array(
      new ApiImplicitParam(name = "address",
                           ...
                           dataType = "string",
                           paramType = "path"),
      new ApiImplicitParam(name = "confirmations",
                           value = "0",
                           required = true,
                           dataType = "integer",
                           paramType = "path")
  ...
  def postData: Route =
    processRequest("data",
                   (req: DataRequest) =>
                     doBroadcast(TransactionFactory.data(req, wallet, time)))

  @Path("/data/{address}")
  @ApiOperation(value = "Complete Data",
  ...
  @ApiOperation(value = "Data by Key",
                notes = "Read data associated with an account and a key",
                httpMethod = "GET")
  @ApiImplicitParams(
    Array(
      new ApiImplicitParam(name = "address",
  ...
  private def balanceJson(address: String,
                          confirmations: Int): ToResponseMarshallable = {
    Address
      .fromString(address)
      .right
      .map(
  ...
  private def balancesDetailsJson(account: Address): BalanceDetails = {
    ...
    BalanceDetails(
      account.address,
      portfolio.balance,
      GeneratingBalanceProvider.balance(blockchain,
  ...
  private def effectiveBalanceJson(
      address: String,
      confirmations: Int): ToResponseMarshallable = {
    Address
      .fromString(address)
      ...
      .map(
        acc =>
          ToResponseMarshallable(
            Balance(acc.address,
                    confirmations,
                    blockchain.effectiveBalance(acc, confirmations))))
      .getOrElse(InvalidAddress)
  }
  ...
  private def accountData(address: String): ToResponseMarshallable = {
    Address
      ...
      .map { acc =>
        ToResponseMarshallable(
          blockchain.accountData(acc).data.values.toSeq.sortBy(_.key))
      }
      .getOrElse(InvalidAddress)
  }

  private def accountData(address: String,
                          key: String): ToResponseMarshallable = {
    val result = for {
    ...
    json[SignedMessage] { m =>
      if (Address.fromString(address).isLeft) {
        ...
        val msg: Try[Array[Byte]] =
          if (decode)
            if (m.message.startsWith("base64:")) Base64.decode(m.message)
            else Base58.decode(m.message, 2048)
  ...
  private def verifySigned(msg: Try[Array[Byte]],
                           signature: String,
                           publicKey: String,
                           address: String) = {
    (msg, Base58.decode(signature), Base58.decode(publicKey)) match {
      case (Success(msgBytes), Success(signatureBytes), Success(pubKeyBytes)) =>
        ...
        val isValid = account.address == address && crypto.verify(
          signatureBytes,
          msgBytes,
          pubKeyBytes)
  ...
  @ApiImplicitParams(
    Array(
      new ApiImplicitParam(name = "publicKey",
                           value = "Public key Base58-encoded",
                           required = true,
                           paramType = "path",
                           dataType = "string")
    ))
  @ApiOperation(value = "Address from Public Key",
...
object AddressApiRoute {
  ...
  case class BalanceDetails(address: String,
                            regular: Long,
                            generating: Long,
                            available: Long,
                            effective: Long)
  ...
  case class AddressScriptInfo(address: String,
