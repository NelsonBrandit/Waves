...
object SignedSetAssetScriptRequest {
  ...
  implicit val signedSetAssetScriptRequestReads
    : Reads[SignedSetAssetScriptRequest] = (
...
case class SignedSetAssetScriptRequest(
  ...
  def toTx: Either[ValidationError, SetAssetScriptTransaction] =
    for {
      ...
      _asset <- parseBase58(assetId, "invalid.assetId", AssetIdStringLength)
        .map(IssuedAsset)
      ...
      t <- SetAssetScriptTransaction.create(chainId,
                                            _sender,
                                            _asset,
                                            _script,
                                            fee,
