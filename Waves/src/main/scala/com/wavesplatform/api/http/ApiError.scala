...
object ApiError {
  def fromValidationError(e: ValidationError): ApiError = {
    e match {
      ...
      case ValidationError.NegativeMinFee(x, of) =>
      ...
      case ValidationError.OrderValidationError(_, m) =>
      ...
      case TransactionValidationError(error, tx) =>
        error match {
          case ValidationError.Mistiming(errorMessage) =>
            Mistiming(errorMessage)
          case ValidationError
                .TransactionNotAllowedByScript(vars, isTokenScript) =>
...
case class ScriptExecutionError(tx: Transaction,
                                error: String,
                                log: Log,
  ...
  override val message: String =
    s"Error while executing ${if (isTokenScript) "token" else "account"}-script: $error"
  override lazy val json: JsObject = ScriptErrorJson(id, tx, message, log)
}

case class TransactionNotAllowedByAccountScript(tx: Transaction, log: Log)
    extends ApiError {
...
case class TransactionNotAllowedByAssetScript(tx: Transaction, log: Log)
    extends ApiError {
  ...
  def apply(errId: Int, tx: Transaction, message: String, log: Log): JsObject =
    Json.obj(
      ...
      "vars" -> Json.arr(log.map {
        case (k, Right(v)) =>
          Json.obj("name" -> k, "value" -> JsString(v.toString))
