...
object Importer extends ScorexLogging {
  def main(args: Array[String]): Unit = {
    ...
    blockchainFilename match {
      case Success(filename) =>
        ...
        createInputStream(filename) match {
          case Success(inputStream) =>
            ...
            val blockchainUpdater = StorageFactory(
              settings,
              db,
              time,
              Observer.empty(
            ...
            val extAppender = BlockAppender(blockchainUpdater,
                                            time,
                                            utxPoolStub,
                                            pos,
                                            settings,
                                            scheduler,
                                            verifyTransactions) _
              ...
              if (s1 == Ints.BYTES) {
                ...
                if (s2 == len) {
                  if (blocksToSkip > 0) {
                    ...
                    val Right(block) =
                      if (format == "BINARY_OLD")
                        Block.parseBytes(buffer).toEither
                      else
                        PBBlocks.vanilla(
                          protobuf.block.PBBlock.parseFrom(buffer),
                          unsafe = true)
      ...
      case Failure(_) =>
        log.error(
          "Usage: Importer [-n | -no-verify] <config file> <blockchain file> [height]")
    }

    time.close()
  }

  private[this] def createInputStream(filename: String) =
