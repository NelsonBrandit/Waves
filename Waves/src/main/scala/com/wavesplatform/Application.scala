...
import com.wavesplatform.api.http.assets.{
  AssetsApiRoute,
  AssetsBroadcastApiRoute
}
import com.wavesplatform.api.http.leasing.{
  LeaseApiRoute,
  LeaseBroadcastApiRoute
}
...
import com.wavesplatform.state.appender.{
  BlockAppender,
  ExtensionAppender,
  MicroblockAppender
}
...
class Application(val actorSystem: ActorSystem,
  ...
  private val spendableBalanceChanged =
    ConcurrentSubject.publish[(Address, Asset)]

  private val blockchainUpdater =
    StorageFactory(settings, db, time, spendableBalanceChanged)
  ...
  def run(): Unit = {
    ...
    val processFork =
      ExtensionAppender(blockchainUpdater,
                        utxStorage,
                        pos,
                        time,
                        settings,
    ...
    if (settings.restAPISettings.enable) {
      val apiRoutes = Seq(
        ...
        PaymentApiRoute(settings.restAPISettings,
                        wallet,
                        utxStorage,
                        allChannels,
                        time),
        ...
        WavesApiRoute(settings.restAPISettings,
                      wallet,
                      utxStorage,
                      allChannels,
                      time),
      ...
      val combinedRoute =
        CompositeHttpService(apiTypes, apiRoutes, settings.restAPISettings)(
          actorSystem).loggingCompositeRoute
  ...
  private def shutdownAndWait(scheduler: SchedulerService,
    ...
    scheduler.shutdown()
    val r =
      Await.result(scheduler.awaitTermination(timeout, global), 2 * timeout)
    if (r)
