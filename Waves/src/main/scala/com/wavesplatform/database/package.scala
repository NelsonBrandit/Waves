...
package object database {
  ...
  def readTransactionHNSeqAndType(
      bs: Array[Byte]): (Height, Seq[(Byte, TxNum)]) = {
    val ndi = newDataInput(bs)
  ...
  def writeTransactionHNSeqAndType(
      v: (Height, Seq[(Byte, TxNum)])): Array[Byte] = {
    val (height, numSeq) = v
  ...
  implicit class EntryExt(val e: JMap.Entry[Array[Byte], Array[Byte]])
      extends AnyVal {
  ...
  implicit class DBExt(val db: DB) extends AnyVal {
    ...
    def iterateOver(prefix: Short)(
        f: JMap.Entry[Array[Byte], Array[Byte]] => Unit): Unit =
    ...
    def iterateOver(prefix: Array[Byte])(
      ...
      try {
        ...
        while (iterator.hasNext && iterator
                 .peekNext()
                 .getKey
                 .startsWith(prefix)) f(iterator.next())
