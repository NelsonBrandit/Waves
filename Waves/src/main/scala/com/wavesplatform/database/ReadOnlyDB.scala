...
class ReadOnlyDB(db: DB, readOptions: ReadOptions) {
  ...
  def iterateOver(prefix: Short)(f: Entry[Array[Byte], Array[Byte]] => Unit) =
    db.iterateOver(prefix)(f)

  def iterateOver(prefix: Array[Byte])(
      f: Entry[Array[Byte], Array[Byte]] => Unit) = db.iterateOver(prefix)(f)

  def read[T](keyName: String, prefix: Array[Byte], seek: Array[Byte], n: Int)(
      deserialize: ReadOnlyDB.Entry => T): Vector[T] = {
    val iter = iterator
    @tailrec def loop(aux: Vector[T],
                      restN: Int,
                      totalBytesRead: Long): (Vector[T], Long) = {
      if (restN > 0 && iter.hasNext) {
        val elem = iter.next()
        if (elem.getKey.startsWith(prefix))
          loop(aux :+ deserialize(elem),
               restN - 1,
               totalBytesRead + elem.getValue.length)
