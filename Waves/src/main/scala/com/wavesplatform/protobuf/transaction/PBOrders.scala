...
object PBOrders {
  ...
  def vanilla(order: PBOrder, version: Int = 0): VanillaOrder = {
    VanillaOrder(
      ...
      vt.assets.exchange.AssetPair(
        Asset.fromProtoId(order.getAssetPair.amountAssetId),
        Asset.fromProtoId(order.getAssetPair.priceAssetId)),
      order.orderSide match {
        ...
        case PBOrder.Side.Unrecognized(v) =>
          throw new IllegalArgumentException(s"Unknown order type: $v")
      },
      order.amount,
      order.price,
      order.timestamp,
  ...
  def protobuf(order: VanillaOrder): PBOrder = {
    PBOrder(
      ...
      Some(
        PBOrder.AssetPair(order.assetPair.amountAsset.protoId,
                          order.assetPair.priceAsset.protoId)),
