...
private[protobuf] object PBInternalImplicits {
  ...
  implicit def byteStrToByteString(bs: ByteStr): PBByteString =
    PBByteString.copyFrom(bs)

  implicit def fromAddressOrAlias(addressOrAlias: AddressOrAlias): Recipient =
    PBRecipients.create(addressOrAlias)

  implicit class PBRecipientImplicitConversionOps(recipient: Recipient) {
    def toAddress: Either[ValidationError, VAddress] =
      PBRecipients.toAddress(recipient)
    def toAlias: Either[ValidationError, VAlias] =
      PBRecipients.toAlias(recipient)
    def toAddressOrAlias: Either[ValidationError, AddressOrAlias] =
      PBRecipients.toAddressOrAlias(recipient)
  }
  ...
  implicit class AmountImplicitConversions(a: Amount) {
    ...
    def assetId: Asset = a.amount match {
      ...
      case Amount.Amount.AssetAmount(AssetAmount(assetId, _)) =>
        IssuedAsset(assetId)
    }
  }
