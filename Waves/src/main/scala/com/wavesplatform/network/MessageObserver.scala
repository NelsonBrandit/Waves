...
class MessageObserver extends ChannelInboundHandlerAdapter with ScorexLogging {
  ...
  implicit val scheduler =
    monix.execution.Scheduler.fixedPool("message-observer", 2)
  ...
  private val microblockInvs =
    ConcurrentSubject.publish[(Channel, MicroBlockInv)]
  private val microblockResponses =
    ConcurrentSubject.publish[(Channel, MicroBlockResponse)]
  ...
  override def channelRead(ctx: ChannelHandlerContext, msg: AnyRef): Unit =
    msg match {
      ...
      case mb: MicroBlockResponse =>
        microblockResponses.onNext((ctx.channel(), mb))
