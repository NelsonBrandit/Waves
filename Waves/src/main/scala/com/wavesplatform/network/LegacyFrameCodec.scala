...
class LegacyFrameCodec(peerDatabase: PeerDatabase,
                       receivedTxsCacheTimeout: FiniteDuration)
    extends ByteToMessageCodec[RawBytes]
    with ScorexLogging {
  ...
  private val receivedTxsCache = CacheBuilder
    .newBuilder()
    .expireAfterWrite(receivedTxsCacheTimeout.length,
                      receivedTxsCacheTimeout.unit)
    .build[String, Object]()
  ...
  override def decode(ctx: ChannelHandlerContext,
    ...
    try {
      ...
      require(
        ...
        s"${spec.messageName} message length $length exceeds ${spec.maxLength}")
