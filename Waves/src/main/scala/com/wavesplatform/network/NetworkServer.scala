...
object NetworkServer extends ScorexLogging {
  ...
  def apply(settings: WavesSettings,
    ...
    val bossGroup =
      new NioEventLoopGroup(0, new DefaultThreadFactory("nio-boss-group", true))
    val workerGroup = new NioEventLoopGroup(
      0,
      new DefaultThreadFactory("nio-worker-group", true))
    ...
    val trafficLogger = new TrafficLogger(
      settings.networkSettings.trafficLogger)
    ...
    val excludedAddresses: Set[InetSocketAddress] = {
      ...
      val localAddresses = if (isLocal) {
        NetworkInterface.getNetworkInterfaces.asScala
          .flatMap(_.getInetAddresses.asScala.map(a =>
            new InetSocketAddress(a, bindAddress.getPort)))
          .toSet
    ...
    val serverHandshakeHandler = new HandshakeHandler.Server(handshake,
                                                             peerInfo,
                                                             peerConnections,
                                                             peerDatabase,
                                                             allChannels)
    ...
    def peerSynchronizer: ChannelHandlerAdapter = {
      if (settings.networkSettings.enablePeersExchange) {
        new PeerSynchronizer(peerDatabase,
                             settings.networkSettings.peersBroadcastInterval)
    ...
    val serverChannel = settings.networkSettings.declaredAddress.map { _ =>
      new ServerBootstrap()
        ...
        .childHandler(new PipelineInitializer[SocketChannel](Seq(
          ...
          new BrokenConnectionDetector(
            settings.networkSettings.breakIdleConnectionsTimeout),
          new HandshakeDecoder(peerDatabase),
          new HandshakeTimeoutHandler(
            settings.networkSettings.handshakeTimeout),
          serverHandshakeHandler,
          lengthFieldPrepender,
          new LengthFieldBasedFrameDecoder(100 * 1024 * 1024, 0, 4, 0, 4),
          new LegacyFrameCodec(
            peerDatabase,
            settings.networkSettings.receivedTxsCacheTimeout),
    ...
    val clientHandshakeHandler = new HandshakeHandler.Client(handshake,
                                                             peerInfo,
                                                             peerConnections,
                                                             peerDatabase,
                                                             allChannels)
    ...
    val bootstrap = new Bootstrap()
      .option(
        ...
        settings.networkSettings.connectionTimeout.toMillis.toInt: Integer)
      .group(workerGroup)
    ...
    def formatOutgoingChannelEvent(channel: Channel, event: String) =
      s"${id(channel)} $event, outgoing channel count: ${outgoingChannels.size()}"

    def handleOutgoingChannelClosed(remoteAddress: InetSocketAddress)(
        closeFuture: ChannelFuture): Unit = {
      outgoingChannels.remove(remoteAddress, closeFuture.channel())
      if (!shutdownInitiated)
        peerDatabase.suspendAndClose(closeFuture.channel())
    ...
    def handleConnectionAttempt(remoteAddress: InetSocketAddress)(
        thisConnFuture: ChannelFuture): Unit = {
      if (thisConnFuture.isSuccess) {
        log.trace(
          formatOutgoingChannelEvent(thisConnFuture.channel(),
                                     "Connection established"))
        peerDatabase.touch(remoteAddress)
        thisConnFuture
          .channel()
          .closeFuture()
          .addListener(handleOutgoingChannelClosed(remoteAddress))
    ...
    def doConnect(remoteAddress: InetSocketAddress): Unit =
      outgoingChannels.computeIfAbsent(
        ...
        _ => {
          ...
          log.trace(
            s"${id(newConnFuture.channel())} Connecting to $remoteAddress")
          newConnFuture
            .addListener(handleConnectionAttempt(remoteAddress))
            .channel()
        }
      )
    ...
    val connectTask = workerGroup.scheduleWithFixedDelay(1.second, 5.seconds) {
      ...
      val all =
        peerInfo.values().iterator().asScala.flatMap(_.declaredAddress).toVector
      ...
      if (outgoingChannels
            .size() < settings.networkSettings.maxOutboundConnections) {
        peerDatabase
    ...
    new NS {
      override def connect(remoteAddress: InetSocketAddress): Unit =
