...
class PeerSynchronizer(peerDatabase: PeerDatabase,
                       peerRequestInterval: FiniteDuration)
  ...
  def requestPeers(ctx: ChannelHandlerContext): Unit =
    ...
    msg match {
      case hs: Handshake =>
        ...
        rda match {
          case None =>
            log.debug(
              s"${id(ctx)} Declared address $rda does not match actual remote address ${ctx.remoteAddress
                .map(_.getAddress)}")
          case Some(x) =>
      ...
      case KnownPeers(peers) if peersRequested =>
        ...
        log.trace(
          s"${id(ctx)} Added peers: ${format(added)}, not added peers: ${format(notAdded)}")
      case KnownPeers(peers) =>
        log.trace(
          s"${id(ctx)} Got unexpected list of known peers containing ${peers.size} entries")
      case _ =>
        super.channelRead(ctx, msg)
    }
  }
