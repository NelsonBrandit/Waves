...
@Sharable
class InboundConnectionFilter(peerDatabase: PeerDatabase,
  ...
  override def accept(ctx: ChannelHandlerContext,
                      remoteAddress: InetSocketAddress): Boolean =
    Option(remoteAddress.getAddress) match {
      case None =>
      ...
      case Some(address) =>
        ...
        val newCountPerHost = perHostConnectionCount.compute(
          address,
          (_, cnt) => Option(cnt).fold(1)(_ + 1))
        ...
        log.trace(
          s"Check inbound connection from $remoteAddress: new inbound total = $newTotal, " +
            s"connections with this host = $newCountPerHost, address ${if (isBlacklisted) "IS"
  ...
  override def channelAccepted(ctx: ChannelHandlerContext,
                               remoteAddress: InetSocketAddress): Unit =
    ctx
      .channel()
      .closeFuture()
      .addListener((_: ChannelFuture) =>
        Option(remoteAddress.getAddress).foreach(dec))

  override def channelRejected(
      ctx: ChannelHandlerContext,
      remoteAddress: InetSocketAddress): ChannelFuture =
