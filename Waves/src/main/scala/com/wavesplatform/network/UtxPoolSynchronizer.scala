...
object UtxPoolSynchronizer extends ScorexLogging {
  ...
  def start(
    ...
    implicit val scheduler: Scheduler = Scheduler.forkJoin(settings.parallelism,
                                                           settings.maxThreads,
                                                           "utx-pool-sync")
    ...
    val knownTransactions = CacheBuilder
      ...
      .expireAfterWrite(settings.networkTxCacheTime.toMillis,
    ...
    val synchronizerFuture = newTxSource
      .whileBusyBuffer(
        OverflowStrategy.DropOldAndSignal(settings.maxQueueSize, { dropped =>
        ...
        case (sender, transaction) =>
          Task {
            concurrent.blocking(utx.putIfNew(transaction)) match {
              case Right((isNew, _)) =>
                if (isNew)
                  Some(allChannels.write(RawBytes.from(transaction),
                                         (_: Channel) != sender))
                else None
