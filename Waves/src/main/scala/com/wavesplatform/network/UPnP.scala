...
class UPnP(settings: UPnPSettings) extends ScorexLogging {
  ...
  lazy val externalAddress =
    gateway.map(_.getExternalIPAddress).map(InetAddress.getByName)
  ...
  Try {
    ...
    val gatewayMap =
      Option(discover.discover).map(_.asScala.toMap).getOrElse(Map())
    if (gatewayMap.isEmpty) {
      ...
      Option(discover.getValidGateway) match {
        ...
        case Some(device) =>
          gateway = Some(device)
          log.debug(
            "Using UPnP gateway device on " + localAddress
              .map(_.getHostAddress)
              .getOrElse("err"))
          log.info(
            "External IP address is " + externalAddress
              .map(_.getHostAddress)
              .getOrElse("err"))
      }
    }
  ...
  def addPort(port: Int): Try[Unit] =
    Try {
      if (gateway.get.addPortMapping(port,
                                     port,
                                     localAddress.get.getHostAddress,
                                     "TCP",
                                     "Scorex")) {
        log.debug(
          "Mapped port [" + externalAddress.get.getHostAddress + "]:" + port)
      } else {
  ...
  def deletePort(port: Int): Try[Unit] =
    Try {
      ...
      case t: Throwable =>
        log.error(
          "Unable to delete mapping for port " + port + ": " + t.toString)
    }
}
