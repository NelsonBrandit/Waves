...
case class LongBlockField(override val name: String, override val value: Long)
  ...
  protected override def b =
    Bytes.ensureCapacity(Longs.toByteArray(value), 8, 0)
}

case class BlockIdField(override val name: String,
...
case class TransactionBlockField(override val name: String,
                                 override val value: Transaction)
...
case class SignerDataBlockField(override val name: String,
                                override val value: SignerData)
    extends BlockField[SignerData] {
  protected override def j =
    Json.obj("generator" -> value.generator.toString,
