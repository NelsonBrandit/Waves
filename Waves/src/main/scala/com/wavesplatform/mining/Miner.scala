...
trait MinerDebugInfo {
  ...
  def getNextBlockGenerationOffset(
      account: PrivateKeyAccount): Either[String, FiniteDuration]
}
...
class MinerImpl(allChannels: ChannelGroup,
  ...
  private lazy val minMicroBlockDurationMills =
    minerSettings.minMicroBlockAge.toMillis
  private lazy val blockchainSettings = settings.blockchainSettings

  private val scheduledAttempts = SerialCancelable()
  private val microBlockAttempt = SerialCancelable()

  private val blockBuildTimeStats = Kamon.histogram(
    "pack-and-forge-block-time",
    MeasurementUnit.time.milliseconds)
  private val microBlockBuildTimeStats =
    Kamon.histogram("forge-microblock-time", MeasurementUnit.time.milliseconds)

  @volatile private var debugState: MinerDebugInfo.State =
    MinerDebugInfo.Disabled

  def getNextBlockGenerationOffset(
      account: PrivateKeyAccount): Either[String, FiniteDuration] =
    nextBlockGenOffsetWithConditions(account)

  private def checkAge(parentHeight: Int,
                       parentTimestamp: Long): Either[String, Unit] =
    Either
      .cond(parentHeight == 1,
            (),
            (timeService.correctedTime() - parentTimestamp).millis)
      .left
      .flatMap(blockAge =>
        Either.cond(
  ...
  private def forgeBlock(account: PrivateKeyAccount)
    : Either[String, (MiningConstraints, Block, MiningConstraint)] = {
    // should take last block right at the time of mining since microblocks might have been added
    val height = blockchainUpdater.height
    val version =
      if (height <= blockchainSettings.functionalitySettings.blockVersion3AfterHeight)
        PlainBlockVersion
      else NgBlockVersion
    val lastBlock = blockchainUpdater.lastBlock.get
    val referencedBlockInfo = blockchainUpdater
      .bestLastBlockInfo(
        System.currentTimeMillis() - minMicroBlockDurationMills)
      .get
    ...
    measureSuccessful(
      ...
      for {
        ...
        validBlockDelay <- pos
          ...
          .leftMap(_.toString)
          .ensure(
            s"$currentTime: Block delay $blockDelay was NOT less than estimated delay")(
            _ < blockDelay)
        _ = log.debug(
          s"Forging with ${account.address}, Time $blockDelay > Estimated Time $validBlockDelay, balance $balance, prev block $refBlockID")
        _ = log.debug(
          s"Previous block ID $refBlockID at $height with target $refBlockBT")
        consensusData <- consensusData(height,
                                       account,
                                       lastBlock,
                                       refBlockBT,
                                       refBlockTS,
                                       balance,
                                       currentTime)
        estimators = MiningConstraints(blockchainUpdater,
                                       height,
                                       Some(minerSettings))
        mdConstraint = MultiDimensionalMiningConstraint(estimators.total,
                                                        estimators.keyBlock)
        (unconfirmed, updatedMdConstraint) = utx.packUnconfirmed(mdConstraint)
        _ = log.debug(
          s"Adding ${unconfirmed.size} unconfirmed transaction(s) to new block")
        block <- Block
  ...
  private def checkQuorumAvailable(): Either[String, Unit] = {
    ...
    Either.cond(
      chanCount >= minerSettings.quorum,
      (),
      s"Quorum not available ($chanCount/${minerSettings.quorum}), not forging block.")
  }
  ...
  private def generateOneMicroBlockTask(
    ...
    log.trace(
      s"Generating microBlock for $account, constraints: $restTotalConstraint")
    val pc = allChannels.size()
    if (pc < minerSettings.quorum) {
      log.trace(
        s"Quorum not available ($pc/${minerSettings.quorum}), not forging microblock with ${account.address}, next attempt in 5 seconds")
      ...
      val (unconfirmed, updatedTotalConstraint) =
        measureLog("packing unconfirmed transactions for microblock") {
          val mdConstraint =
            MultiDimensionalMiningConstraint(restTotalConstraint,
                                             constraints.micro)
          val (unconfirmed, updatedMdConstraint) =
            utx.packUnconfirmed(mdConstraint)
      ...
      if (unconfirmed.isEmpty) {
        log.trace {
          if (updatedTotalConstraint.isEmpty)
            s"Stopping forging microBlocks, the block is full: $updatedTotalConstraint"
          else
            "Stopping forging microBlocks, because all transactions are too big"
        }
        Task.now(Stop)
      } else {
        ...
        (for {
          ...
          microBlock <- EitherT.fromEither[Task](
            MicroBlock.buildAndSign(account,
                                    unconfirmed,
                                    accumulatedBlock.signerData.signature,
                                    signedBlock.signerData.signature))
          _ = microBlockBuildTimeStats.safeRecord(
            System.currentTimeMillis() - start)
          _ <- EitherT(
            MicroblockAppender(blockchainUpdater,
                               utx,
                               appenderScheduler,
                               verify = false)(microBlock))
          ...
          case Right((microBlock, signedBlock)) =>
            ...
            allChannels.broadcast(
              MicroBlockInv(account,
                            microBlock.totalResBlockSig,
                            microBlock.prevResBlockSig))
            if (updatedTotalConstraint.isEmpty) {
              log.trace(
                s"$microBlock has been mined for $account. Stop forging microBlocks, the block is full: $updatedTotalConstraint")
              Stop
            } else {
  ...
  private def generateMicroBlockSequence(
      account: PrivateKeyAccount,
      accumulatedBlock: Block,
      delay: FiniteDuration,
      constraints: MiningConstraints,
    ...
    generateOneMicroBlockTask(account,
                              accumulatedBlock,
                              constraints,
                              restTotalConstraint)
      ...
      .flatMap {
        ...
        case Success(newTotal, updatedTotalConstraint) =>
          generateMicroBlockSequence(account,
                                     newTotal,
                                     minerSettings.microBlockInterval,
                                     constraints,
  ...
  private def nextBlockGenerationTime(
    ...
    val balance = GeneratingBalanceProvider.balance(blockchainUpdater,
                                                    fs,
                                                    account.toAddress,
                                                    block.uniqueId)

    if (GeneratingBalanceProvider.isMiningAllowed(blockchainUpdater,
                                                  height,
                                                  balance)) {
      for {
        expectedTS <- pos
          .getValidBlockDelay(height,
                              account.publicKey,
  ...
  private def nextBlockGenOffsetWithConditions(
      account: PrivateKeyAccount): Either[String, FiniteDuration] = {
    val height = blockchainUpdater.height
    val lastBlock = blockchainUpdater.lastBlock.get
    for {
      ...
      ts <- nextBlockGenerationTime(blockchainSettings.functionalitySettings,
                                    height,
                                    lastBlock,
                                    account)
      calculatedOffset = ts - timeService.correctedTime()
      offset = Math
        .max(calculatedOffset,
             minerSettings.minimalBlockGenerationOffset.toMillis)
        .millis
      ...
      case Right(offset) =>
        ...
        generateOneBlockTask(account)(offset).flatMap {
          case Right((estimators, block, totalConstraint)) =>
            BlockAppender(blockchainUpdater,
                          timeService,
                          utx,
                          pos,
                          settings,
                          appenderScheduler)(block)
              .asyncBoundary(minerScheduler)
              .map {
                case Left(err) =>
                ...
                case Right(Some(score)) =>
                  log.debug(
                    s"Forged and applied $block by ${account.address} with cumulative score $score")
                  BlockStats.mined(block, blockchainUpdater.height)
                  allChannels.broadcast(BlockForged(block))
                  scheduleMining()
                  if (ngEnabled && !totalConstraint.isEmpty)
                    startMicroBlockMining(account,
                                          block,
                                          estimators,
                                          totalConstraint)
                case Right(None) =>
                  log.warn(
                    "Newly created block has already been appended, should not happen")
              }
  ...
  def scheduleMining(): Unit = {
    ...
    val nonScriptedAccounts =
      wallet.privateKeyAccounts.filterNot(blockchainUpdater.hasScript(_))
    scheduledAttempts := CompositeCancelable.fromSet(
      nonScriptedAccounts.map(generateBlockTask).map(_.runAsyncLogErr).toSet)
  ...
  private def startMicroBlockMining(
    ...
    microBlockAttempt := generateMicroBlockSequence(
      account,
      lastBlock,
      Duration.Zero,
      constraints,
      restTotalConstraint).runAsyncLogErr
...
object Miner {
  ...
  val Disabled = new Miner with MinerDebugInfo {
    ...
    override def getNextBlockGenerationOffset(
        account: PrivateKeyAccount): Either[String, FiniteDuration] =
      Left("Disabled")
  ...
  case class Success(b: Block, totalConstraint: MiningConstraint)
      extends MicroblockMiningResult
