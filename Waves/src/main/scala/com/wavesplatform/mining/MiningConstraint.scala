...
object MiningConstraint {
  case object Unlimited extends MiningConstraint {
    ...
    override def put(blockchain: Blockchain, x: Transaction): MiningConstraint =
      this
  }
}

case class OneDimensionalMiningConstraint(rest: Long,
                                          estimator: TxEstimators.Fn,
                                          description: String = "")
  ...
  override def put(blockchain: Blockchain,
                   x: Transaction): OneDimensionalMiningConstraint =
    put(estimator(blockchain, x))
...
case class MultiDimensionalMiningConstraint(
    constraints: NonEmptyList[MiningConstraint])
    extends MiningConstraint {
  ...
  override def put(blockchain: Blockchain,
                   x: Transaction): MultiDimensionalMiningConstraint =
    MultiDimensionalMiningConstraint(constraints.map(_.put(blockchain, x)))
}

object MultiDimensionalMiningConstraint {
  def apply(constraint1: MiningConstraint,
            constraint2: MiningConstraint): MultiDimensionalMiningConstraint =
