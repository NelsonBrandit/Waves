...
class CustomInfluxDBReporter(config: Config = Kamon.config())
  ...
  private def translateToLineProtocol(
      periodSnapshot: PeriodSnapshot): RequestBody = {
    import periodSnapshot.metrics._
    val builder = StringBuilder.newBuilder

    counters.foreach(c =>
      writeMetricValue(builder, c, "count", periodSnapshot.to.getEpochSecond))
    gauges.foreach(g =>
      writeMetricValue(builder, g, "value", periodSnapshot.to.getEpochSecond))
    histograms.foreach(
      h =>
        writeMetricDistribution(builder,
                                h,
                                settings.percentiles,
                                periodSnapshot.to.getEpochSecond))
  ...
  private def writeMetricValue(builder: StringBuilder,
                               metric: MetricValue,
                               fieldName: String,
                               timestamp: Long): Unit = {
  ...
  private def writeMetricDistribution(builder: StringBuilder,
    ...
    percentiles.foreach(p => {
      writeDoubleField(builder,
                       "p" + String.valueOf(p),
                       metric.distribution.percentile(p).value)
    })

    writeIntField(builder,
                  "max",
                  metric.distribution.max,
                  appendSeparator = false)
    writeTimestamp(builder, timestamp)
  }

  private def writeNameAndTags(builder: StringBuilder,
                               name: String,
                               metricTags: Map[String, String]): Unit = {
    builder
      .append(name)

    val tags =
      if (settings.additionalTags.nonEmpty)
        metricTags ++ settings.additionalTags
      else metricTags
  ...
  def writeDoubleField(builder: StringBuilder,
                       fieldName: String,
                       value: Double,
                       appendSeparator: Boolean = true): Unit = {
    builder
      .append(fieldName)
      .append('=')
  ...
  def writeIntField(builder: StringBuilder,
                    fieldName: String,
                    value: Long,
                    appendSeparator: Boolean = true): Unit = {
    builder
  ...
  private def buildClient(settings: Settings): OkHttpClient = {
    ...
    authenticator
      .foldLeft(basicBuilder) {
        case (builder, auth) => builder.authenticator(auth)
      }
      .build()
  }
}
...
object CustomInfluxDBReporter {
  case class Settings(url: String,
                      percentiles: Seq[Double],
  ...
  def readSettings(config: Config): Settings = {
    ...
    val credentials = authConfig.map(conf =>
      Credentials.basic(conf.getString("user"), conf.getString("password")))
    val port = root.getInt("port")
    val database = root.getString("database")
    val url = s"http://$host:$port/write?precision=s&db=$database"

    val additionalTags =
      EnvironmentTagBuilder.create(root.getConfig("additional-tags"))
