...
object GenesisBlockGenerator extends App {
  ...
  private val settings: Settings = {
    ...
    val file = new File(
      args.headOption.getOrElse(
        throw new IllegalArgumentException("Specify a path to genesis.conf")))
    if (!file.exists()) {
  ...
  val shares: Map[FullAddressInfo, Share] = {
    settings.distributions.map {
      case (seedText, part) => toFullAddressInfo(seedText) -> part
    }
  }
  ...
  val genesisTxs: Seq[GenesisTransaction] = shares.map {
    case (addrInfo, part) =>
      GenesisTransaction(addrInfo.accountAddress,
                         part,
                         timestamp,
                         ByteStr.empty)
  }.toSeq
