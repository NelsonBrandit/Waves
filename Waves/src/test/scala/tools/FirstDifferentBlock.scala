...
object FirstDifferentBlock extends App {
  ...
  def blockAt(nodeHttp: String, blockHeight: Int) =
    get(nodeHttp + "/blocks/at/" + blockHeight)
  def blockSigAt(nodeHttp: String, blockHeight: Int) =
    (Json.parse(blockAt(nodeHttp, blockHeight)) \ "signature").get.as[String]
  ...
  println(
    "first different block height is " + firstDifferent(
      1,
      258,
      nodeComparator(DEVNET3D, DEVNET3)))
}
