...
class TxEstimatorsSuite
  ...
  "scriptRunNumber" - {
    ...
    "smart tokens" - {
      ...
      "should count transactions working with smart tokens" in {
        ...
        (blockchain.assetDescription _)
          .when(*)
          .onCall((_: IssuedAsset) => Some(assetDescription))
          .anyNumberOfTimes()
  ...
  private val script =
    ExprScript(V1, Terms.TRUE, checkSize = false).explicitGet()
