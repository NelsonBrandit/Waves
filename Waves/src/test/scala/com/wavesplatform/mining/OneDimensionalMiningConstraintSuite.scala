...
class OneDimensionalMiningConstraintSuite
  ...
  private def tests(
      toConstraint: (Int, List[Transaction]) => MiningConstraint): Unit = {
    ...
    "multiple items don't reach the limit" in forAll(dontReachLimitGen) {
      updatedConstraint =>
    ...
    "multiple items reach the limit softly" in forAll(reachSoftLimitGen) {
      updatedConstraint =>
    ...
    "multiple items reach the limit with gap" in forAll(reachHardLimitGen) {
      updatedConstraint =>
        updatedConstraint.isEmpty shouldBe true
