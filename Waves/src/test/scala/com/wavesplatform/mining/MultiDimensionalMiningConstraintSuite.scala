...
class MultiDimensionalMiningConstraintSuite
  ...
  "MultiDimensionalMiningConstraint" - {
    "isEmpty" - {
      val emptyConstraintGen: Gen[MultiDimensionalMiningConstraint] = for {
        ...
        if isLeft || isRight
        leftMaxSize <- if (isLeft) Gen.const(0)
        else Gen.chooseNum(1, Int.MaxValue)
        rightMaxSize <- if (isRight) Gen.const(0)
        else Gen.chooseNum(1, Int.MaxValue)
      } yield
        MultiDimensionalMiningConstraint(createConstConstraint(leftMaxSize, 1),
                                         createConstConstraint(rightMaxSize, 1))

      "should be true if one dimension is empty" in forAll(emptyConstraintGen) {
        constraint =>
      ...
      "should be false is both of two dimensions are non-empty" in forAll(
        nonEmptyConstraintGen) { constraint =>
        constraint.isEmpty shouldBe false
        constraint.isOverfilled shouldBe false
      }
    }

    "put(transaction)" - tests(createConstConstraint(_, transactionSize = 1)) {
      (initConstraint, txs) =>
  ...
  private def tests(estimator: Int => MiningConstraint)(
      fold: (MultiDimensionalMiningConstraint,
             Seq[Transaction]) => MultiDimensionalMiningConstraint): Unit = {
    "should return None if the operation is unsuccessful for one of dimensions" - {
      val noOverfillGen: Gen[MultiDimensionalMiningConstraint] = for {
        ...
        val constraint =
          MultiDimensionalMiningConstraint(estimator(commonLimit),
                                           estimator(commonLimit))
        fold(constraint, txs)
      }
      ...
      val firstOverfillsGen: Gen[MultiDimensionalMiningConstraint] = for {
        ...
        val constraint =
          MultiDimensionalMiningConstraint(estimator(firstLimit),
                                           estimator(secondLimit))
        fold(constraint, txs)
      }
