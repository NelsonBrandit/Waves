...
import com.wavesplatform.settings.fee.OrderFeeSettings.{
  FixedWavesSettings,
  OrderFeeSettings,
  PercentSettings
}
...
class OrderValidatorSpecification
  ...
  private val accountScript =
    ExprScript(V2, Terms.TRUE, checkSize = false).explicitGet()

  private val defaultPortfolio =
    Portfolio(0, LeaseBalance.empty, Map(wbtc -> 10 * Constants.UnitsInWave))
  ...
  "OrderValidator" should {
    ...
    "reject new order" when {
      "this order had already been accepted" in asa(orderStatus = _ => true) {
        v =>
          v should produce("OrderDuplicate")
      }

      "sender's address is blacklisted" in {
        val blacklistedAccount = PrivateKeyAccount(
          "3irbW78fffj5XDzAMjaEeo3kn8V".getBytes(Charsets.UTF_8))
      ...
      "v1 order from a scripted account" in forAll(accountGen) { scripted =>
        portfolioTest(defaultPortfolio) { (ov, bc) =>
          ...
          (bc.accountScript _)
            .when(scripted.toAddress)
            .returns(Some(ExprScript(Terms.TRUE).explicitGet()))
          (bc.height _).when().returns(50).once()

          ov(newBuyOrder(scripted)) should produce(
            "ScriptedAccountTradingUnsupported")
        }
      }

      "sender's address has a script, but trading from smart accounts hasn't been activated" in forAll(
        accountGen) { scripted =>
        portfolioTest(defaultPortfolio) { (ov, bc) =>
          activate(bc, BlockchainFeatures.SmartAccountTrading -> 100)
          (bc.accountScript _)
      ...
      "sender's address has a script returning FALSE" in forAll(accountGen) {
        scripted =>
          portfolioTest(defaultPortfolio) { (ov, bc) =>
            activate(bc, BlockchainFeatures.SmartAccountTrading -> 100)
            (bc.accountScript _)
              .when(scripted.toAddress)
              .returns(Some(ExprScript(Terms.FALSE).explicitGet()))
            (bc.height _).when().returns(150).anyNumberOfTimes()

            ov(newBuyOrder(scripted, version = 2)) should produce(
              "AccountScriptDeniedOrder")
          }
      }

      "order expires too soon" in forAll(
        Gen.choose[Long](1, OrderValidator.MinExpiration),
        accountGen) { (offset, pk) =>
        val tt = new TestTime
        val unsigned = newBuyOrder
        val signed = Order.sign(unsigned
                                  .updateExpiration(tt.getTimestamp() + offset)
                                  .updateSender(pk),
                                pk)
      ...
      "amount is invalid" in {
        ...
        OrderValidator.timeAware(ntpTime)(signed) should produce(
          "amount should be > 0")
      }

      "order signature is invalid" in portfolioTest(defaultPortfolio) {
        (ov, bc) =>
          val pk = PrivateKeyAccount(randomBytes())
          (bc.accountScript _).when(pk.toAddress).returns(None)
          val order = newBuyOrder(pk) match {
            case x: OrderV1 =>
              x.copy(proofs = Proofs(Seq(ByteStr(Array.emptyByteArray))))
            case x: OrderV2 =>
              x.copy(proofs = Proofs(Seq(ByteStr(Array.emptyByteArray))))
          }
      ...
      "order exists" in {
        val pk = PrivateKeyAccount(randomBytes())
        val ov = OrderValidator.accountStateAware(pk,
                                                  defaultPortfolio.balanceOf,
                                                  1,
                                                  _ => true)(_)
        ov(newBuyOrder(pk, 1000)) should produce("OrderDuplicate")
      }

      "order price has invalid non-zero trailing decimals" in forAll(
        assetIdGen(1),
        accountGen,
        Gen.choose(1, 7)) {
        case (amountAsset, sender, amountDecimals) =>
          portfolioTest(
            Portfolio(11 * Constants.UnitsInWave,
                      LeaseBalance.empty,
                      Map.empty)) { (ov, bc) =>
            (bc.hasScript _).when(sender.toAddress).returns(false)
            (bc.assetDescription _)
              .when(amountAsset)
              .returns(mkAssetDescription(amountDecimals))
      ...
      "matcher's fee asset in order is blacklisted" in forAll(
        orderV3WithArbitraryFeeAssetGenerator) { order =>
        val orderValidator =
          OrderValidator
            .matcherSettingsAware(
              order.matcherPublicKey,
              Set.empty,
              order.matcherFeeAssetId.fold(Set.empty[IssuedAsset])(
                Set[IssuedAsset](_)),
      ...
      "matcher's fee asset in order doesn't meet matcher's settings requirements (fixed-waves mode and incorrect asset)" in {
        val preconditions = for {
          ...
          fixedWavesSettings <- Gen.const(
            FixedWavesSettings(order.matcherFee - 1000L))
      ...
      "matcher's fee in order is too small (percent mode)" in {
        ...
        def updateOrder(order: Order,
          ...
          order
            .updateFee(
              OrderValidator.getMinValidFee(order, percentSettings) + offset) // incorrect fee (less than minimal admissible by offset)
            .updateMatcherFeeAssetId(OrderValidator.getValidFeeAsset(
              order,
              percentSettings.assetType)) // but correct asset
        }

        val preconditions =
          for {
            percentFeeSettings <- percentSettingsGenerator
            order <- orderV3Generator map (ord =>
              updateOrder(ord, percentFeeSettings, -100L))
        ...
        val preconditions =
          for {
            ...
            fixedFeeSettings <- fixedSettingsGenerator(
              ...
              upperMinFeeBound =
                Math.min(order.matcherFee * 2, Long.MaxValue - 1)
            )
          } yield (order, fixedFeeSettings)
      ...
      "matcher's fee in order is too small (fixed-waves mode)" in {
        ...
        val preconditions =
          for {
            ...
            fixedWavesFeeSettings <- Gen.const(
              FixedWavesSettings(order.matcherFee + 1000L))
        ...
        forAll(orderWithMatcherSettingsGenerator) {
          case (order, sender, orderFeeSettings) =>
            ...
            activate(blockchain,
                     ...
                     BlockchainFeatures.OrderV3 -> 0)

            val transactionCreator =
              exchangeTransactionCreator(blockchain).createTransaction _
            ...
            val minFee =
              ExchangeTransactionCreator.minFee(blockchain,
                                                order.matcherPublicKey,
                                                order.assetPair,
                                                baseFee)
            val correctedOrder =
              Order.sign(order.updateFee(minFee - 1000L), sender)
              ...
              assets.foreach {
                case asset @ IssuedAsset(_) =>
                  (blockchain.assetDescription _)
                    .when(asset)
                    .returns(mkAssetDescription(8))
                  (blockchain.assetScript _).when(asset).returns(None)
                case _ =>
              }
            }

            setAssetsDescriptionAndEmptyScript(order.assetPair.amountAsset,
            ...
            orderFeeSettings match {
              case _: FixedWavesSettings =>
                orderValidator(correctedOrder) should produce("FeeNotEnough")
      ...
      "matcher's fee in order in insufficient in case of scripted account or asset" in {
        forAll(orderWithoutWavesInPairAndWithMatcherSettingsGenerator) {
          case (order, _, orderFeeSettings) =>
            ...
            val setScriptAndValidateWithSettings =
              setScriptsAndValidate(orderFeeSettings) _

            orderFeeSettings match {
              case _: FixedWavesSettings =>
                setScriptAndValidateWithSettings(Some(trueScript), None, None)(
                  order) should produce("FeeNotEnough")
                setScriptAndValidateWithSettings(None, Some(trueScript), None)(
                  order) should produce("FeeNotEnough")
                setScriptAndValidateWithSettings(None, None, Some(trueScript))(
                  order) should produce("FeeNotEnough")
    ...
    "validate order with any number of signatures from a scripted account" in forAll(
      Gen.choose(0, 5)) { proofsNumber =>
      validateOrderProofsTest(
        (1 to proofsNumber).map(x => ByteStr(Array(x.toByte))))
    }

    "meaningful error for undefined functions in matcher" in portfolioTest(
      defaultPortfolio) { (ov, bc) =>
      activate(bc, BlockchainFeatures.SmartAccountTrading -> 0)

      val pk = PrivateKeyAccount(randomBytes())
      val o = newBuyOrder(pk, version = 2)
      val script = ScriptCompiler("true && (height > 0)", isAssetScript = false)
        .explicitGet()
        ._1
    ...
    "validate order with smart token" when {
      ...
      def test(
          f: (Order => OrderValidator.Result[Order], Blockchain, Order) => Any)
        : Unit = (1 to 2).foreach { version =>
    ...
    "deny OrderV2 if SmartAccountTrading hasn't been activated yet" in forAll(
      accountGen) { account =>
      portfolioTest(defaultPortfolio) { (ov, bc) =>
        activate(bc, BlockchainFeatures.SmartAccountTrading -> 100)
        (bc.accountScript _)
          .when(account.toAddress)
          .returns(Some(accountScript))
          .anyNumberOfTimes()
        (bc.height _).when().returns(0).anyNumberOfTimes()

        ov(newBuyOrder(account, version = 2)) should produce(
          "OrderVersionUnsupported")
      }
    }

    "deny blockchain functions in account script" in forAll(accountGen) {
      account =>
        portfolioTest(defaultPortfolio) { (ov, bc) =>
          activate(bc, BlockchainFeatures.SmartAccountTrading -> 0)
          (bc.height _).when().returns(0).anyNumberOfTimes()
  ...
  "sunny day test when order meets matcher's settings requirements" in forAll(
    orderWithMatcherSettingsGenerator) {
    case (order, _, orderFeeSettings) =>
      val orderValidator =
        OrderValidator
          .matcherSettingsAware(order.matcherPublicKey,
  ...
  private def portfolioTest(p: Portfolio)(
      f: (Order => OrderValidator.Result[Order], Blockchain) => Any): Unit = {
  ...
  private def newBuyOrder(pk: PrivateKeyAccount,
                          ts: Long = 0,
                          version: Byte = 1) =
    buy(
      pair = pairWavesBtc,
  ...
  private def activate(bc: Blockchain,
                       features: (BlockchainFeature, Int)*): Unit = {
    (bc.activatedFeatures _)
      .when()
      .returns(features.map(x => x._1.id -> x._2).toMap)
      .anyNumberOfTimes()
  }
  ...
  private def tradableBalance(p: Portfolio)(assetId: Asset): Long =
    assetId.fold(p.spendableBalance)(p.assets.getOrElse(_, 0L))

  private def exchangeTransactionCreator(blockchain: Blockchain) =
    new ExchangeTransactionCreator(blockchain,
                                   MatcherAccount,
  ...
  private def asa[A](
    ...
    f(
      OrderValidator
        .accountStateAware(o.sender, tradableBalance(p), 0, orderStatus)(o))
  ...
  private def setScriptsAndValidate(orderFeeSettings: OrderFeeSettings)(
      ...
      scriptForAccount: Option[Script])(
      order: Order): OrderValidator.Result[Order] = {
    val blockchain = stub[Blockchain]
        ...
        case (assetOption, scriptOption) =>
          assetOption match {
            case asset: IssuedAsset =>
              ...
              (blockchain.hasAssetScript _)
                .when(asset)
                .returns(scriptOption.isDefined)
            case _ =>
          }
      }
    }
    ...
    (blockchain.accountScript _)
      ...
      .returns(scriptForAccount)
    (blockchain.hasScript _)
      .when(MatcherAccount.toAddress)
      .returns(scriptForAccount.isDefined)
