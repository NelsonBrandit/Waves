...
class OrderHistoryBalanceSpecification
  ...
  def openVolume(address: Address, asset: Asset): Long =
    oh.ref(address).openVolume(asset)

  def activeOrderIds(sender: Address): Seq[ByteStr] =
    oh.ref(sender).activeOrderIds
  def allOrderIds(sender: Address): Seq[ByteStr] = oh.ref(sender).allOrderIds
  def activeOrderIdsByPair(sender: Address, pair: AssetPair): Seq[ByteStr] =
    oh.ref(sender).activeOrderIdsByPair(pair)
  def allOrderIdsByPair(sender: Address, pair: AssetPair): Seq[ByteStr] =
    oh.ref(sender).allOrderIdsByPair(pair)
  ...
  property(
    "Should not reserve fee, if seller receives more WAVES than total fee in sell order") {
  ...
  property(
    "Should not reserve fee, if buyer receives more WAVES than total fee in buy order") {
  ...
  property("Two sell orders added") {
    ...
    val ord1 = sell(WavesBtc,
                    10000,
                    0.0005,
                    Some(pk),
                    matcherFee = Some(30000L),
                    ts = Some(System.currentTimeMillis()))
    val ord2 = sell(WavesBtc,
                    16000,
                    0.0008,
                    Some(pk),
                    matcherFee = Some(30000L),
                    ts = Some(System.currentTimeMillis() + 1))
  ...
  property("Buy WAVES order filled exactly") {
    ...
    withClue("executed exactly") {
      ...
      orderStatus(submitted.id()) shouldBe OrderStatus.Filled(
        exec.executedAmount)
    }

    withClue(
      s"has no reserved assets, counter.senderPublicKey: ${counter.senderPublicKey}, counter.order.id=${counter
        .idStr()}") {
    ...
    withClue(
      s"has no reserved assets, submitted.senderPublicKey: ${submitted.senderPublicKey}, submitted.order.id=${submitted
        .idStr()}") {
    ...
    withClue(s"orders list of counter owner ${counter.sender}") {
      ...
      allOrderIdsByPair(counter.senderPublicKey, WavesBtc) shouldBe Seq(
        counter.id())
    }
    ...
    withClue(s"orders list of submitted owner ${submitted.sender}") {
      ...
      allOrderIdsByPair(submitted.senderPublicKey, WavesBtc) shouldBe Seq(
        submitted.id())
    }
  }
  ...
  property("Buy WAVES order filled with remainder") {
    val counter =
      sell(WavesBtc, 840340L, 0.00000238, matcherFee = Some(300000L))
    val submitted =
      buy(WavesBtc, 425532L, 0.00000238, matcherFee = Some(300000L))

    val counterLo = LimitOrder(counter)
    oh.process(OrderAdded(counterLo))
    withClue(
      s"account checks, counter.senderPublicKey: ${counter.senderPublicKey}, counter.order.id=${counter
        .idStr()}") {
      openVolume(counter.senderPublicKey, WavesBtc.priceAsset) shouldBe 0L
    ...
    withClue(s"counter.order.id=${submitted.idStr()}") {
      ...
      orderStatus(counter.id()) shouldBe OrderStatus.PartiallyFilled(
        exec.executedAmount)
    }
    ...
    withClue(
      s"account checks, submitted.senderPublicKey: ${submitted.senderPublicKey}, submitted.order.id=${submitted
        .idStr()}") {
      openVolume(submitted.senderPublicKey, WavesBtc.amountAsset) shouldBe 0L
      openVolume(submitted.senderPublicKey, WavesBtc.priceAsset) shouldBe 0L
    }
  ...
  property("Sell WAVES order - filled, buy order - partial") {
    ...
    withClue(s"submitted: ${submitted.idStr()}") {
      ...
      orderStatus(submitted.id()) shouldBe OrderStatus.PartiallyFilled(
        100000000)
    }
...
//                 OrderInfo.safeSum(LimitOrder.getPartialFee(submitted.matcherFee, submitted.amount, submitted.amount - counter.amount), -20000000L))
      openVolume(submitted.senderPublicKey, WavesBtc.priceAsset) shouldBe (BigDecimal(
        0.00085) * 20000000L).toLong
    }
  ...
  property("Buy WAVES order - filled with 2 steps, sell order - partial") {
    ...
    val submitted1 =
      sell(WavesBtc, 50000000, 0.00075, matcherFee = Some(300001L))
    val submitted2 =
      sell(WavesBtc, 80000000, 0.0008, matcherFee = Some(300001L))
    ...
    withClue(
      s"account checks, ord3.senderPublicKey: ${submitted2.senderPublicKey}, ord3.order.id=${submitted2
        .idStr()}") {
  ...
  property("WCT/BTC: sell - filled partially, buy - filled") {
    ...
    withClue(
      ...
      openVolume(counter.senderPublicKey, Waves) shouldBe counter.matcherFee - LimitOrder
        .partialFee(counter.matcherFee, counter.amount, exec.executedAmount)
    }
  ...
  property("Sell ETH twice (filled, partial), buy WAVES order - filled") {
    ...
    oh.processAll(OrderAdded(LimitOrder(counter1)),
                  OrderAdded(LimitOrder(counter2)))
  ...
  property("Total execution of two counter orders and the one submitted") {
    ...
    val exec1 = OrderExecuted(LimitOrder(submitted), LimitOrder(counter1))
    oh.processAll(exec1,
                  OrderAdded(exec1.submittedRemaining),
                  OrderExecuted(exec1.submittedRemaining, LimitOrder(counter2)))
  ...
  property("Cancel buy order") {
    ...
    oh.processAll(OrderAdded(LimitOrder(ord1)),
                  OrderCanceled(LimitOrder(ord1), unmatchable = false))
  ...
  property("Cancel partially executed order") {
    ...
    val submitted =
      buy(WavesBtc, 1000000000, 0.00081, matcherFee = Some(300000L))

    oh.process(OrderAdded(LimitOrder(counter)))
    val exec1 = OrderExecuted(LimitOrder(submitted), LimitOrder(counter))
    oh.processAll(
      exec1,
      OrderCanceled(exec1.counter.partial(exec1.counterRemainingAmount,
                                          exec1.counterRemainingFee),
                    unmatchable = false))
  ...
  property("Sorting by status then timestamp") {
    val pk = PrivateKeyAccount("private".getBytes("utf-8"))
    val ord1 = buy(WavesBtc,
                   110000000,
                   0.0008,
                   Some(pk),
                   Some(300000L),
                   Some(1L)) // Filled
    val ord2 = buy(WavesBtc,
                   120000000,
                   0.0006,
                   Some(pk),
                   Some(300000L),
                   Some(2L)) // Accepted
    val ord3 = buy(WavesBtc,
                   130000000,
                   0.0005,
                   Some(pk),
                   Some(300000L),
                   Some(3L)) // Canceled
    val ord4 = sell(WavesBtc,
                    2100000000,
                    0.00079,
                    Some(pk),
                    Some(300000L),
                    Some(4L)) // Partial
    val ord5 = buy(WavesBtc,
                   130000000,
                   0.0004,
                   Some(pk),
                   Some(300000L),
  ...
  property("History with more than max limit") {
    ...
    val newOrder =
      buy(WavesBtc, 100000000, 0.001, Some(pk), Some(300000L), Some(1L))
  ...
  property(
    "History by pair - added orders more than history by pair limit (200 active)") {
    val pk = PrivateKeyAccount("private".getBytes("utf-8"))
  ...
  property(
    "History by pair - added and canceled orders both more than history by pair limit (200 active, 10 canceled)") {
    val pk = PrivateKeyAccount("private".getBytes("utf-8"))
    ...
    ordersToCancel.foreach(o =>
      oh.process(OrderCanceled(LimitOrder(o), unmatchable = false)))
  ...
  property("History by pair contains more elements than in common") {
    ...
    withClue("after 2 step") {
      ...
      withClue("common") {
        val allIds = allOrderIds(pk)
        val expectedIds = pair2Orders.map(_.id()).reverse ++ pair1Orders
          .map(_.id())
          .reverse
          .take(MaxElements - pair2Orders.size)
        allIds shouldBe expectedIds
      }
...
private object OrderHistoryBalanceSpecification {
  ...
  private implicit class AddressActorExt(val ref: ActorRef) extends AnyVal {
    def orderIds(assetPair: Option[AssetPair],
                 activeOnly: Boolean): Seq[Order.Id] =
      askAddressActor[Seq[(ByteStr, OrderInfo[OrderStatus])]](
        ref,
        AddressActor.GetOrders(assetPair, activeOnly)).map(_._1)

    def activeOrderIds: Seq[Order.Id] = orderIds(None, true)

    def allOrderIds: Seq[Order.Id] = orderIds(None, false)

    def activeOrderIdsByPair(pair: AssetPair): Seq[Order.Id] =
      orderIds(Some(pair), true)

    def allOrderIdsByPair(pair: AssetPair): Seq[Order.Id] =
      orderIds(Some(pair), false)

    def openVolume(asset: Asset): Long =
      askAddressActor[Map[Asset, Long]](ref, AddressActor.GetReservedBalance)
        .getOrElse(asset, 0L)
  ...
  private implicit class OrderExecutedExt(val oe: OrderExecuted.type)
      extends AnyVal {
    def apply(submitted: LimitOrder, counter: LimitOrder): OrderExecuted =
      OrderExecuted(submitted, counter, submitted.order.timestamp)
  }
}
