...
class ExchangeTransactionCreatorSpecification
  ...
  "ExchangeTransactionCreator" when {
    "SmartAccountTrading hasn't been activated yet" should {
      "create an ExchangeTransactionV1" in {
        ...
        val tc = new ExchangeTransactionCreator(bc,
                                                MatcherAccount,
                                                matcherSettings.orderFee)
        tc.createTransaction(LimitOrder(submitted),
                             LimitOrder(counter),
        ...
        List((1, 2), (2, 1), (2, 2)).foreach {
          case (buyVersion, sellVersion) =>
            s"buyV$buyVersion and sellV$sellVersion" in {
              val counter = buy(pair,
                                100000,
                                0.0008,
                                matcherFee = Some(2000L),
                                version = buyVersion.toByte)
              val submitted = sell(pair,
                                   100000,
                                   0.0007,
                                   matcherFee = Some(1000L),
                                   version = sellVersion.toByte)
              ...
              tc.createTransaction(LimitOrder(submitted),
                                   LimitOrder(counter),
                                   System.currentTimeMillis()) should produce(
    ...
    "SmartAccountTrading has been activated" should {
      "create an ExchangeTransactionV2" in {
        val counter =
          buy(pair, 100000, 0.0008, matcherFee = Some(2000L), version = 2)
        val submitted =
          sell(pair, 100000, 0.0007, matcherFee = Some(1000L), version = 2)

        val bc = stub[Blockchain]
        (bc.activatedFeatures _)
          .when()
          .returns(Map(BlockchainFeatures.SmartAccountTrading.id -> 0))
          .anyNumberOfTimes()
        val tc = new ExchangeTransactionCreator(bc,
                                                MatcherAccount,
                                                matcherSettings.orderFee)
  ...
  "ExchangeTransactionCreator" should {
    ...
    (bc.activatedFeatures _)
      .when()
      .returns(Map(BlockchainFeatures.OrderV3.id -> 0,
    ...
    "calculate fees in exchange transaction which are equal to matcher fees in fully matched orders" in {
      ...
      val preconditions =
        for {
          ...
          orderSettings <- Gen.oneOf(
            percentSettingsGenerator,
            fixedSettingsGenerator(buyOrder.matcherFeeAssetId),
      ...
      forAll(preconditions) {
        case (buyOrder, sellOrder, orderSettings) =>
          val tc =
            new ExchangeTransactionCreator(bc, MatcherAccount, orderSettings)
          val tx = tc
            .createTransaction(LimitOrder(buyOrder),
    ...
    "create valid exchange transaction when orders are matched partially" in {
      ...
      val preconditions =
        for {
          ...
          val sellOrderWithUpdatedAmount =
            sellOrder.updateAmount(sellOrder.amount / 2)
          val newSignature =
            crypto.sign(senderSell, sellOrderWithUpdatedAmount.bodyBytes())
          val correctedSellOrder = sellOrderWithUpdatedAmount.updateProofs(
            Proofs(Seq(ByteStr(newSignature))))
      ...
      forAll(preconditions) {
        case (buyOrder, sellOrder, orderSettings) =>
          val tc =
            new ExchangeTransactionCreator(bc, MatcherAccount, orderSettings)
          val tx = tc.createTransaction(LimitOrder(buyOrder),
                                        LimitOrder(sellOrder),
                                        System.currentTimeMillis)
