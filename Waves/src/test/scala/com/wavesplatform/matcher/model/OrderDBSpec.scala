...
class OrderDBSpec
  ...
  private def finalizedOrderInfoGen(
      o: Order): Gen[(Order, OrderInfo[OrderStatus.Final])] =
    for {
      filledAmount <- Gen.choose(0, o.amount)
      status <- Gen.oneOf(OrderStatus.Filled(o.amount),
                          OrderStatus.Cancelled(filledAmount))
    } yield o -> o.toInfo(status)

  private def finalizedOrderSeqGen(
      orderCount: Int): Gen[(PrivateKeyAccount,
                             AssetPair,
                             Seq[(Order, OrderInfo[OrderStatus.Final])])] =
    for {
      sender <- accountGen
      pair <- distinctPairGen
      orderList <- Gen.listOfN(
        orderCount,
        orderGenerator(sender, pair).flatMap(o => finalizedOrderInfoGen(o)))
    } yield (sender, pair, orderList)

  private val finalizedOrderInfoGen
    : Gen[(Order, OrderInfo[OrderStatus.Final])] = for {
    (o, _) <- orderGenerator
  ...
  "Default OrderDB implementation" - {
    ...
    "does not overwrite finalized info" in test { odb =>
      val dualFinalizedOrderInfoGen: Gen[(Order,
                                          OrderInfo[OrderStatus.Final],
                                          OrderInfo[OrderStatus.Final])] = for {
        (o, _) <- orderGenerator
        filledAmount <- Gen.choose(0, o.amount)
        s1 <- Gen.oneOf(OrderStatus.Filled(o.amount),
                        OrderStatus.Cancelled(filledAmount))
        s2 <- Gen.oneOf(OrderStatus.Filled(o.amount),
                        OrderStatus.Cancelled(filledAmount))
      } yield
        (
          o,
          OrderInfo(o.orderType,
                    o.amount,
                    o.price,
                    o.timestamp,
                    s1,
    ...
    "loads remaining orders when limits are not exceeded" in test { odb =>
      forAll(finalizedOrderSeqGen(20)) {
        case (sender, pair, orders) =>
          ...
          tuples should contain allElementsOf orders.map {
            case (o, i) => o.id() -> i
          }
      }
    }
    ...
    "does not load more orders when there are too many active orders" in {
      ...
      val paramGen = for {
        ...
        (sender,
         pair,
         activeOrders.map(o => o.id() -> o.toInfo(OrderStatus.Accepted)),
         finalizedOrders)
