...
class OrderBookSpec
  ...
  "place buy orders with different prices" in {
    ...
    ob.allOrders.toSeq shouldEqual Seq(LimitOrder(ord2),
                                       LimitOrder(ord1),
                                       LimitOrder(ord3))
  }
  ...
  "sell market" in {
    ...
    ob.allOrders shouldEqual Seq(
      BuyLimitOrder(ord2.amount, ord2.matcherFee, ord2),
      BuyLimitOrder(ord1.amount, ord1.matcherFee, ord1))
  ...
  "execute orders at different price levels" in {
    ...
    ob.allOrders shouldEqual Seq(
      SellLimitOrder(
        restAmount,
        ord3.matcherFee - LimitOrder.partialFee(ord3.matcherFee,
                                                ord3.amount,
                                                ord3.amount - restAmount),
        ord3
      ))
  }
  ...
  "partially execute order with small remaining part" in {
    ...
    ob.allOrders shouldEqual Seq(
      SellLimitOrder(ord1.amount, ord1.matcherFee, ord1))
  }
  ...
  "partially execute order with zero fee remaining part" in {
    ...
    val restFee = ord1.matcherFee - LimitOrder.partialFee(ord1.matcherFee,
                                                          ord1.amount,
                                                          corrected2)
    ob.allOrders.toSeq shouldEqual Seq(
      SellLimitOrder(restAmount, restFee, ord1))
  }

  "partially execute order with price > 1 and zero fee remaining part " in {
    val pair = AssetPair(IssuedAsset(ByteStr("BTC".getBytes)),
                         IssuedAsset(ByteStr("USD".getBytes)))
    ...
    val restFee = ord1.matcherFee - LimitOrder.partialFee(
      ord1.matcherFee,
      ord1.amount,
      ord3.amount - ord2.amount)
    ob.allOrders.toSeq shouldEqual Seq(
      SellLimitOrder(restAmount, restFee, ord1))
  }

  "buy small amount of pricey asset" in {
    val p = AssetPair(IssuedAsset(ByteStr("WAVES".getBytes)),
                      IssuedAsset(ByteStr("USD".getBytes)))
    ...
    val restFee = s.matcherFee - LimitOrder.partialFee(s.matcherFee,
                                                       s.amount,
                                                       restSAmount)
