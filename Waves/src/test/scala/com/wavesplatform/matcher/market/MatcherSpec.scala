...
trait MatcherSpecLike
  ...
  override protected def beforeEach(): Unit = {
    val p = TestProbe()
    p.send(StorageExtension(system).journalStorage,
           InMemoryJournalStorage.ClearJournal)
    p.expectMsg(akka.actor.Status.Success(""))
    p.send(StorageExtension(system).snapshotStorage,
