...
import com.wavesplatform.matcher.market.MatcherActorSpecification.{
  FailAtStartActor,
  NothingDoActor,
  RecoveringActor
}
...
class MatcherActorSpecification
  ...
  (blockchain.assetDescription _)
    ...
    .returns(
      Some(
        AssetDescription(PrivateKeyAccount(Array.empty),
                         "Unknown".getBytes,
  ...
  "MatcherActor" should {
    ...
    // snapshotOffset == 17
    val pair23 = AssetPair(
      IssuedAsset(ByteStr(Array(1))),
      IssuedAsset(ByteStr(Array(2)))) // key = 2-3, snapshot offset = 9: 9, 26, 43, ...
    val pair45 = AssetPair(
      IssuedAsset(ByteStr(Array(3))),
      IssuedAsset(ByteStr(Array(4)))) // key = 4-5, snapshot offset = 12: 12, 29, 46, ...
    ...
    "force an order book to create a snapshot" when {
      "it didn't do snapshots for a long time" when {
        ...
        "multiple order books" in snapshotTest(pair23, pair45) {
          (matcherActor, probes) =>
      ...
      "received a lot of messages and tries to maintain a snapshot's offset" in snapshotTest(
        pair23) { (matcherActor, probes) =>
        val probe = probes.head
  ...
  private def sendBuyOrders(actor: ActorRef,
                            assetPair: AssetPair,
                            indexes: Range): Unit = {
    val ts = System.currentTimeMillis()
    indexes.foreach { i =>
      actor ! wrap(i,
                   buy(assetPair, amount = 1000, price = 1, ts = Some(ts + i)))
    }
  }
  ...
  /**
    ...
    */
  private def snapshotTest(assetPairs: AssetPair*)(
      f: (ActorRef, List[TestProbe]) => Any): Any = {
          ...
          (assetPair, _) => {
            ...
            if (idx < 0)
              throw new RuntimeException(
                s"Can't find $assetPair in $assetPairs")
            r(idx)._1
          },
          blockchain.assetDescription
        )
  ...
  private def defaultActor(
      ob: AtomicReference[Map[AssetPair, Either[Unit, ActorRef]]] =
        emptyOrderBookRefs): TestActorRef[MatcherActor] = {
    val txFactory = new ExchangeTransactionCreator(
      EmptyBlockchain,
      MatcherAccount,
      matcherSettings.orderFee).createTransaction _
  ...
  private def waitInitialization(
      x: TestActorRef[MatcherActor]): TestActorRef[MatcherActor] =
  ...
  private def provideSnapshot(pairs: AssetPair*): Unit = {
    val snapshot = SerializationExtension(system)
      .serialize(Snapshot(MatcherActor.Snapshot(pairs.toSet)))
      .get
    val p = TestProbe()
    p.send(
  ...
  private def doNothingOnRecovery(
      x: Either[String, (ActorRef, QueueEventWithMeta.Offset)]): Unit = {}

  private def emptyOrderBookRefs =
    new AtomicReference(Map.empty[AssetPair, Either[Unit, ActorRef]])
  private def randomAssetId: Asset = IssuedAsset(ByteStr(randomBytes()))
}

object MatcherActorSpecification {
  private class NothingDoActor extends Actor {
    override def receive: Receive = Actor.ignoringBehavior
  }
  private class RecoveringActor(owner: ActorRef, assetPair: AssetPair)
      extends NothingDoActor {
