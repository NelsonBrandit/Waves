...
class OrderBookActorSpecification
  ...
  private val txFactory = new ExchangeTransactionCreator(
    EmptyBlockchain,
    MatcherAccount,
    matcherSettings.orderFee).createTransaction _
  private val obc =
    new ConcurrentHashMap[AssetPair, OrderBook.AggregatedSnapshot]
  private val md = new ConcurrentHashMap[AssetPair, MarketStatus]

  private def update(ap: AssetPair)(
      snapshot: OrderBook.AggregatedSnapshot): Unit = obc.put(ap, snapshot)
  ...
  "OrderBookActor" should {
    ...
    "place buy and sell order to the order book and preserve it after restart" in obcTest {
      (pair, orderBook, tp) =>
        ...
        tp.receiveN(2) shouldEqual Seq(ord2, ord1).map(o =>
          OrderAdded(LimitOrder(o)))
    }

    "execute partial market orders and preserve remaining after restart" in obcTest {
      (pair, actor, tp) =>
        val ord1 = buy(pair, 10 * Order.PriceConstant, 100)
        ...
        tp.expectMsg(
          OrderAdded(
            SellLimitOrder(
              ...
              ord2.matcherFee - LimitOrder
                .partialFee(ord2.matcherFee, ord2.amount, ord1.amount),
              ord2
            )))
    }

    "execute one order fully and other partially and restore after restart" in obcTest {
      (pair, actor, tp) =>
        val ord1 = buy(pair, 10 * Order.PriceConstant, 100)
        ...
        tp.expectMsg(
          OrderAdded(
            BuyLimitOrder(
              restAmount,
              ord2.matcherFee - LimitOrder.partialFee(ord2.matcherFee,
                                                      ord2.amount,
                                                      ord2.amount - restAmount),
              ord2
            )))
    }

    "match multiple best orders at once and restore after restart" in obcTest {
      (pair, actor, tp) =>
    ...
    "place orders and restart without waiting for response" in obcTest {
      (pair, actor, tp) =>
        val ord1 = sell(pair, 10 * Order.PriceConstant, 100)
    ...
    "don't do a snapshot if there is no changes" in obcTest {
      (pair, actor, tp) =>
        (1 to 10).foreach { i =>
          actor ! wrap(i, buy(pair, 100000000, 0.00041))
        }
