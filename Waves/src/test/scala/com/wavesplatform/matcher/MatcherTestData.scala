...
trait MatcherTestData extends NTPTime { _: Suite =>
  ...
  val bytes32gen: Gen[Array[Byte]] =
    Gen.listOfN(signatureSize, Arbitrary.arbitrary[Byte]).map(xs => xs.toArray)
  val WalletSeed = ByteStr("Matcher".getBytes())
  val MatcherSeed: Array[Byte] =
    crypto.secureHash(Bytes.concat(Ints.toByteArray(0), WalletSeed.arr))
  ...
  def wrap(n: Long, x: Order): QueueEventWithMeta =
    wrap(n, QueueEvent.Placed(x))
  private def wrap(n: Long, event: QueueEvent): QueueEventWithMeta =
    QueueEventWithMeta(n, System.currentTimeMillis(), event)
  ...
  protected def mkAssetId(prefix: String): IssuedAsset = {
    ...
    IssuedAsset(
      ByteStr(
        (prefixBytes ++ Array.fill[Byte](32 - prefixBytes.length)(0.toByte))
          .take(32)))
  }

  val assetPairGen = Gen.frequency((18, distinctPairGen),
                                   (1, assetIdGen(1).map(AssetPair(_, Waves))),
                                   (1, assetIdGen(2).map(AssetPair(Waves, _))))

  val maxTimeGen: Gen[Long] =
    Gen.choose(10000L, Order.MaxLiveTime).map(_ + System.currentTimeMillis())
  val createdTimeGen: Gen[Long] =
    Gen.choose(0L, 10000L).map(System.currentTimeMillis() - _)

  val config = loadConfig(
    ConfigFactory.parseString("""waves {
  ...
  def buy(pair: AssetPair,
    ...
    rawBuy(pair,
           amount,
           (price * Order.PriceConstant).toLong,
           sender,
           matcherFee,
           ts,
           version)
  ...
  def rawBuy(pair: AssetPair,
             ...
             version: Byte = 1): Order =
    valueFromGen(
      buyGenerator(pair, amount, price, sender, matcherFee, version, ts))._1
  ...
  def rawSell(pair: AssetPair,
              ...
              version: Byte = 1): Order =
    valueFromGen(
      sellGenerator(pair, amount, price, sender, matcherFee, ts, version))._1

  val orderTypeGenerator: Gen[OrderType] =
    Gen.oneOf(OrderType.BUY, OrderType.SELL)
  ...
  def orderGenerator(sender: PrivateKeyAccount, pair: AssetPair): Gen[Order] =
    for {
      ...
      matcherFeeAssetId <- Gen.oneOf(pair.amountAsset,
                                     pair.priceAsset,
                                     Waves,
                                     arbitraryAsset)
    } yield {
  ...
  def orderV3WithFeeInPredefinedAssetGenerator(
      asset: Option[Asset] = None): Gen[Order] = {
    for {
      sender: PrivateKeyAccount <- accountGen
      pair <- assetPairGen
      orderType <- orderTypeGenerator
      ...
      OrderV3(sender,
              ...
              expiration,
              matcherFee,
              asset getOrElse arbitraryAsset)
    }
  }

  def orderV3PairGenerator
    : Gen[((PrivateKeyAccount, Order), (PrivateKeyAccount, Order))] =
    for {
  ...
  def fixedSettingsGenerator(
      defaultAsset: Asset,
      lowerMinFeeBound: Long = 1,
      upperMinFeeBound: Long = 1000000L): Gen[FixedSettings] =
    for { minFee <- Gen.choose(lowerMinFeeBound, upperMinFeeBound) } yield {
      FixedSettings(defaultAsset, minFee)
    }

  def fixedWavesSettingsGenerator(
      lowerBaseFeeBound: Long = 1,
      upperBaseFeeBound: Long = 1000000L): Gen[FixedWavesSettings] =
    for { baseFee <- Gen.choose(lowerBaseFeeBound, upperBaseFeeBound) } yield {
      FixedWavesSettings(baseFee)
    }

  val orderWithMatcherSettingsGenerator
    : Gen[(Order, PrivateKeyAccount, OrderFeeSettings)] = {
    for {
      defaultAsset <- assetIdGen(100)
      orderFeeSettings <- Gen.oneOf(percentSettingsGenerator,
                                    fixedSettingsGenerator(defaultAsset),
                                    fixedWavesSettingsGenerator())
      (order, sender) <- orderGenerator
    } yield {
      val correctedOrder =
        correctOrderByFeeSettings(order, sender, orderFeeSettings)
      (correctedOrder, sender, orderFeeSettings)
    }
  }

  val orderWithoutWavesInPairAndWithMatcherSettingsGenerator
    : Gen[(Order, PrivateKeyAccount, OrderFeeSettings)] = {
    for {
      sender: PrivateKeyAccount <- accountGen
  ...
  def correctOrderByFeeSettings(order: Order,
                                sender: PrivateKeyAccount,
                                orderFeeSettings: OrderFeeSettings): Order = {
    val correctedOrder = (order.version, orderFeeSettings) match {
      ...
      case (3, percentSettings: PercentSettings) =>
        order
          .updateMatcherFeeAssetId(
            OrderValidator.getValidFeeAsset(order, percentSettings.assetType))
