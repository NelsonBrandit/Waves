...
class OrderBookSnapshotHttpCacheSpec
    extends FreeSpec
    with Matchers
    with TransactionGenBase
    with NTPTime {

  private val defaultAssetPair =
    AssetPair(Waves, IssuedAsset(ByteStr("asset".getBytes)))
  ...
  "OrderBookSnapshotHttpCache" - {
    ...
    "should not drop the cache if the timeout after an access was not reached" in using(
    ...
    "should drop the cache after timeout" in using(createDefaultCache) {
      cache =>
        def get = cache.get(defaultAssetPair, Some(1))
    ...
    "should clear all depth caches after invalidate" in {
      ...
      using {
        ...
        val prev = depths
          .map(x => x -> orderBookFrom(cache.get(defaultAssetPair, Some(x))))
          .toMap
  ...
  private def createDefaultCache =
    new OrderBookSnapshotHttpCache(
      OrderBookSnapshotHttpCache.Settings(50.millis, List(3, 9)),
      ntpTime,
      _ => None)

  private def orderBookFrom(x: HttpResponse): OrderBookResult =
    JsonSerializer.deserialize[OrderBookResult](
      x.entity
