...
class ReservedBalanceSpecification
  ...
  override protected def actorSystemName: String =
    "ReservedBalanceSpecification" // getClass.getSimpleName
        ...
        address =>
          Props(
            new AddressActor(
              ...
              _ =>
                Future.failed(
                  new IllegalStateException("Should not be used in the test"))
            ))
      )
    ))

  private def openVolume(senderPublicKey: PublicKeyAccount,
                         assetId: Asset): Long =
    Await
      .result(
        (addressDir ? AddressDirectory.Envelope(
          senderPublicKey,
          AddressActor.GetReservedBalance)).mapTo[Map[Asset, Long]],
        Duration.Inf
      )
  ...
  def execute(counter: Order, submitted: Order): OrderExecuted = {
    ...
    val exec = OrderExecuted(LimitOrder(submitted),
                             LimitOrder(counter),
                             submitted.timestamp)
    addressDir ! exec
    exec
  ...
  forAll(
    ...
    (counterType: OrderType,
     counterAmount: Long,
     counterPrice: Long,
     submittedAmount: Long,
     submittedPrice: Long) =>
      property(
        s"Reserves should be 0 when remains are 0: $counterType $counterAmount/$counterPrice:$submittedAmount/$submittedPrice") {
        val counter =
          if (counterType == BUY) rawBuy(pair, counterAmount, counterPrice)
          else rawSell(pair, counterAmount, counterPrice)
        val submitted =
          if (counterType == BUY) rawSell(pair, submittedAmount, submittedPrice)
          else rawBuy(pair, submittedAmount, submittedPrice)
     ...
     submittedPrice: Long) =>
      property(
        ...
        withClue(s"Counter sender should have reserved asset:") {
          val (expectedAmountReserve, expectedPriceReserve) =
            if (counterType == BUY) (0, 1)
            else (p.minAmountFor(counterPrice), 0)
     ...
     submittedPrice: Long) =>
      property(
        ...
        withClue(s"Submitted sender should have reserved asset:") {
          val (expectedAmountReserve, expectedPriceReserve) =
            if (counterType == BUY) (p.minAmountFor(submittedPrice), 0)
            else (0, 1)
     ...
     submittedPrice: Long) =>
      property(
        ...
        withClue(s"Counter sender should have reserved asset:") {
          val (expectedAmountReserve, expectedPriceReserve) =
            if (counterType == BUY) (0, 1)
            else (p.minAmountFor(submittedPrice), 0)
          openVolume(counter.senderPublicKey, pair.amountAsset) shouldBe expectedAmountReserve
          openVolume(counter.senderPublicKey, pair.priceAsset) shouldBe expectedPriceReserve
        }
