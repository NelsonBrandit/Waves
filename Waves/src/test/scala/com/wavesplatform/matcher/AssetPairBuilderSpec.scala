...
class AssetPairBuilderSpec extends FreeSpec with Matchers with MockFactory {
  ...
  private val WUSD = IssuedAsset(
    ByteStr.decodeBase58("HyFJ3rrq5m7FxdkWtQXkZrDat1F7LjVVGfpSkUuEXQHj").get)
  private val WBTC = IssuedAsset(
    ByteStr.decodeBase58("Fmg13HEHJHuZYbtJq8Da8wifJENq8uBxDuWoP9pVe2Qe").get)
  private val WEUR = IssuedAsset(
    ByteStr.decodeBase58("2xnE3EdpqXtFgCP156qt1AbyjpqdZ5jGjWo3CwTawcux").get)
  private val WCNY = IssuedAsset(
    ByteStr.decodeBase58("6pmDivReTLikwYqQtJTv6dTcE59knriaodB3AK8T9cF8").get)
  ...
  private val priceAssets = ConfigFactory.parseString(s"""waves.matcher {
       ...
       |  price-assets = [${predefinedPriceAssets.map(_.id.base58)
                                                           .mkString(",")}]
       |}""".stripMargin)
  ...
  "AssetPairBuilder" - {
    ...
    "rejects a pair when" - {
      "blacklist" - {
        "contains asset id" in {
          (blockchain.assetDescription _)
            .when(Asset3)
            .returns(mkAssetDescription())
          builder.validateAssetPair(AssetPair(Asset3, Waves)) should produce(
            "Invalid Asset ID")
        }
        "matchers asset name" in {
          ...
          (blockchain.assetDescription _)
            .when(Asset2)
            .returns(mkAssetDescription("forbidden Asset name"))
          (blockchain.assetDescription _)
            .when(Asset3)
            .returns(mkAssetDescription("name of an asset"))

          builder.validateAssetPair(AssetPair(Asset3, Asset1)) should produce(
            "Invalid Asset ID")
          builder.validateAssetPair(AssetPair(Asset2, Asset1)) should produce(
            "Invalid Asset ID")
        }
      }
      ...
      "amount and price assets are the same" in {
        builder.validateAssetPair(AssetPair(WUSD, WUSD)) should produce(
          "Amount and price assets must be different")
      }
    }
  }
}
...
object AssetPairBuilderSpec {
  private def mkAssetId(index: Byte): IssuedAsset =
    IssuedAsset(ByteStr(Array.fill[Byte](32)(index)))
  private def mkAssetDescription(
      assetName: String = ""): Option[AssetDescription] =
    Some(
