...
class TestOrderDB(maxOrdersPerRequest: Int) extends OrderDB {
  ...
  private var idsForPair =
    Map.empty[(Address, AssetPair), Seq[ByteStr]].withDefaultValue(Seq.empty)
  private var idsForAddress =
    Map.empty[Address, Seq[ByteStr]].withDefaultValue(Seq.empty)

  override def containsInfo(id: ByteStr): Boolean = orderInfo.contains(id)

  override def status(id: ByteStr): OrderStatus.Final =
    orderInfo.get(id).fold[OrderStatus.Final](OrderStatus.NotFound)(_.status)
  ...
  override def saveOrderInfo(id: ByteStr,
    ...
    if (!containsInfo(id)) {
      ...
      idsForPair += (sender, oi.assetPair) -> (id +: idsForPair(
        sender -> oi.assetPair))
    }
