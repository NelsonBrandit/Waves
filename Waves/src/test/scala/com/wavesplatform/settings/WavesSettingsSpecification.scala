...
class WavesSettingsSpecification extends FlatSpec with Matchers {
  ...
  val expectedDir =
    ConfigFactory
      ...
      .withFallback(ConfigFactory.parseString(
        s"waves.directory=${com.wavesplatform.settings.defaultDirectory}"))
      .resolve()
  ...
  def testConfig(configName: String)(additionalChecks: WavesSettings => Unit =
                                       _ => ()) {
  ...
  "WavesSettings" should "resolve folders correctly" in {
    ...
    settings.walletSettings.file should be(
      Some(new File("/xxx/wallet/wallet.dat")))
    settings.matcherSettings.journalDataDir should be("/xxx/matcher/journal")
    settings.matcherSettings.snapshotsDataDir should be(
      "/xxx/matcher/snapshots")
  }
