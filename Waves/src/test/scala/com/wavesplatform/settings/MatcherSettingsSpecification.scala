...
class MatcherSettingsSpecification extends FlatSpec with Matchers {
  ...
  "MatcherSettings" should "read values" in {
    ...
    settings.blacklistedAddresses shouldBe Set(
      "3N5CBq8NYBMBU3UVS3rfMgaQEpjZrkWcBAD")
    settings.orderBookSnapshotHttpCache shouldBe OrderBookSnapshotHttpCache
      .Settings(
  ...
  "MatcherSettings" should "produce errors" in {
    ...
    def getSettingByConfig(conf: Config): Either[String, MatcherSettings] =
      Try(MatcherSettings.fromConfig(conf)).toEither.leftMap(_.getMessage)
    ...
    val settingsInvalidMode =
      getSettingByConfig(configStrWithOrderFeeSettings(invalidMode))
    val settingsInvalidTypeAndPercent = getSettingByConfig(
      configStrWithOrderFeeSettings(invalidAssetTypeAndPercent))
    val settingsInvalidAssetAndFee =
      getSettingByConfig(configStrWithOrderFeeSettings(invalidAssetAndFee))
    val settingsInvalidWavseAsset =
      getSettingByConfig(configStrWithOrderFeeSettings(invalidWavesAsset))
    val settingsInvalidFeeInWaves =
      getSettingByConfig(configStrWithOrderFeeSettings(invalidFeeInWaves))

    settingsInvalidMode shouldBe Left(
      "Invalid setting waves.matcher.order-fee.mode value: invalid")
    ...
    settingsInvalidWavseAsset shouldBe
      Left(
        "Invalid setting waves.matcher.order-fee.fixed.asset-id value: WAVES, asset must not be Waves")

    settingsInvalidFeeInWaves shouldBe Left(
      "Invalid setting waves.matcher.order-fee.fixed-waves.base-fee value: -350000, must be > 0")
  }
}
