...
class BlockchainUpdaterBlockMicroblockSequencesSameTransactionsTest
  ...
  type Setup = (GenesisTransaction,
                TransferTransactionV1,
                TransferTransactionV1,
                TransferTransactionV1)

  property(
    "resulting miner balance should not depend on tx distribution among blocks and microblocks") {
    forAll(g(100, 5)) {
      case (gen, rest) =>
  ...
  property(
    "Miner fee from microblock [Genesis] <- [Empty] <~ (Micro with tx) <- [Empty]") {
    val preconditionsAndPayments: Gen[
      (PrivateKeyAccount, GenesisTransaction, TransferTransactionV1, Int)] =
      for {
        master <- accountGen
        miner <- accountGen
        ts <- positiveIntGen
        ...
        genesis: GenesisTransaction = GenesisTransaction
          ...
          .explicitGet()
        payment: TransferTransactionV1 = createWavesTransfer(master,
                                                             master,
                                                             amt,
                                                             fee,
                                                             ts).explicitGet()
    ...
    scenario(preconditionsAndPayments, MicroblocksActivatedAt0WavesSettings) {
      case (domain, (miner, genesis, payment, ts)) =>
        ...
        val (base, micros) = chainBaseAndMicro(genBlock.uniqueId,
                                               Seq.empty,
                                               Seq(Seq(payment)),
                                               miner,
                                               3,
                                               ts)
        val emptyBlock = customBuildBlockOfTxs(micros.last.totalResBlockSig,
                                               Seq.empty,
                                               miner,
                                               3,
                                               ts)
        ...
        domain
          .portfolio(genesis.recipient)
          .balance shouldBe (genesis.amount - payment.fee)
    }
  }

  def randomPayment(accs: Seq[PrivateKeyAccount],
                    ts: Long): Gen[TransferTransactionV1] =
    for {
      from <- Gen.oneOf(accs)
      to <- Gen.oneOf(accs)
      fee <- smallFeeGen
  ...
  def randomPayments(accs: Seq[PrivateKeyAccount],
                     ts: Long,
                     amt: Int): Gen[Seq[TransferTransactionV1]] =
    if (amt == 0)
      Gen.const(Seq.empty)
    else
      for {
  ...
  def accsAndGenesis()
    : Gen[(Seq[PrivateKeyAccount], PrivateKeyAccount, Block, Int)] =
    for {
      alice <- accountGen
      bob <- accountGen
      charlie <- accountGen
      ...
      miner <- accountGen
      ts <- positiveIntGen
      genesis1: GenesisTransaction = GenesisTransaction
        .create(alice, TOTAL_WAVES / 4, ts)
        .explicitGet()
      genesis2: GenesisTransaction = GenesisTransaction
        .create(bob, TOTAL_WAVES / 4, ts + 1)
        .explicitGet()
      genesis3: GenesisTransaction = GenesisTransaction
        .create(charlie, TOTAL_WAVES / 4, ts + 2)
        .explicitGet()
      genesis4: GenesisTransaction = GenesisTransaction
        .create(dave, TOTAL_WAVES / 4, ts + 4)
        .explicitGet()
    } yield
      (Seq(alice, bob, charlie, dave),
       miner,
       customBuildBlockOfTxs(randomSig,
                             Seq(genesis1, genesis2, genesis3, genesis4),
                             defaultSigner,
                             1,
                             ts),
       ts)

  def g(totalTxs: Int, totalScenarios: Int)
    : Gen[(Block, Seq[(BlockAndMicroblockSequence, Block)])] =
    for {
      aaa @ (accs, miner, genesis, ts) <- accsAndGenesis()
      payments: Seq[TransferTransactionV1] <- randomPayments(accs, ts, totalTxs)
      intSeqs: Seq[BlockAndMicroblockSizes] <- randomSequences(totalTxs,
                                                               totalScenarios)
    } yield {
      val version = 3: Byte
      val blocksAndMicros = intSeqs.map { intSeq =>
        val blockAndMicroblockSequence =
          r(payments, intSeq, genesis.uniqueId, miner, version, ts)
        val ref = bestRef(blockAndMicroblockSequence.last)
        val lastBlock =
          customBuildBlockOfTxs(ref, Seq.empty, miner, version, ts)
...
object BlockchainUpdaterBlockMicroblockSequencesSameTransactionsTest {
  ...
  def genSplitSizes(total: Int): Gen[(Int, Seq[Int])] = genSizes(total).map {
    case (h :: tail) => (h, tail)
  }
  ...
  def randomSizeSequence(total: Int): Gen[BlockAndMicroblockSizes] =
    for {
      ...
      t <- if (totalStep < total) randomSizeSequence(total - totalStep)
      else Gen.const(Seq.empty)
    } yield h +: t

  def randomSequences(total: Int,
                      sequences: Int): Gen[Seq[BlockAndMicroblockSizes]] =
    if (sequences == 0)
      Gen.const(Seq.empty)
    else
      for {
  ...
  def take(txs: Seq[Transaction], sizes: BlockAndMicroblockSize)
    : ((Seq[Transaction], Seq[Seq[Transaction]]), Seq[Transaction]) = {
    val (blockAmt, microsAmts) = sizes
    val (blockTxs, rest) = txs.splitAt(blockAmt)
    val (reversedMicroblockTxs, res) =
      microsAmts.foldLeft((Seq.empty[Seq[Transaction]], rest)) {
        case ((acc, pool), amt) =>
