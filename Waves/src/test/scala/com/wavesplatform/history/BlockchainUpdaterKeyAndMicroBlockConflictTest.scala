...
class BlockchainUpdaterKeyAndMicroBlockConflictTest
  ...
  property("new key block should be validated to previous") {
    forAll(Preconditions.conflictingTransfers()) {
      case (prevBlock, keyBlock, microBlocks, keyBlock1) =>
        withDomain(MicroblocksActivatedAt0WavesSettings) { d =>
          ...
          microBlocks.foreach(
            d.blockchainUpdater.processMicroBlock(_) shouldBe 'right)
    ...
    forAll(Preconditions.leaseAndLeaseCancel()) {
      case (genesisBlock,
            ...
            secondAccount) =>
        withDomain(MicroblocksActivatedAt0WavesSettings) { d =>
          Seq(genesisBlock, leaseBlock, keyBlock).foreach(
            d.blockchainUpdater.processBlock(_) shouldBe 'right)
          assert(d.blockchainUpdater.effectiveBalance(secondAccount, 0) > 0)

          microBlocks.foreach(
            d.blockchainUpdater.processMicroBlock(_) shouldBe 'right)
          assert(
            d.blockchainUpdater.effectiveBalance(secondAccount,
                                                 0,
                                                 leaseBlock.uniqueId) > 0)

          assert(
            d.blockchainUpdater
              .processBlock(transferBlock)
              .toString
              .contains("negative effective balance"))
        }
    }
  }
  ...
  private[this] object Preconditions {
    ...
    def conflictingTransfers(): Gen[(Block, Block, Seq[MicroBlock], Block)] = {
      for {
        ...
        val genesisBlock = unsafeBlock(
          ...
          txs = Seq(
            GenesisTransaction
              .create(richAccount, tsAmount + FeeAmount, 0)
              .explicitGet()),
    ...
    def conflictingTransfersInMicro()
      : Gen[(Block, Block, Seq[MicroBlock], Block)] = {
      for {
        richAccount <- accountGen
        secondAccount <- accountGen
    ...
    def leaseAndLeaseCancel(): Gen[
      (Block, Block, Block, Seq[MicroBlock], Block, PrivateKeyAccount)] = {
      for {
        richAccount <- accountGen
        secondAccount <- accountGen
        ...
        val genesisBlock = unsafeBlock(
          ...
          txs = Seq(
            GenesisTransaction
              .create(richAccount, tsAmount + FeeAmount * 3, 0)
              .explicitGet()),
          signer = TestBlock.defaultSigner,
          version = 3,
          timestamp = 0
