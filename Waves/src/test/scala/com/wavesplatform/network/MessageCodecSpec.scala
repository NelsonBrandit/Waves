...
class MessageCodecSpec
  ...
  "should block a sender of invalid messages" in {
    ...
    ch.writeInbound(
      RawBytes(TransactionSpec.messageCode,
  ...
  "should not block a sender of valid messages" in forAll(randomTransactionGen) {
    origTx: ProvenTransaction =>
      val codec = new SpiedMessageCodec
      val ch = new EmbeddedChannel(codec)
