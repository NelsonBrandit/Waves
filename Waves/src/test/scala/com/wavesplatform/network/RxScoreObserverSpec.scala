...
class RxScoreObserverSpec
  ...
  def withObserver(
      f: (Coeval[Seq[SyncWith]],
          PublishSubject[BigInt],
          PublishSubject[(Channel, BigInt)],
          PublishSubject[Channel]) => Any) = {
    ...
    val (syncWith, _) = RxScoreObserver(1.minute,
                                        0.seconds,
                                        0,
                                        localScores,
                                        remoteScores,
    ...
    try {
      f(newItems(syncWith.map(_.syncWith))(implicitScheduler),
        localScores,
        remoteScores,
        channelClosed)
    } finally {
  ...
  "should emit better channel" - {
    "when a new channel has the better score than the local one" in withObserver {
      (newSyncWith, localScores, remoteScores, _) =>
    ...
    "when the connection with the best one is closed" in withObserver {
      (newSyncWith, localScores, remoteScores, closed) =>
    ...
    "when the best channel upgrades score" in withObserver {
      (newSyncWith, localScores, remoteScores, _) =>
        val testChannel = new LocalChannel()
    ...
    "when the best channel downgrades score" in withObserver {
      (newSyncWith, localScores, remoteScores, _) =>
        val testChannel = new LocalChannel()
  ...
  "should emit None" - {
    "stop when local score is as good as network's" in withObserver {
      (newSyncWith, localScores, remoteScores, _) =>
        val ch100 = new LocalChannel()
    ...
    "stop when local score is better than network's" in withObserver {
      (newSyncWith, localScores, remoteScores, _) =>
        val ch100 = new LocalChannel()
