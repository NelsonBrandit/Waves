...
class RxExtensionLoaderSpec
  ...
  val MaxRollback = 10
  type Applier =
    (Channel, ExtensionBlocks) => Task[Either[ValidationError, Option[BigInt]]]
  val simpleApplier: Applier = (_, _) => Task(Right(Some(0)))

  override def testSchedulerName: String = "test-rx-extension-loader"

  private def withExtensionLoader(lastBlockIds: Seq[ByteStr] = Seq.empty,
  ...
  "should propagate unexpected block" in withExtensionLoader() {
    (_, blocks, _, _, singleBlocks) =>
  ...
  "should blacklist GetSignatures timeout" in withExtensionLoader(
    Seq.tabulate(100)(byteStr),
    1.millis) { (_, _, _, ccsw, _) =>
    val ch = new EmbeddedChannel()
    test(for {
      _ <- send(ccsw)(
        ChannelClosedAndSyncWith(None, Some(BestChannel(ch, 1: BigInt))))
    } yield {
      ch.isOpen shouldBe false
    })
  }

  "should request GetSignatures and then span blocks from peer" in withExtensionLoader(
    Seq.tabulate(100)(byteStr)) { (_, _, sigs, ccsw, _) =>
    val ch = new EmbeddedChannel()
    val totalBlocksInHistory = 100
    test(for {
      _ <- send(ccsw)(
        ChannelClosedAndSyncWith(None, Some(BestChannel(ch, 1: BigInt))))
      _ = ch.readOutbound[GetSignatures].signatures shouldBe Range(
        totalBlocksInHistory - MaxRollback,
        totalBlocksInHistory).map(byteStr).reverse
  ...
  "should blacklist if received Signatures contains banned id" in withExtensionLoader(
    Seq.tabulate(100)(byteStr),
    1.millis) { (invBlockStorage, _, sigs, ccsw, _) =>
  ...
  "should blacklist if some blocks didn't arrive in due time" in withExtensionLoader(
    Seq.tabulate(100)(byteStr),
    1.second) { (_, blocks, sigs, ccsw, _) =>
    val ch = new EmbeddedChannel()
      ...
      Task.fromFuture {
        for {
          _ <- send(ccsw)(
            ChannelClosedAndSyncWith(None, Some(BestChannel(ch, 1: BigInt))))
          _ = ch
            .readOutbound[GetSignatures]
            .signatures
            .size shouldBe MaxRollback
          _ <- send(sigs)((ch, Signatures(Range(97, 102).map(byteStr))))
      ...
      for {
        _ <- Task
          ...
          .flatMap {
            case Left(_) =>
              Task.raiseError(
                new Exception("Preparation for the test takes too much time"))
            case _ => Task.pure(())
          }
  ...
  "should process received extension" in {
    ...
    withExtensionLoader(Seq.tabulate(100)(byteStr), applier = successfulApplier) {
