...
class MicroBlockSynchronizerSpec
  ...
  val defaultSettings =
    MicroblockSynchronizerSettings(1.second, 1.minute, 1.minute)

  private def withMs(f: (
      PS[ByteStr],
    ...
    val (r, _) = MicroBlockSynchronizer(defaultSettings,
                                        peers,
                                        lastBlockIds,
                                        microInvs,
                                        microResponses,
  ...
  "should request and propagate next microblock" in withMs {
    (lastBlockIds, microInvs, microResponses, r) =>
      ...
      test(for {
        ...
        _ <- send(microInvs)(
          (ch, MicroBlockInv(signer, byteStr(1), byteStr(0))))
        _ = ch.readOutbound[MicroBlockRequest] shouldBe MicroBlockRequest(
          byteStr(1))
  ...
  "should re-request next block if a previous one failed" in withMs {
    (lastBlockIds, microInvs, microResponses, r) =>
      ...
      test(for {
        ...
        _ = ch.readOutbound[MicroBlockRequest] shouldBe MicroBlockRequest(
          byteStr(1))
        _ <- send(microInvs)(
          (ch2, MicroBlockInv(signer, byteStr(1), byteStr(0))))
        _ = Thread.sleep(2000)
        _ = ch2.readOutbound[MicroBlockRequest] shouldBe MicroBlockRequest(
          byteStr(1))
  ...
  "should not request the same micro if received before" in withMs {
    (lastBlockIds, microInvs, microResponses, _) =>
      val ch = new EmbeddedChannel()
      test(for {
        _ <- send(lastBlockIds)(byteStr(0))
        _ <- send(microInvs)(
  ...
  "should not request forked microblocks" in withMs {
    (lastBlockIds, microInvs, _, _) =>
      val ch = new EmbeddedChannel()
      val ch2 = new EmbeddedChannel()
      test(for {
        _ <- send(lastBlockIds)(byteStr(0))
        ...
        _ = ch.readOutbound[MicroBlockRequest] shouldBe MicroBlockRequest(
          byteStr(1))
        _ <- send(microInvs)(
          (ch2, MicroBlockInv(signer, byteStr(2), byteStr(0))))
      } yield Option(ch2.readOutbound[MicroBlockRequest]) shouldBe None)
  }

  "should remember inv to make a request later" in withMs {
    (lastBlockIds, microInvs, microResponses, _) =>
      val ch = new EmbeddedChannel()
      val ch2 = new EmbeddedChannel()
      test(
        for {
          ...
          _ = ch.readOutbound[MicroBlockRequest] shouldBe MicroBlockRequest(
            byteStr(1))
          _ <- send(microInvs)(
            (ch2, MicroBlockInv(signer, byteStr(2), byteStr(1))))
          _ <- send(microResponses)((ch, MicroBlockResponse(microBlock(1, 0))))
          _ <- send(lastBlockIds)(byteStr(1))
        } yield
          ch2.readOutbound[MicroBlockRequest] shouldBe MicroBlockRequest(
            byteStr(2)))
  }
