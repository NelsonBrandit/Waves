...
class MicroBlockSpecification
    extends FunSuite
    with Matchers
    with MockFactory
    with ShouldVerb {

  val prevResBlockSig = ByteStr(
    Array.fill(Block.BlockIdLength)(Random.nextInt(100).toByte))
  val totalResBlockSig = ByteStr(
    Array.fill(Block.BlockIdLength)(Random.nextInt(100).toByte))
  ...
  test("MicroBlock with txs bytes/parse roundtrip") {
    ...
    val tr: TransferTransactionV1 = TransferTransactionV1
      .selfSigned(Waves, sender, gen, 5, ts + 1, Waves, 2, Array())
      .explicitGet()
    val assetId = IssuedAsset(
      ByteStr(Array.fill(AssetIdLength)(Random.nextInt(100).toByte)))
    val tr2: TransferTransactionV1 = TransferTransactionV1
      .selfSigned(assetId, sender, gen, 5, ts + 2, Waves, 2, Array())
      .explicitGet()

    val transactions = Seq(tr, tr2)

    val microBlock = MicroBlock
      .buildAndSign(sender, transactions, prevResBlockSig, totalResBlockSig)
      .explicitGet()
  ...
  test("MicroBlock cannot be created with zero transactions") {
    ...
    val eitherBlockOrError = MicroBlock.buildAndSign(sender,
                                                     transactions,
                                                     prevResBlockSig,
                                                     totalResBlockSig)
  ...
  test("MicroBlock cannot contain more than Miner.MaxTransactionsPerMicroblock") {
    ...
    val transaction = TransferTransactionV1
      .selfSigned(Waves,
                  sender,
                  gen,
                  5,
      ...
      .explicitGet()
    val transactions =
      Seq.fill(Miner.MaxTransactionsPerMicroblock + 1)(transaction)
