...
private object UtxPoolSpecification {
  ...
  final case class TempDB(fs: FunctionalitySettings) {
    ...
    val writer = new LevelDBWriter(db,
...
class UtxPoolSpecification
  ...
  private def mkBlockchain(senderAccount: Address, senderBalance: Long) = {
    ...
    val genesisSettings =
      TestHelpers.genesisSettings(Map(senderAccount -> senderBalance))
  ...
  private def transferWithRecipient(sender: PrivateKeyAccount,
  ...
  private def massTransferWithRecipients(sender: PrivateKeyAccount,
                                         recipients: List[PublicKeyAccount],
                                         maxAmount: Long,
                                         time: Time) = {
    val amount = maxAmount / (recipients.size + 1)
    val transfers = recipients.map(r => ParsedTransfer(r.toAddress, amount))
    val minFee = CommonValidation.FeeConstants(TransferTransaction.typeId) + CommonValidation
      .FeeConstants(MassTransferTransaction.typeId) * transfers.size
  ...
  private val twoOutOfManyValidPayments = (for {
    ...
    val txs = for (_ <- 1 to n)
      yield
        createWavesTransfer(sender,
                            recipient,
                            amountPart,
                            fee,
                            time.getTimestamp()).explicitGet()
  ...
  private val withValidPayments = (for {
    ...
    recipient <- accountGen
    time = new TestTime()
    txs <- Gen.nonEmptyListOf(
      transferWithRecipient(sender, recipient, senderBalance / 10, time))
  } yield {
    val settings = UtxSettings(10,
                               PoolDefaultMaxBytes,
                               Set.empty,
                               Set.empty,
                               allowTransactionsFromSmartAccounts = true)
    val utxPool = new UtxPoolImpl(time,
                                  bcu,
                                  ignoreSpendableBalanceChanged,
                                  FunctionalitySettings.TESTNET,
                                  settings)
  ...
  private val withBlacklisted = (for {
    ...
    recipient <- accountGen
    time = new TestTime()
    txs <- Gen.nonEmptyListOf(transferWithRecipient(
      sender,
      recipient,
      senderBalance / 10,
      time)) // @TODO: Random transactions
  } yield {
    val settings =
      UtxSettings(10,
                  PoolDefaultMaxBytes,
  ...
  private def massTransferWithBlacklisted(allowRecipients: Boolean) =
    (for {
      ...
      addressGen = Gen
        .listOf(accountGen)
        .filter(list => if (allowRecipients) list.nonEmpty else true)
      recipients <- addressGen
      time = new TestTime()
      txs <- Gen.nonEmptyListOf(
        massTransferWithRecipients(sender,
                                   recipients,
                                   senderBalance / 10,
                                   time))
    } yield {
      val whitelist: Set[String] =
        if (allowRecipients) recipients.map(_.address).toSet else Set.empty
      val settings =
        UtxSettings(txs.length,
                    PoolDefaultMaxBytes,
                    Set(sender.address),
  ...
  private def utxTest(utxSettings: UtxSettings = UtxSettings(
                        20,
                        PoolDefaultMaxBytes,
                        Set.empty,
                        Set.empty,
                        allowTransactionsFromSmartAccounts = true),
                      txCount: Int = 10)(
      f: (Seq[TransferTransactionV1], UtxPool, TestTime) => Unit): Unit =
    forAll(stateGen, chooseNum(2, txCount).label("txCount")) {
      case ((sender, senderBalance, bcu), count) =>
        val time = new TestTime()

        forAll(listOfN(count, transfer(sender, senderBalance / 2, time))) {
          txs =>
  ...
  private val dualTxGen
    : Gen[(UtxPool, TestTime, Seq[Transaction], Seq[Transaction])] =
    for {
      (sender, senderBalance, bcu) <- stateGen
      ts = System.currentTimeMillis()
      count1 <- chooseNum(5, 10)
      tx1 <- listOfN(count1,
                     transfer(sender, senderBalance / 2, new TestTime(ts)))
      tx2 <- listOfN(count1,
                     transfer(sender,
                              senderBalance / 2,
                              new TestTime(ts + maxAge.toMillis + 1000)))
    } yield {
      val time = new TestTime()
  ...
  private def preconditionsGen(lastBlockId: ByteStr,
                               master: PrivateKeyAccount): Gen[Seq[Block]] =
    for {
      version <- Gen.oneOf(SetScriptTransaction.supportedVersions.toSeq)
      ts <- timestampGen
    } yield {
      val setScript = SetScriptTransaction
        .selfSigned(master, Some(script), 100000, ts + 1)
        .explicitGet()
      Seq(TestBlock.create(ts + 1, lastBlockId, Seq(setScript)))
    }

  private def withScriptedAccount(
      scEnabled: Boolean): Gen[(PrivateKeyAccount, Long, UtxPoolImpl, Long)] =
    for {
      (sender, senderBalance, bcu) <- stateGen
      preconditions <- preconditionsGen(bcu.lastBlockId.get, sender)
    } yield {
      val smartAccountsFs = TestFunctionalitySettings.Enabled.copy(
  ...
  private def transactionV1Gen(sender: PrivateKeyAccount,
                               ts: Long,
                               feeAmount: Long): Gen[TransferTransactionV1] =
    accountGen.map { recipient =>
      TransferTransactionV1
        .selfSigned(Waves,
                    sender,
                    recipient,
  ...
  private def transactionV2Gen(sender: PrivateKeyAccount,
                               ts: Long,
                               feeAmount: Long): Gen[TransferTransactionV2] =
    accountGen.map { recipient =>
  ...
  "UTX Pool" - {
    "does not add new transactions when full" in utxTest(
      UtxSettings(1,
                  ...
                  Set.empty,
                  Set.empty,
                  allowTransactionsFromSmartAccounts = true)) { (txs, utx, _) =>
    ...
    "does not add new transactions when full in bytes" in utxTest(
      UtxSettings(999999,
                  ...
                  allowTransactionsFromSmartAccounts = true)) { (txs, utx, _) =>
      utx.putIfNew(txs.head) shouldBe 'right
      all(txs.tail.map(t => utx.putIfNew(t))) should produce(
        "pool bytes size limit")
    }

    "does not broadcast the same transaction twice" in utxTest() {
      (txs, utx, _) =>
        utx.putIfNew(txs.head) shouldBe 'right
        utx.putIfNew(txs.head) shouldBe 'right
    }
    ...
    "evicts expired transactions when packUnconfirmed is called" in forAll(
      dualTxGen) {
      case (utx, time, txs, _) =>
        all(txs.map(utx.putIfNew)) shouldBe 'right
        utx.all.size shouldEqual txs.size
    ...
    "evicts one of mutually invalid transactions when packUnconfirmed is called" in forAll(
      twoOutOfManyValidPayments) {
    ...
    "pessimisticPortfolio" - {
      ...
      "is changed after transactions with these assets are removed" in forAll(
        withValidPayments) {
        ...
        case (sender, state, utxPool, _, _) =>
          ...
          withClue("unchanged") {
            unchangedAssetIds.foreach { assetId =>
              basePortfolio.balanceOf(assetId) shouldBe basePortfolio.balanceOf(
                assetId)
            }
          }
          ...
          withClue("changed") {
            changedAssetIds.foreach { assetId =>
              basePortfolio.balanceOf(assetId) should not be basePortfolio
                .balanceOf(assetId)
            }
          }
      }
    ...
    "blacklisting" - {
      "prevent a transfer transaction from specific addresses" in {
        val transferGen =
          Gen.oneOf(withBlacklisted,
                    massTransferWithBlacklisted(allowRecipients = false))
        forAll(transferGen) {
          case (_, utxPool, txs) =>
            val r = txs.forall { tx =>
      ...
      "allow a transfer transaction from blacklisted address to specific addresses" in {
        val transferGen =
          Gen.oneOf(withBlacklistedAndAllowedByRule,
                    massTransferWithBlacklisted(allowRecipients = true))
        forAll(transferGen) {
          case (_, utxPool, txs) =>
            all(txs.map { t =>
              utxPool.putIfNew(t)
    ...
    "smart accounts" - {
      "signed txs from scripted account is not allowed" in {
        ...
        val (utx, tx) = enoughFeeTxWithScriptedAccount.sample.getOrElse(
      ...
      "any transaction from scripted account is not allowed if smartAccounts disabled in utx pool" - {
        ...
        def enoughFeeTxWithScriptedAccount(
            version: Int): Gen[(UtxPoolImpl, TransferTransaction)] =
          for {
        ...
        "v1" in {
          val (utx1, tx1) = enoughFeeTxWithScriptedAccount(1).sample
            .getOrElse(throw new IllegalStateException("NO SAMPLE"))
          utx1.putIfNew(tx1) shouldBe 'left
        }
        "v2" in {
          val (utx2, tx2) = enoughFeeTxWithScriptedAccount(2).sample
            .getOrElse(throw new IllegalStateException("NO SAMPLE"))
