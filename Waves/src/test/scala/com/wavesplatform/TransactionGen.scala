...
trait TransactionGenBase extends ScriptGen with TypedScriptGen with NTPTime {
  _: Suite =>
  ...
  def byteArrayGen(length: Int): Gen[Array[Byte]] =
    Gen.containerOfN[Array, Byte](length, Arbitrary.arbitrary[Byte])
  ...
  val ntpTimestampGen: Gen[Long] =
    Gen.choose(1, 1000).map(ntpTime.correctedTime() - _)

  val accountGen: Gen[PrivateKeyAccount] =
    bytes32gen.map(seed => PrivateKeyAccount(seed))

  val aliasSymbolChar: Gen[Char] = Gen.oneOf('.', '@', '_', '-')

  val invalidAliasSymbolChar: Gen[Char] =
    Gen.oneOf('~', '`', '!', '#', '$', '%', '^', '&', '*', '=', '+')

  val aliasAlphabetGen: Gen[Char] =
    frequency((1, numChar), (1, aliasSymbolChar), (9, alphaLowerChar))

  val invalidAliasAlphabetGen: Gen[Char] =
    frequency((1, numChar), (3, invalidAliasSymbolChar), (9, alphaUpperChar))
  ...
  val accountOrAliasGen: Gen[AddressOrAlias] =
    Gen.oneOf(aliasGen, accountGen.map(PublicKeyAccount.toAddress(_)))

  def otherAccountGen(candidate: PrivateKeyAccount): Gen[PrivateKeyAccount] =
    accountGen.flatMap(Gen.oneOf(candidate, _))
  ...
  val maxOrderTimeGen: Gen[Long] =
    Gen.choose(10000L, Order.MaxLiveTime).map(_ + ntpTime.correctedTime())
  val timestampGen: Gen[Long] = Gen.choose(1, Long.MaxValue - 100)

  val wavesAssetGen: Gen[Option[ByteStr]] = Gen.const(None)
  val assetIdGen: Gen[Option[ByteStr]] = Gen.frequency(
    (1, wavesAssetGen),
    (10, Gen.option(bytes32gen.map(ByteStr(_)))))

  val assetPairGen = assetIdGen.flatMap {
    case None => bytes32gen.map(b => AssetPair(Waves, IssuedAsset(ByteStr(b))))
    case Some(a1bytes) =>
      val a2bytesGen: Gen[Option[Array[Byte]]] =
        byteArrayGen(31).map(a2bytes => Option(a1bytes.arr(0) +: a2bytes))
  ...
  val contractScriptGen: Gen[Script] =
    contractGen.map(e => ContractScript(V3, e).explicitGet())
  val contractOrExpr = Gen.oneOf(scriptGen, contractScriptGen)
  val setAssetScriptTransactionGen
    : Gen[(Seq[Transaction], SetAssetScriptTransaction)] = for {
    ...
    (Seq(issue),
     SetAssetScriptTransaction
       .create(AddressScheme.current.chainId,
               sender,
  ...
  val setScriptTransactionGen: Gen[SetScriptTransaction] = for {
    ...
    SetScriptTransaction
      .create(sender, script, fee, timestamp, proofs)
      .explicitGet()

  def selfSignedSetScriptTransactionGenP(sender: PrivateKeyAccount,
                                         s: Script): Gen[SetScriptTransaction] =
    for {
      version <- Gen.oneOf(SetScriptTransaction.supportedVersions.toSeq)
      fee <- smallFeeGen
      timestamp <- timestampGen
      ...
      SetScriptTransaction
        .selfSigned(sender, Some(s), fee, timestamp)
        .explicitGet()
  ...
  val selfPaymentGen: Gen[PaymentTransaction] =
    accountGen.flatMap(acc => paymentGeneratorP(acc, acc))

  def paymentGeneratorP(sender: PrivateKeyAccount,
                        recipient: PrivateKeyAccount): Gen[PaymentTransaction] =
    timestampGen.flatMap(ts => paymentGeneratorP(ts, sender, recipient))

  def paymentGeneratorP(timestamp: Long,
    ...
    for {
      ...
      PaymentTransaction
        .create(sender, recipient, amount, fee, timestamp)
        .explicitGet()
  ...
  def createLease(sender: PrivateKeyAccount,
    ...
    val v1 = LeaseTransactionV1
      .selfSigned(sender, amount, fee, timestamp, recipient)
      .explicitGet()
    val v2 = LeaseTransactionV2
      .selfSigned(sender, amount, fee, timestamp, recipient)
      .explicitGet()
    Gen.oneOf(v1, v2)
  }
  ...
  def createLeaseCancel(sender: PrivateKeyAccount,
                        ...
                        cancelFee: Long,
                        timestamp: Long) = {
    val v1 = LeaseCancelTransactionV1
      .selfSigned(sender, leaseId, cancelFee, timestamp + 1)
      .explicitGet()
  ...
  val leaseAndCancelGen: Gen[(LeaseTransaction, LeaseCancelTransaction)] = for {
    ...
    leaseCancel <- createLeaseCancel(sender,
                                     lease.id(),
                                     cancelFee,
                                     timestamp + 1)
  ...
  val leaseAndCancelWithOtherSenderGen
    : Gen[(LeaseTransaction, LeaseCancelTransaction)] = for {
    (sender, amount, fee, timestamp, recipient) <- leaseParamGen
  ...
  def transferGeneratorP(sender: PrivateKeyAccount,
                         recipient: AddressOrAlias,
                         assetId: Asset,
                         feeAssetId: Asset): Gen[TransferTransactionV1] =
    for {
  ...
  def versionedTransferGeneratorP(
      sender: PrivateKeyAccount,
      recipient: AddressOrAlias,
      assetId: Asset,
      feeAssetId: Asset): Gen[TransferTransactionV2] =
    for {
      (_, _, _, amount, timestamp, _, feeAmount, attachment) <- transferParamGen
    } yield
  ...
  def transferGeneratorP(timestamp: Long,
                         sender: PrivateKeyAccount,
                         recipient: AddressOrAlias,
                         maxAmount: Long): Gen[TransferTransactionV1] =
    for {
  ...
  def transferGeneratorPV2(timestamp: Long,
                           sender: PrivateKeyAccount,
                           recipient: AddressOrAlias,
                           maxAmount: Long): Gen[TransferTransactionV2] =
    for {
      amount <- Gen.choose(1, maxAmount)
      (_, _, _, _, _, _, feeAmount, attachment) <- transferParamGen
    } yield
  ...
  def wavesTransferGeneratorP(
      sender: PrivateKeyAccount,
      recipient: AddressOrAlias): Gen[TransferTransactionV1] =
  ...
  def massTransferGeneratorP(sender: PrivateKeyAccount,
                             transfers: List[ParsedTransfer],
                             assetId: Asset): Gen[MassTransferTransaction] =
    for {
  ...
  def createWavesTransfer(
    ...
    TransferTransactionV1.selfSigned(Waves,
                                     sender,
                                     recipient,
                                     amount,
                                     timestamp,
  ...
  def versionedTransferGenP(sender: PublicKeyAccount,
                            recipient: Address,
                            proofs: Proofs) =
    (for {
      amt <- positiveLongGen
  ...
  val massTransferGen: Gen[MassTransferTransaction] = massTransferGen(
    MaxTransferCount)
  ...
  val createAliasGen: Gen[CreateAliasTransaction] = for {
    ...
    tx <- Gen.oneOf(
      CreateAliasTransactionV1
        .selfSigned(sender, alias, MinIssueFee, timestamp)
        .explicitGet(),
  ...
  def createAliasGen(sender: PrivateKeyAccount,
                     alias: Alias,
                     fee: Long,
                     timestamp: Long): Gen[CreateAliasTransaction] = {
    for {
      tx <- Gen.oneOf(
        CreateAliasTransactionV1
          .selfSigned(sender, alias, fee, timestamp)
          .explicitGet(),
        CreateAliasTransactionV2
          .selfSigned(sender, alias, fee, timestamp)
          .explicitGet()
  ...
  val issueParamGen = for {
    sender: PrivateKeyAccount <- accountGen
    assetName <- genBoundedString(IssueTransaction.MinAssetNameLength,
  ...
  val issueReissueBurnGen
    : Gen[(IssueTransaction, ReissueTransaction, BurnTransaction)] = for {
    amount <- positiveLongGen
    sender: PrivateKeyAccount <- accountGen
    r <- issueReissueBurnGeneratorP(amount, amount, amount, sender)
  } yield r

  def issueReissueBurnGeneratorP(issueQuantity: Long, sender: PrivateKeyAccount)
    : Gen[(IssueTransaction, ReissueTransaction, BurnTransaction)] =
    issueReissueBurnGeneratorP(issueQuantity,
                               issueQuantity,
                               issueQuantity,
                               sender)
  ...
  def createIssue(issuer: PrivateKeyAccount,
    ...
    val issuev1 = IssueTransactionV1
      .selfSigned(issuer,
                  assetName,
                  description,
                  quantity,
  ...
  def createBurn(burner: PrivateKeyAccount,
                 assetId: IssuedAsset,
                 amount: Long,
                 fee: Long,
                 timestamp: Long): Gen[BurnTransaction] = {
    for {
      tx <- Gen.oneOf(
        BurnTransactionV1
          .selfSigned(burner, assetId, amount, fee, timestamp)
          .explicitGet(),
        BurnTransactionV2
          .selfSigned(AddressScheme.current.chainId,
                      burner,
  ...
  def issueReissueBurnGeneratorP(issueQuantity: Long,
    ...
    for {
      ...
      issue <- createIssue(sender,
                           ...
                           iFee,
                           timestamp)
      reissue <- createReissue(sender,
                               IssuedAsset(issue.assetId()),
                               reissueQuantity,
                               reissuable2,
                               fee,
                               timestamp)
      burn <- createBurn(sender,
                         IssuedAsset(issue.assetId()),
                         burnQuantity,
                         fee,
                         timestamp)
  ...
  val issueWithInvalidReissuesGen: Gen[
    (IssueTransactionV1, ReissueTransactionV1, ReissueTransactionV1)] = for {
    (sender, assetName, description, quantity, decimals, _, iFee, timestamp) <- issueParamGen
    fee <- smallFeeGen
  } yield {
    val issue = IssueTransactionV1
      .selfSigned(sender,
                  assetName,
                  description,
                  quantity,
                  ...
                  timestamp)
      .explicitGet()
    val reissue1 = ReissueTransactionV1
      .selfSigned(sender,
                  IssuedAsset(issue.assetId()),
                  quantity,
                  reissuable = false,
  ...
  def issueGen(sender: PrivateKeyAccount,
               fixedQuantity: Option[Long] = None): Gen[IssueTransactionV1] =
    for {
  ...
  def sponsorFeeCancelSponsorFeeGen(
      sender: PrivateKeyAccount): Gen[(IssueTransaction,
  ...
  val invokeScriptGen = for {
    ...
    InvokeScriptTransaction
      .selfSigned(sender, contractAddress, fc, po.toSeq, fee, Waves, timestamp)
      .explicitGet()
  ...
  val exchangeTransactionGen: Gen[ExchangeTransaction] = for {
    ...
    buyerMatcherFeeAssetId <- Gen.oneOf(assetPair.amountAsset,
                                        assetPair.priceAsset,
                                        buyerAnotherAsset,
                                        Waves)
    sellerMatcherFeeAssetId <- Gen.oneOf(assetPair.amountAsset,
                                         assetPair.priceAsset,
                                         sellerAnotherAsset,
                                         Waves)
    r <- Gen.oneOf(
      exchangeV1GeneratorP(sender1,
                           sender2,
  ...
  def exchangeV1GeneratorP(
    ...
    for {
      ...
      matchedAmount: Long <- Gen.choose(Math.min(amount1, amount2) / 2000,
                                        Math.min(amount1, amount2) / 1000)
      assetPair = AssetPair(amountAssetId, priceAssetId)
    } yield {
      val matcherFee = fixedMatcherFee.getOrElse(genMatcherFee)
      val o1 = Order.buy(buyer,
                         matcher,
                         assetPair,
                         amount1,
                         price,
      ...
      val o2 = Order.sell(seller,
                          ...
                          1: Byte)
      val buyFee =
        (BigInt(matcherFee) * BigInt(matchedAmount) / BigInt(amount1))
          .longValue()
      val sellFee =
        (BigInt(matcherFee) * BigInt(matchedAmount) / BigInt(amount2))
          .longValue()
      val trans =
  ...
  private type OrderConstructor = (PrivateKeyAccount,
                                   PublicKeyAccount,
                                   AssetPair,
                                   Long,
                                   Long,
  ...
  def exchangeV2GeneratorP(buyer: PrivateKeyAccount,
                           ...
                           fixedMatcher: Option[PrivateKeyAccount] = None)
    ...
    for {
      (_,
       ...
       timestamp,
       expiration,
       generatedMatcherFee) <- orderParamGen
      amount2: Long <- matcherAmountGen
  ...
  val randomTransactionGen: Gen[ProvenTransaction] = (for {
    ...
    ca <- createAliasGen
      .retryUntil(_.version == 1)
      .map(_.asInstanceOf[CreateAliasTransactionV1])
    xt <- exchangeTransactionGen
    tx <- Gen.oneOf(tr,
                    is.asInstanceOf[IssueTransactionV1],
                    ri.asInstanceOf[ReissueTransactionV1],
                    ca,
                    bu.asInstanceOf[BurnTransactionV1],
                    xt)
  ...
  val genesisGen: Gen[GenesisTransaction] =
    accountGen.flatMap(genesisGeneratorP)
  ...
  def dataEntryGen(maxSize: Int, keyGen: Gen[String] = dataKeyGen) =
    Gen.oneOf(longEntryGen(keyGen),
              booleanEntryGen(keyGen),
              binaryEntryGen(maxSize, keyGen),
              stringEntryGen(maxSize, keyGen))

  val dataTransactionGen: Gen[DataTransaction] = dataTransactionGen(
    DataTransaction.MaxEntryCount)
  ...
  def dataTransactionGen(maxEntryCount: Int, useForScript: Boolean = false) =
    (for {
      ...
      maxEntrySize = if (useForScript) 200
      else
        (DataTransaction.MaxBytes - 122) / (size max 1) min DataEntry.MaxValueSize
      data <- if (useForScript)
        Gen.listOfN(size, dataEntryGen(maxEntrySize, dataScriptsKeyGen))
      ...
      DataTransaction
        .selfSigned(sender, uniq, 15000000, timestamp)
        .explicitGet())
      .label("DataTransaction")

  def dataTransactionGenP(sender: PrivateKeyAccount,
                          data: List[DataEntry[_]]): Gen[DataTransaction] =
    (for {
      version <- Gen.oneOf(DataTransaction.supportedVersions.toSeq)
      timestamp <- timestampGen
    } yield
      DataTransaction
        .selfSigned(sender, data, 15000000, timestamp)
        .explicitGet())
      .label("DataTransactionP")

  def preconditionsTransferAndLease(typed: EXPR): Gen[(GenesisTransaction,
                                                       SetScriptTransaction,
                                                       LeaseTransaction,
                                                       TransferTransactionV2)] =
    for {
      ...
      setScript <- selfSignedSetScriptTransactionGenP(
        master,
        ExprScript(typed).explicitGet())
      transfer <- transferGeneratorPV2(ts,
                                       master,
                                       recipient.toAddress,
                                       ENOUGH_AMT / 2)
      fee <- smallFeeGen
      lease = LeaseTransactionV2
        .selfSigned(master, ENOUGH_AMT / 2, fee, ts, recipient)
        .explicitGet()
    } yield (genesis, setScript, lease, transfer)

  def smartIssueTransactionGen(
      senderGen: Gen[PrivateKeyAccount] = accountGen,
