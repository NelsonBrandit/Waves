...
class FairPoSCalculatorTest extends PropSpec with Matchers {
  ...
  property(
    "Correct consensus parameters distribution of blocks generated with FairPoS") {

    val miners = mkMiners
    val first = Block(0,
                      defaultBaseTarget,
                      PrivateKeyAccount(generationSignature),
  ...
  def mineBlock(prev: Block,
                grand: Option[Block],
                minerWithBalance: (PrivateKeyAccount, Long)): Block = {
  ...
  def calcPerfomance(
      chain: List[Block],
      miners: Map[PrivateKeyAccount, Long]): Map[Long, Double] = {
    ...
    chain
      ...
      .map(mbs => {
        ...
        val expectedBlocks =
          ((minerBalance.toDouble / balanceSum) * blocksCount).toLong
