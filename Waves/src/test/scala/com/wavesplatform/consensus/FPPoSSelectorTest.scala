...
class FPPoSSelectorTest
          ...
          val fork1Delay = {
            val blockForHit =
              fork1
                .lift(100)
                .orElse(
                  blockchain.blockAt(blockchain.height + fork1.length - 100))
                .getOrElse(fork1.head)

            calcDelay(blockForHit,
                      fork1.head.consensusData.baseTarget,
                      miner1.publicKey,
                      miner1Balance)
          }
          ...
          val fork2Delay = {
            val blockForHit =
              fork2
                .lift(100)
                .orElse(
                  blockchain.blockAt(blockchain.height + fork2.length - 100))
                .getOrElse(fork2.head)

            calcDelay(blockForHit,
                      fork2.head.consensusData.baseTarget,
                      miner1.publicKey,
                      miner1Balance)
          }
    ...
    "failed when GS is incorrect" in {
      withEnv(chainGen(List(ENOUGH_AMT), 100)) {
        case Env(pos, blockchain, miners) =>
          val miner = miners.head
          val height = blockchain.height
          val block = forgeBlock(miner, blockchain, pos)(updateGS = gs =>
            ByteStr(gs.arr |< Random.nextBytes))
  ...
  "regression" - {
    ...
    "base target" in {
      FairPoSCalculator.calculateBaseTarget(100l,
                                            30,
                                            100l,
                                            100000000000l,
                                            Some(99000l),
                                            100000l) shouldBe 99l
      FairPoSCalculator.calculateBaseTarget(100l,
                                            10,
                                            100l,
                                            100000000000l,
                                            None,
                                            100000000000l) shouldBe 100l
      FairPoSCalculator.calculateBaseTarget(100l,
                                            10,
                                            100l,
                                            100000000000l,
      ...
      FairPoSCalculator.calculateBaseTarget(100l,
                                            ...
                                            100000000000l,
                                            Some(1l),
                                            1000000l) shouldBe 101l
    }
  }

  def withEnv(gen: Time => Gen[(Seq[PrivateKeyAccount], Seq[Block])])(
      f: Env => Unit): Unit = {
...
object FPPoSSelectorTest {
  ...
  final case class Env(pos: PoSSelector,
                       blockchain: BlockchainUpdater with NG,
                       miners: Seq[PrivateKeyAccount])

  def produce(errorMessage: String): ProduceError =
    new ProduceError(errorMessage)

  def mkFork(blockCount: Int,
    ...
    ((1 to blockCount) foldLeft List(lastBlock)) { (forkChain, ind) =>
      ...
      val bt = FairPoSCalculator.calculateBaseTarget(
        ...
        (forkChain.lift(2) orElse blockchain
          .blockAt(height + ind - 3)) map (_.timestamp),
  ...
  def forgeBlock(miner: PrivateKeyAccount,
    ...
    val updatedCData = cData.copy(updateBT(cData.baseTarget),
  ...
  def chainGen(balances: List[Long], blockCount: Int)(
      t: Time): Gen[(Seq[PrivateKeyAccount], Seq[Block])] = {
    ...
    Gen
      ...
      .map { accountsWithBalances =>
        for {
          ...
          (acc, GenesisTransaction.create(acc, balance, ts + i).explicitGet())
      }
      .map { txs =>
        ...
        val chain = (1 to blockCount foldLeft List(genesisBlock)) {
          (blocks, d) =>
  ...
  def calcDelay(blockForHit: Block,
                prevBT: Long,
                minerPK: Array[Byte],
                effBalance: Long): Long = {
