...
class ScriptCacheTest
  ...
  def blockGen(scripts: List[Script],
    ...
    Gen
      ...
      .map { accounts =>
        for {
          ...
          (account,
           GenesisTransaction
             .create(account.toAddress, AMOUNT, ts + i)
             .explicitGet())
      }
      .map { ag =>
  ...
  def withBlockchain(gen: Time => Gen[(Seq[PrivateKeyAccount], Seq[Block])])(
      f: (Seq[PrivateKeyAccount], BlockchainUpdater with NG) => Unit): Unit = {
