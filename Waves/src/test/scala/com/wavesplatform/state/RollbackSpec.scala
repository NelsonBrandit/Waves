...
class RollbackSpec
  ...
  private def genesisBlock(genesisTs: Long,
                           address: Address,
                           initialBalance: Long) = TestBlock.create(
    genesisTs,
    ByteStr(Array.fill[Byte](SignatureLength)(0)),
    Seq(
      GenesisTransaction
        .create(address, initialBalance, genesisTs)
        .explicitGet())
  )

  private def transfer(sender: PrivateKeyAccount,
                       recipient: Address,
                       amount: Long) =
  ...
  private def randomOp(sender: PrivateKeyAccount,
                       recipient: Address,
                       amount: Long,
                       op: Int,
                       nextTs: => Long = nextTs) = {
    ...
    op match {
      case 1 =>
        val lease = LeaseTransactionV1
          .selfSigned(sender, amount, 100000, nextTs, recipient)
          .explicitGet()
        List(lease,
             LeaseCancelTransactionV1
               .selfSigned(sender, lease.id(), 1, nextTs)
               .explicitGet())
      case 2 =>
        List(
          MassTransferTransaction
            .selfSigned(Waves,
                        sender,
                        List(ParsedTransfer(recipient, amount),
  ...
  "Rollback resets" - {
    "Rollback save dropped blocks order" in forAll(accountGen,
                                                   positiveLongGen,
                                                   Gen.choose(1, 10)) {
    ...
    "forget rollbacked transaction for querying" in forAll(
      accountGen,
      accountGen,
      Gen.nonEmptyListOf(Gen.choose(1, 10))) {
      case (sender, recipient, txCount) =>
        withDomain(createSettings(MassTransfer -> 0)) { d =>
          d.appendBlock(
            genesisBlock(nextTs,
                         sender,
                         com.wavesplatform.state.diffs.ENOUGH_AMT))

          val genesisSignature = d.lastBlockId

          val transferAmount = 100

          val transfers = txCount.map(
            tc =>
              Seq
                .fill(tc)(randomOp(sender, recipient, transferAmount, tc % 3))
                .flatten)
          ...
          val stransactions1 =
            d.addressTransactions(sender).explicitGet().sortBy(_._2.timestamp)
          val rtransactions1 = d
            .addressTransactions(recipient)
            .explicitGet()
    ...
    "waves balances" in forAll(accountGen,
                               positiveLongGen,
                               accountGen,
                               Gen.nonEmptyListOf(Gen.choose(1, 10))) {
      case (sender, initialBalance, recipient, txCount) =>
        withDomain() { d =>
          ...
          d.portfolio(recipient)
            .balance shouldBe (transferAmount * totalTxCount)
          d.portfolio(sender)
            .balance shouldBe (initialBalance - (transferAmount + 1) * totalTxCount)
    ...
    "lease balances and states" in forAll(accountGen,
                                          positiveLongGen,
                                          accountGen) {
      case (sender, initialBalance, recipient) =>
        withDomain() { d =>
          ...
          val lt = LeaseTransactionV1
            .selfSigned(sender, leaseAmount, 1, nextTs, recipient)
            .explicitGet()
          ...
          d.blockchainUpdater.leaseDetails(lt.id()) should contain(
            LeaseDetails(sender, recipient, 2, leaseAmount, true))
          ...
          d.appendBlock(
            TestBlock.create(
              ...
              Seq(
                LeaseCancelTransactionV1
                  .selfSigned(sender, lt.id(), 1, nextTs)
                  .explicitGet())
            ))
          d.blockchainUpdater.leaseDetails(lt.id()) should contain(
            LeaseDetails(sender, recipient, 2, leaseAmount, false))
    ...
    "asset balances" in forAll(accountGen,
                               positiveLongGen,
                               positiveLongGen,
                               accountGen) {
      case (sender, initialBalance, assetAmount, recipient) =>
        withDomain() { d =>
          ...
          d.portfolio(sender)
            .assets
            .get(IssuedAsset(issueTransaction.id())) should contain(assetAmount)
          d.portfolio(recipient)
            .assets
            .get(IssuedAsset(issueTransaction.id())) shouldBe 'empty
          ...
          d.appendBlock(
            TestBlock.create(
              ...
              Seq(
                TransferTransactionV1
                  .selfSigned(IssuedAsset(issueTransaction.id()),
                              sender,
                              recipient,
                              assetAmount,
                              nextTs,
          ...
          d.portfolio(sender)
            .assets
            .getOrElse(IssuedAsset(issueTransaction.id()), 0) shouldEqual 0
          d.portfolio(recipient)
            .assets
            .getOrElse(IssuedAsset(issueTransaction.id()), 0) shouldEqual assetAmount
    ...
    "asset quantity and reissuability" in forAll(accountGen,
                                                 positiveLongGen,
                                                 byteArrayGen(10),
                                                 byteArrayGen(12)) {
      case (sender, initialBalance, name, description) =>
        withDomain() { d =>
          ...
          val issueTransaction = IssueTransactionV1
            .selfSigned(sender, name, description, 2000, 8, true, 1, nextTs)
            .explicitGet()
          d.blockchainUpdater.assetDescription(
            IssuedAsset(issueTransaction.id())) shouldBe 'empty
          ...
          d.blockchainUpdater.assetDescription(
            IssuedAsset(issueTransaction.id())) should contain(
          ...
          d.appendBlock(
            TestBlock.create(nextTs,
                             ...
                             Seq(
                               ReissueTransactionV1
                                 .selfSigned(sender,
                                             IssuedAsset(issueTransaction.id()),
                                             2000,
                                             false,
                                             1,
                                             nextTs)
    ...
    "aliases" in forAll(accountGen, positiveLongGen, aliasGen) {
      case (sender, initialBalance, alias) =>
        withDomain() { d =>
          ...
          d.blockchainUpdater.resolveAlias(alias) shouldBe Left(
          ...
          d.appendBlock(
            TestBlock.create(
              ...
              Seq(
                CreateAliasTransactionV1
                  .selfSigned(sender, alias, 1, nextTs)
                  .explicitGet())
            ))

          d.blockchainUpdater.resolveAlias(alias) shouldBe Right(
            sender.toAddress)
          d.removeAfter(genesisBlockId)
    ...
    "data transaction" in forAll(accountGen,
                                 positiveLongGen,
                                 dataEntryGen(1000)) {
      case (sender, initialBalance, dataEntry) =>
        withDomain(createSettings(BlockchainFeatures.DataTransaction -> 0)) {
          d =>
            d.appendBlock(genesisBlock(nextTs, sender, initialBalance))
            val genesisBlockId = d.lastBlockId
            ...
            d.appendBlock(
              TestBlock.create(
                ...
                Seq(
                  DataTransaction
                    .selfSigned(sender, List(dataEntry), 1, nextTs)
                    .explicitGet())
              ))

            d.blockchainUpdater
              .accountData(sender, dataEntry.key) should contain(dataEntry)

            d.removeAfter(genesisBlockId)
            d.blockchainUpdater
              .accountData(sender, dataEntry.key) shouldBe 'empty
        }
    }
        ...
        withDomain(createSettings(SmartAccounts -> 0)) { d =>
            ...
            TestBlock.create(
              ...
              Seq(
                SetScriptTransaction
                  .selfSigned(sender, Some(script), 400000, nextTs)
                  .explicitGet())
            ))
          ...
          d.appendBlock(
            TestBlock.create(
              ...
              Seq(
                SetScriptTransaction
                  .selfSigned(sender, None, 800000, nextTs)
                  .explicitGet())
            ))
    ...
    def createSettings(
        preActivatedFeatures: (BlockchainFeature, Int)*): WavesSettings = {
      ...
      history.DefaultWavesSettings.copy(
        blockchainSettings = history.DefaultWavesSettings.blockchainSettings
          .copy(functionalitySettings = tfs))
    }
    ...
    "asset sponsorship" in forAll(for {
      ...
      case (sender, (issueTransaction, sponsor1, sponsor2, cancel)) =>
        ...
        withDomain(createSettings(FeeSponsorship -> 0)) { d =>
          ...
          d.blockchainUpdater
            .assetDescription(sponsor1.asset)
            .get
            .sponsorship shouldBe sponsor1.minSponsoredAssetFee.get
          d.portfolio(sender)
            .assets
            .get(IssuedAsset(issueTransaction.id())) should contain(
          ...
          d.blockchainUpdater
            .assetDescription(sponsor1.asset)
            .get
            .sponsorship shouldBe sponsor2.minSponsoredAssetFee.get
    ...
    "carry fee" in forAll(for {
      sender <- accountGen
      sponsorship <- sponsorFeeCancelSponsorFeeGen(sender)
      transfer <- transferGeneratorP(sponsorship._1.timestamp,
                                     sender,
                                     sender,
                                     10000000000L)
    } yield {
    ...
    "relean rollbacked transaction" in forAll(accountGen,
                                              accountGen,
                                              Gen.listOfN(66,
                                                          Gen.choose(1, 10))) {
      case (sender, recipient, txCount) =>
        withDomain(createSettings(MassTransfer -> 0)) { d =>
          ...
          d.appendBlock(
            genesisBlock(ts, sender, com.wavesplatform.state.diffs.ENOUGH_AMT))
          ...
          try {
            ...
            case e: Throwable =>
              Assertions.assert(e.getMessage().contains("AlreadyInTheState"))
          }
        }
    }
  }
