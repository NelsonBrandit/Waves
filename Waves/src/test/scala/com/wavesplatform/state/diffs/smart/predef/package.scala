...
package object predef {
  ...
  def runScript[T <: EVALUATED](script: String,
                                version: StdLibVersion,
                                t: In,
                                blockchain: Blockchain,
                                chainId: Byte): Either[String, T] = {
    val Success(expr, _) = Parser.parseExpr(script)
    for {
      compileResult <- ExpressionCompiler(
        compilerContext(version, ContentType.Expression, isAssetScript = false),
        expr)
  ...
  def runScript[T <: EVALUATED](script: String,
                                t: In = null,
                                ctxV: StdLibVersion = V1): Either[String, T] =
  ...
  def runScript[T <: EVALUATED](script: String,
                                tx: Transaction,
                                blockchain: Blockchain): Either[String, T] =
    runScript[T](script, V1, Coproduct(tx), blockchain, chainId)

  private def dropLastLine(str: String): String =
    str.replace("\r", "").split('\n').init.mkString("\n")

  def scriptWithAllFunctions(tx: DataTransaction,
                             t: TransferTransaction): String =
  ...
  def scriptWithPureFunctions(tx: DataTransaction,
                              t: TransferTransaction): String =
    s"""
       | # Pure context
       ...
       |      body + base64'${ByteStr(tx.bodyBytes.apply()).base64}' == base64'${ByteStr(
         tx.bodyBytes
           .apply()).base64}' + base64'${ByteStr(tx.bodyBytes.apply()).base64}'
  ...
  def scriptWithWavesFunctions(tx: DataTransaction,
                               t: TransferTransaction): String =
    s""" # Waves context
       | let txById = match tx {
       |     case _: DataTransaction => true
       ...
       |     let dataByKey = toString(int) == "${tx
         .data(0)
         .value}" || toString(bool) == "${tx.data(1).value}" ||
       ...
       |     let long = extract(getInteger(add,"${tx
         .data(0)
         .key}")) == ${tx.data(0).value}
       |     let bool1 = extract(getBoolean(add,"${tx
         .data(1)
         .key}")) == ${tx.data(1).value}
       |     let bin = extract(getBinary(add,"${tx
         .data(2)
         .key}")) ==  base58'${tx.data(2).value}'
       |     let str1 = extract(getString(add,"${tx
         .data(3)
         .key}")) == "${tx.data(3).value}"
       |     long && bool1 && bin && str1
       |
