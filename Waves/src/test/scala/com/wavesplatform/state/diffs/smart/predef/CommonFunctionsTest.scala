...
class CommonFunctionsTest
  ...
  property("size()") {
    ...
    runScript(s"size(base58'${ByteStr(arr).base58}')".stripMargin) shouldBe evaluated(
      3L)
  }
  ...
  property("getTransfer should extract MassTransfer transfers") {
    ...
    forAll(
      massTransferGen.retryUntil(
        tg =>
          tg.transfers.nonEmpty && tg.transfers
            .map(_.address)
            .forall(_.isInstanceOf[Address]))) {
      case massTransfer =>
        ...
        resultAddress shouldBe evaluated(
          massTransfer.transfers(0).address.bytes)
  ...
  property("general shadowing verification") {
    forAll(
      Gen.oneOf(transferV1Gen, transferV2Gen, issueGen, massTransferGen(10))) {
      case (transfer) =>
        val result = runScript(
          s"""
            |match tx {
            | case tx : TransferTransaction  => tx.id == base58'${transfer
               .id()
               .base58}'
        ...
        try {
          runScript(
            s"""
               |let t = 100
               |match tx {
               | case t: TransferTransaction  => t.id == base58'${transfer
                 .id()
                 .base58}'
          ...
          case ex: MatchError =>
            Assertions.assert(
              ex.getMessage()
                .contains(
                  "Compilation failed: Value 't' already defined in the scope"))
  ...
  property("shadowing of empty ref") {
    try {
      ...
      case ex: MatchError =>
        Assertions.assert(
          ex.getMessage()
            .contains("Compilation failed: A definition of 'p' is not found"))
      case _: Throwable => Assertions.fail("Some unexpected error")
    }
  }
  ...
  property("data constructors") {
    forAll(transferV2Gen, longEntryGen(dataAsciiKeyGen)) { (t, entry) =>
      val compareClause = t.recipient match {
        case addr: Address =>
          s"tx.recipient == Address(base58'${addr.address}')"
      ...
      val dataTx = DataTransaction
        .create(t.sender, List(entry), 100000L, t.timestamp, Proofs(Seq.empty))
        .explicitGet()
  ...
  property("data constructors bad syntax") {
    ...
    val cases = Seq(
      ...
      ("Address(base58'GzumLunBoK', 4)",
       "Function 'Address' requires 1 arguments, but 2 are provided"),
      ("Address()",
       "Function 'Address' requires 1 arguments, but 0 are provided"),
