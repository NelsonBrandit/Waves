...
class NotaryControlledTransferScenarioTest
    extends PropSpec
    with PropertyChecks
    with Matchers
    with TransactionGen
  ...
  val preconditions: Gen[(Seq[GenesisTransaction],
    ...
    for {
      ...
      genesis4 = GenesisTransaction
        .create(accountA, ENOUGH_AMT, ts)
        .explicitGet()
      genesis5 = GenesisTransaction
        .create(accountB, ENOUGH_AMT, ts)
        .explicitGet()
      ...
      kingDataTransaction = DataTransaction
        .selfSigned(
          king,
          List(BinaryDataEntry("notary1PK", ByteStr(notary.publicKey))),
          1000,
          ts + 1)
        .explicitGet()
      ...
      notaryDataTransaction = DataTransaction
        .selfSigned(notary,
                    List(BooleanDataEntry(transferFromAToB.id().base58, true)),
                    1000,
                    ts + 4)
        .explicitGet()
  ...
  property("Script toBase58String") {
    ...
    eval(s"""toBase58String(base58'$s') == \"$s\"""") shouldBe Testing
      .evaluated(true)
  }

  property("Script toBase64String") {
    val s = "Kl0pIkOM3tRikA=="
    eval(s"""toBase64String(base64'$s') == \"$s\"""") shouldBe Testing
      .evaluated(true)
  }

  property("addressFromString() returns None when address is too long") {
    val longAddress = "A" * (Global.MaxBase58String + 1)
    eval(s"""addressFromString("$longAddress")""") shouldBe Left(
      "base58Decode input exceeds 100")
  }
    ...
    forAll(preconditions) {
      case (genesis,
        ...
        assertDiffAndState(smartEnabledFS) { append =>
          ...
          append(Seq(issue, kingDataTransaction, transferFromCompanyToA))
            .explicitGet()
