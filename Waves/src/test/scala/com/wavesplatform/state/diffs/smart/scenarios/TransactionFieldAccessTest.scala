...
class TransactionFieldAccessTest
    extends PropSpec
    with PropertyChecks
    with Matchers
    with TransactionGen
  ...
  private def preconditionsTransferAndLease(
      code: String): Gen[(GenesisTransaction,
  ...
  property(
    "accessing field of transaction without checking its type first results on exception") {
    forAll(preconditionsTransferAndLease(script)) {
      case ((genesis, script, lease, transfer)) =>
        ...
        assertDiffEi(Seq(TestBlock.create(Seq(genesis, script))),
