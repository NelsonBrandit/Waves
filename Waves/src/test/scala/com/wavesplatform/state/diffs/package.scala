...
package object diffs extends WithState with Matchers {
  ...
  def assertDiffEi(preconditions: Seq[Block],
                   block: Block,
    ...
    withStateAndHistory(fs) { state =>
      ...
      preconditions.foreach { precondition =>
        val (preconditionDiff, preconditionFees, _) =
          differ(state, precondition).explicitGet()
  ...
  private def assertDiffAndState(
      ...
      withNg: Boolean)(assertion: (Diff, Blockchain) => Unit): Unit =
    withStateAndHistory(fs) { state =>
      def differ(
          blockchain: Blockchain,
          prevBlock: Option[Block],
          b: Block): Either[ValidationError, (Diff, Long, MiningConstraint)] =
        BlockDiffer.fromBlock(fs,
                              blockchain,
      ...
      val (diff, fees, _) =
        differ(state, preconditions.lastOption, block).explicitGet()
  ...
  def assertNgDiffState(preconditions: Seq[Block],
                        block: Block,
                        fs: FunctionalitySettings = TFS.Enabled)(
      assertion: (Diff, Blockchain) => Unit): Unit =
    assertDiffAndState(preconditions, block, fs, withNg = true)(assertion)

  def assertDiffAndState(preconditions: Seq[Block],
                         block: Block,
                         fs: FunctionalitySettings = TFS.Enabled)(
      assertion: (Diff, Blockchain) => Unit): Unit =
    assertDiffAndState(preconditions, block, fs, withNg = false)(assertion)

  def assertDiffAndState(fs: FunctionalitySettings)(
      test: (Seq[Transaction] => Either[ValidationError, Unit]) => Unit): Unit =
    withStateAndHistory(fs) { state =>
      def differ(blockchain: Blockchain, b: Block) =
        BlockDiffer.fromBlock(fs,
                              blockchain,
  ...
  def assertLeft(
      preconditions: Seq[Block],
      block: Block,
      fs: FunctionalitySettings = TFS.Enabled)(errorMessage: String): Unit =
