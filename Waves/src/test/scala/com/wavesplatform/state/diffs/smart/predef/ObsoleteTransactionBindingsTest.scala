...
class ObsoleteTransactionBindingsTest
    extends PropSpec
    with PropertyChecks
    with Matchers
    with TransactionGen
  ...
  def script(g: GenesisTransaction, p: PaymentTransaction): String =
    s"""
      ...
      |
      |     let bodyBytes = pay.bodyBytes == base64'${ByteStr(
         p.bodyBytes.apply()).base64}'
      |     let sender = pay.sender == addressFromPublicKey(base58'${ByteStr(
         p.sender.publicKey).base58}')
      |     let senderPublicKey = pay.senderPublicKey == base58'${ByteStr(
         p.sender.publicKey).base58}'
  ...
  val preconditionsAndPayments = for {
    ...
    payment = PaymentTransaction
      .create(master, recipient, ENOUGH_AMT * 2, fee, ts)
      .explicitGet()
    ...
    setScriptTransaction: SetScriptTransaction = SetScriptTransaction
      ...
      .explicitGet()
    nextTransfer <- transferGeneratorPV2(ts,
                                         recipient,
                                         master.toAddress,
                                         ENOUGH_AMT)
  ...
  val settings =
    TestFunctionalitySettings.Enabled.copy(blockVersion3AfterHeight = 100)
  property("Obsolete transaction bindings") {
    forAll(preconditionsAndPayments) {
      case ((genesis, payment, setScriptTransaction, nextTransfer)) =>
        assertDiffAndState(
          Seq(TestBlock.create(Seq(genesis, payment, setScriptTransaction))),
          TestBlock.create(Seq(nextTransfer)),
          settings) { (blockDiff, newState) =>
          ()
        }
    }
  }
