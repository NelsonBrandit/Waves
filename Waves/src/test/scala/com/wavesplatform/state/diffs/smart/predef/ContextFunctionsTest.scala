...
class ContextFunctionsTest
  ...
  def compactDataTransactionGen(sender: PrivateKeyAccount) =
    for {
      ...
      bin <- binaryEntryGen(40, dataAsciiKeyGen).filter(e =>
        e.key != long.key && e.key != bool.key)
      str <- stringEntryGen(40, dataAsciiKeyGen).filter(e =>
  ...
  val preconditionsAndPayments = for {
    ...
    ts <- positiveIntGen
    genesis1 = GenesisTransaction
      .create(master, ENOUGH_AMT * 3, ts)
      .explicitGet()
    genesis2 = GenesisTransaction
      .create(recipient, ENOUGH_AMT * 3, ts)
      .explicitGet()
    ...
    typedScript = {
      val compilerScript = ExpressionCompiler(
        compilerContext(V1, ContentType.Expression, isAssetScript = false),
        untypedScript).explicitGet()._1
    ...
    (Seq(genesis1, genesis2), setScriptTransaction, dataTransaction, transfer)
      ...
      case (_, _, tx, _) =>
        ...
        val result = runScript(
          s"""
               ...
               |  let okBin  = bin  == base58'${Base58.encode(
               bin.asInstanceOf[BinaryDataEntry].value.arr)}'
               |  let okStr  = str  == "${str.value}"
               |
               |  let okIntV  = int + 1  == ${int.value} + 1
               |  let okBoolV = bool || true == ${bool.value} || true
               |  let okBinV  = bin  == base58'${Base58.encode(
               bin.asInstanceOf[BinaryDataEntry].value.arr)}'
  ...
  property("reading from data transaction array by index") {
    forAll(preconditionsAndPayments, Gen.choose(4, 40)) {
      case ((_, _, tx, _), badIndex) =>
        ...
        outOfBounds shouldBe Left(
          s"java.lang.IndexOutOfBoundsException: $badIndex")
    }
  }
  ...
  property("base64 amplification") {
    val script =
      ...
      """.stripMargin
    runScript(script) shouldBe Left(
      s"base64Encode input exceeds ${Global.MaxBase64Bytes}")
  }
}
