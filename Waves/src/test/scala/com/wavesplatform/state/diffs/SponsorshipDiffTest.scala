...
class SponsorshipDiffTest
    ...
    forAll(setup) {
      case (genesis, issue, sponsor, sponsor1, cancel) =>
        ...
        assertDiffAndState(setupBlocks, block(Seq(sponsor)), s) {
          case (diff, state) =>
            diff.sponsorship shouldBe Map(
              sponsor.asset -> SponsorshipValue(
                sponsor.minSponsoredAssetFee.get))
            state
              .assetDescription(sponsor.asset)
              .map(_.sponsorship) shouldBe sponsor.minSponsoredAssetFee
        }
        assertDiffAndState(setupBlocks, block(Seq(sponsor, sponsor1)), s) {
          case (diff, state) =>
            diff.sponsorship shouldBe Map(
              sponsor.asset -> SponsorshipValue(
                sponsor1.minSponsoredAssetFee.get))
            state
              .assetDescription(sponsor.asset)
              .map(_.sponsorship) shouldBe sponsor1.minSponsoredAssetFee
        }
        assertDiffAndState(setupBlocks,
                           block(Seq(sponsor, sponsor1, cancel)),
                           s) {
          case (diff, state) =>
            diff.sponsorship shouldBe Map(sponsor.asset -> SponsorshipValue(0))
            state
              .assetDescription(sponsor.asset)
              .map(_.sponsorship) shouldBe Some(0)
        }
    }
  }
      ...
      case (genesis, issue, sponsor) =>
        ...
        assertDiffEi(setupBlocks, block(Seq(sponsor)), s) { blockDiffEi =>
          blockDiffEi should produce(
            "Fee Sponsorship feature has not been activated yet")
        }
    }
  }
  ...
  property("not enough fee") {
    ...
    forAll(setup) {
      case (genesis,
        ...
        assertDiffEi(setupBlocks, block(Seq(assetOverspend)), s) {
          blockDiffEi =>
            blockDiffEi should produce("unavailable funds")
        }
        assertDiffEi(setupBlocks, block(Seq(insufficientFee)), s) {
          blockDiffEi =>
            blockDiffEi should produce(
              "does not exceed minimal value of 100000 WAVES")
        }
        assertDiffEi(setupBlocks, block(Seq(wavesOverspend)), s) {
          blockDiffEi =>
  ...
  property("not enough waves to pay fee after leasing") {
    ...
    val setup = for {
      ...
      leasingTx: LeaseTransactionV1 = LeaseTransactionV1
        .selfSigned(master,
                    amount - issueTx.fee - sponsorTx.fee - 2 * fee,
                    fee,
                    ts + 3,
                    bob)
        .right
    ...
    forAll(setup) {
      case (genesis,
            ...
            insufficientFee,
            leasingToMaster) =>
        val setupBlocks = Seq(block(Seq(genesis, genesis2, issueTx, sponsorTx)),
                              block(Seq(transferAssetTx, leasingTx)))
        assertDiffEi(setupBlocks, block(Seq(insufficientFee)), s) {
          blockDiffEi =>
            blockDiffEi should produce("negative effective balance")
        }
        assertDiffEi(setupBlocks,
                     block(Seq(leasingToMaster, insufficientFee)),
                     s) { blockDiffEi =>
  ...
  property("cannot cancel sponsorship") {
    ...
    forAll(setup) {
      case (genesis, issueTx, sponsorTx, senderNotIssuer, insufficientFee) =>
        ...
        assertDiffEi(setupBlocks, block(Seq(senderNotIssuer)), s) {
          blockDiffEi =>
        ...
        assertDiffEi(setupBlocks, block(Seq(insufficientFee)), s) {
          blockDiffEi =>
            blockDiffEi should produce(
              "does not exceed minimal value of 100000000 WAVES: 99999999")
        }
    }
  }
  ...
  property("sponsor has no WAVES but receives them just in time") {
    ...
    val setup = for {
      ...
      ts <- timestampGen
      genesis: GenesisTransaction = GenesisTransaction
        .create(master, 300000000, ts)
        .explicitGet()
      issue = IssueTransactionV1
        .selfSigned(master,
                    Base58.decode("Asset").get,
                    Array.emptyByteArray,
                    100,
                    2,
                    reissuable = false,
      ...
      sponsor = SponsorFeeTransaction
        .selfSigned(master, assetId, Some(100), 100000000, ts + 2)
        .explicitGet()
    ...
    forAll(setup) {
      case (genesis,
        ...
        assertDiffAndState(
          Seq(
            block(Seq(genesis, issue, sponsor, assetTransfer, wavesTransfer))),
