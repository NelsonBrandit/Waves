...
class SetScriptTransactionDiffTest
  ...
  val preconditionsAndSetScript
    : Gen[(GenesisTransaction, SetScriptTransaction)] = for {
    ...
    (genesis,
     SetScriptTransaction.selfSigned(master, script, fee, ts).explicitGet())
  ...
  val preconditionsAndSetContract
    : Gen[(GenesisTransaction, SetScriptTransaction)] = for {
    version <- Gen.oneOf(SetScriptTransaction.supportedVersions.toSeq)
    master <- accountGen
    ts <- timestampGen
    ...
    script = ContractScript(
      ...
      Contract(
        List.empty,
        List(
          CallableFunction(CallableAnnotation("sender"),
                           Terms.FUNC("foo",
                                      List("a"),
                                      FUNCTION_CALL(Native(203),
                                                    List(REF("a"),
                                                         REF("sender")))))),
        None
      )
  ...
  property("Script with BlockV2 only works after Ride4DApps feature activation") {
    ...
    forAll(setup) {
      case (genesis, tx) =>
        assertDiffEi(Seq(block(Seq(genesis))),
                     block(Seq(tx)),
                     settingsUnactivated) { blockDiffEi =>
