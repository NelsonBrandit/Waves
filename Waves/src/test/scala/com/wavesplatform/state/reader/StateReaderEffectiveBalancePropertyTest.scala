...
class StateReaderEffectiveBalancePropertyTest
    ...
    with NoShrink {
  property(
    "No-interactions genesis account's effectiveBalance doesn't depend on depths") {
    ...
    forAll(setup) {
      case (genesis: GenesisTransaction,
        ...
        assertDiffAndState(genesisBlock +: nextBlocks, block(Seq.empty)) {
          (_, newState) =>
  ...
  property("Negative generating balance case") {
    val fs = Enabled.copy(
      preActivatedFeatures =
        Map(SmartAccounts.id -> 0, SmartAccountTrading.id -> 0))
    val Fee = 100000
    val setup = for {
      master <- accountGen
      ts <- positiveLongGen
      ...
      leaser <- accountGen
      xfer1 <- transferGeneratorPV2(ts + 1,
                                    master,
                                    leaser.toAddress,
                                    ENOUGH_AMT / 3)
      lease1 = LeaseTransactionV2
        .signed(leaser,
                xfer1.amount - Fee,
                Fee,
                ts + 2,
                ...
                leaser)
        .explicitGet()
      xfer2 <- transferGeneratorPV2(ts + 3,
                                    master,
                                    leaser.toAddress,
                                    ENOUGH_AMT / 3)
      lease2 = LeaseTransactionV2
        .signed(leaser,
                xfer2.amount - Fee,
                Fee,
                ts + 4,
    ...
    forAll(setup) {
      case (leaser, genesis, xfer1, lease1, xfer2, lease2) =>
        assertDiffAndState(Seq(block(Seq(genesis)), block(Seq(xfer1, lease1))),
          ...
          GeneratingBalanceProvider.balance(state,
                                            fs,
                                            leaser,
                                            state.lastBlockId.get) shouldBe 0
