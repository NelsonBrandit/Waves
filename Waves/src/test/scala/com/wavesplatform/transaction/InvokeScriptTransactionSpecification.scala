...
class InvokeScriptTransactionSpecification
    extends PropSpec
    with PropertyChecks
    with Matchers
    with TransactionGen {
    ...
    val tx = InvokeScriptTransaction
      .selfSigned(
        ...
        Terms.FUNCTION_CALL(
          ...
          List(Terms.CONST_BYTESTR(ByteStr(Base64.decode("YWxpY2U=").get)))),
        Seq(
          InvokeScriptTransaction.Payment(
            7,
            IssuedAsset(
              ByteStr
                .decodeBase58("73pu8pHFNpj9tmWuYjqnZ962tXzJvLGX86dxjZxGYhoK")
                .get))),
        100000,
        Waves,
        1526910778245L,
  ...
  property("Signed ContractInvocationTransactionRequest parser") {
    ...
    val req = SignedInvokeScriptRequest(
      ...
      fee = 1,
      feeAssetId = None,
      call = InvokeScriptRequest.FunctionCallPart(
        "bar",
        List(Terms.CONST_BYTESTR(ByteStr.decodeBase64("YWxpY2U=").get))),
      payment = Some(Seq(Payment(1, Waves))),
      contractAddress = "3Fb641A9hWy63K18KsBJwns64McmdEATgJd",
      timestamp = 11,
      proofs = List(
        "CC1jQ4qkuVfMvB2Kpg2Go6QKXJxUFC8UUswUxBsxwisrR8N5s3Yc8zA6dhjTwfWKfdouSTAnRXCxTXb3T6pJq3T")
    )
  ...
  property(s"can't have more than ${ContractLimits.MaxInvokeScriptArgs} args") {
    ...
    val pk = PublicKeyAccount
      .fromBase58String("73pu8pHFNpj9tmWuYjqnZ962tXzJvLGX86dxjZxGYhoK")
      .explicitGet()
    InvokeScriptTransaction.create(
      pk,
      pk.toAddress,
      Terms.FUNCTION_CALL(FunctionHeader.User("foo"),
                          Range(0, 23).map(_ => Terms.CONST_LONG(0)).toList),
      Seq(),
      1,
      Waves,
      1,
  ...
  property("can't be more 5kb") {
    ...
    InvokeScriptTransaction.create(
      ...
      pk.toAddress,
      Terms.FUNCTION_CALL(FunctionHeader.User("foo"),
                          List(Terms.CONST_STRING(largeString))),
      Seq(),
      1,
      Waves,
      1,
