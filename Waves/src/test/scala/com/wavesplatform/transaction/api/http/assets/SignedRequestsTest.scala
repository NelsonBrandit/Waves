...
class SignedRequestsTest extends FunSuite with Matchers {
  ...
  "minSponsoredAssetFee": null
    ...
    val req =
      Json.parse(js1).validate[SignedSponsorFeeRequest].get.toTx.right.get
    ...
    for (js <- Seq(js2, js3, js4)) {
      val req =
        Json.parse(js).validate[SignedSponsorFeeRequest].get.toTx.right.get
      Proofs(
        Seq(
          ByteStr
            .decodeBase58(
  ...
  test("SetAssetScriptRequest json parsing works") {
    ...
    val tx = req.toTx.explicitGet()
    tx.asset.id.base58 shouldBe "Ha35nwsnmYxHRF8UmKG3S523BycBLZFU4FZnjXryKd4L"
    tx.script shouldBe Some(
      Script
        .fromBase64String("base64:AQkAAGcAAAACAHho/EXujJiPAJUhuPXZYac+rt2jYg==")
        .explicitGet())
    tx.fee shouldBe 100000L
