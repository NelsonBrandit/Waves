...
class BlockchainUpdaterTest
  ...
  private val WavesSettings = history.DefaultWavesSettings.copy(
    blockchainSettings = history.DefaultWavesSettings.blockchainSettings.copy(
      functionalitySettings =
        history.DefaultWavesSettings.blockchainSettings.functionalitySettings
          .copy(
    ...
    featuresSettings = history.DefaultWavesSettings.featuresSettings
      .copy(autoShutdownOnUnsupportedFeature = true)
  )

  private val WavesSettingsWithDoubling = WavesSettings.copy(
    blockchainSettings = WavesSettings.blockchainSettings.copy(
      functionalitySettings =
        WavesSettings.blockchainSettings.functionalitySettings.copy(
  ...
  "features approved and accepted as height grows" in withDomain(WavesSettings) {
    domain =>
      ...
      b.featureStatus(3, ApprovalPeriod) shouldBe BlockchainFeatureStatus
        .Undefined
      ...
      b.featureStatus(3, 2 * ApprovalPeriod) shouldBe BlockchainFeatureStatus
        .Undefined
  ...
  "features rollback with block rollback" in withDomain(WavesSettings) {
    domain =>
      val b = domain.blockchainUpdater
      b.processBlock(genesisBlock)
      ...
      b.featureStatus(2, ApprovalPeriod - 1) shouldBe BlockchainFeatureStatus
        .Undefined
  ...
  "feature activation height is not overriden with further periods" in withDomain(
  ...
  "feature activated only by 90% of blocks" in withDomain(WavesSettings) {
    domain =>
      val b = domain.blockchainUpdater
      ...
      (1 until ApprovalPeriod).foreach { i =>
        b.processBlock(
            getNextTestBlockWithVotes(b, if (i % 2 == 0) Set(1) else Set()))
          .explicitGet()
      }
      b.featureStatus(1, ApprovalPeriod) shouldBe BlockchainFeatureStatus
        .Undefined

      (1 to ApprovalPeriod).foreach { i =>
        b.processBlock(
            getNextTestBlockWithVotes(b, if (i % 10 == 0) Set() else Set(1)))
          .explicitGet()
      }
      b.featureStatus(1, ApprovalPeriod * 2) shouldBe BlockchainFeatureStatus
        .Approved
  ...
  "features votes resets when voting window changes" in withDomain(
    WavesSettings) { domain =>
    val b = domain.blockchainUpdater
  ...
  "block processing should fail if unimplemented feature was activated on blockchain when autoShutdownOnUnsupportedFeature = yes and exit with code 38" in withDomain(
    ...
    b.processBlock(getNextTestBlockWithVotes(b, Set(-1))) should produce(
  ...
  "sunny day test when known feature activated" in withDomain(WavesSettings) {
    domain =>
      val b = domain.blockchainUpdater
      b.processBlock(genesisBlock)
  ...
  "empty blocks should not disable activation" in withDomain(WavesSettings) {
    domain =>
      val b = domain.blockchainUpdater
  ...
  "doubling of feature periods works in the middle of activation period" in withDomain(
    WavesSettingsWithDoubling) { domain =>
    val b = domain.blockchainUpdater
  ...
  "doubling of feature periods should work after defined height" in withDomain(
    WavesSettingsWithDoubling) { domain =>
    val b = domain.blockchainUpdater
