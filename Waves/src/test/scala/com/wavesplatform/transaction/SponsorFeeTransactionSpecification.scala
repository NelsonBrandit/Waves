...
class SponsorFeeTransactionSpecification
    extends PropSpec
    with PropertyChecks
    with Matchers
    with TransactionGen {
  val One = 100000000L
  val NgAndSponsorshipSettings = TestFunctionalitySettings.Enabled.copy(
 ...
 "minSponsoredAssetFee": null
  ...
  property("sponsorship negative fee") {
    forAll(invalidFee) { fee: Long =>
      for {
        ...
        SponsorFeeTransaction.selfSigned(sender,
                                         IssuedAsset(assetId),
                                         Some(minFee),
                                         fee,
                                         timestamp) should produce(
          "insufficient fee")
    }
  }
  ...
  property("miner receives one satoshi less than sponsor pays") {
    val setup = for {
      ...
      issue = IssueTransactionV1
        .selfSigned(acc, name, desc, quantity, decimals, reissuable, fee, ts)
        .explicitGet()
      minFee <- Gen.choose(1, issue.quantity)
      sponsor = SponsorFeeTransaction
        .selfSigned(acc, IssuedAsset(issue.id()), Some(minFee), One, ts)
        .explicitGet()
      transfer = TransferTransactionV1
        .selfSigned(Waves,
                    ...
                    1,
                    ts,
                    feeAssetId = IssuedAsset(issue.id()),
                    minFee,
                    Array())
        .explicitGet()
  ...
  property("miner receives one satoshi more than sponsor pays") {
    val setup = for {
      ...
      sponsor = SponsorFeeTransaction
        .selfSigned(acc, IssuedAsset(issue.id()), Some(minFee), One, ts)
        .explicitGet()
      transfer1 = TransferTransactionV1
        .selfSigned(Waves,
                    acc,
                    acc,
                    1,
                    ...
                    Array())
        .explicitGet()
      transfer2 = TransferTransactionV1
        .selfSigned(Waves,
                    acc,
                    acc,
                    1,
  ...
  property("sponsorship changes in the middle of a block") {
    val setup = for {
      ...
      sponsor1 = SponsorFeeTransaction
        .selfSigned(acc, IssuedAsset(issue.id()), Some(minFee), One, ts)
        .explicitGet()
      transfer1 = TransferTransactionV1
        .selfSigned(Waves,
                    ...
                    Array())
        .explicitGet()
      sponsor2 = SponsorFeeTransaction
        .selfSigned(acc, IssuedAsset(issue.id()), Some(minFee * 10), One, ts)
        .explicitGet()
      transfer2 = TransferTransactionV1
        .selfSigned(Waves,
                    acc,
