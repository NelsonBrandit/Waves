...
class DataTransactionSpecification
    extends PropSpec
    with PropertyChecks
    with Matchers
    with TransactionGen {
  ...
  property("JSON roundtrip") {
    implicit val signedFormat: Format[SignedDataRequest] =
      Json.format[SignedDataRequest]
    ...
    forAll(dataTransactionGen, dataEntryGen(500)) {
      case (DataTransaction(sender, data, fee, timestamp, proofs), entry) =>
        def check(data: List[DataEntry[_]]): Assertion = {
          val txEi =
            DataTransaction.create(sender, data, fee, timestamp, proofs)
          txEi shouldBe Right(
            DataTransaction(sender, data, fee, timestamp, proofs))
          checkSerialization(txEi.explicitGet())
        }

        check(List.empty) // no data
        check(List.tabulate(MaxEntryCount)(n =>
          IntegerDataEntry(n.toString, n))) // maximal data
        ...
        check(List(BinaryDataEntry(
          "bin",
          ByteStr(Array.fill(MaxValueSize)(1: Byte))))) // max binary value size
  ...
  property("negative validation cases") {
    forAll(dataTransactionGen) {
      case DataTransaction(sender, data, fee, timestamp, proofs) =>
        val dataTooBig = List.tabulate(100)(n =>
          StringDataEntry((100 + n).toString, "a" * 1527))
        val dataTooBigEi =
          DataTransaction.create(sender, dataTooBig, fee, timestamp, proofs)
        dataTooBigEi shouldBe Left(ValidationError.TooBigArray)

        val emptyKey = List(IntegerDataEntry("", 2))
        val emptyKeyEi =
          DataTransaction.create(sender, emptyKey, fee, timestamp, proofs)
        emptyKeyEi shouldBe Left(
          ValidationError.GenericError("Empty key found"))

        val keyTooLong = data :+ BinaryDataEntry("a" * (MaxKeySize + 1),
                                                 ByteStr(Array(1, 2)))
        val keyTooLongEi =
          DataTransaction.create(sender, keyTooLong, fee, timestamp, proofs)
        keyTooLongEi shouldBe Left(ValidationError.TooBigArray)

        val valueTooLong = data :+ BinaryDataEntry(
          "key",
          ByteStr(Array.fill(MaxValueSize + 1)(1: Byte)))
        val valueTooLongEi =
          DataTransaction.create(sender, valueTooLong, fee, timestamp, proofs)
        ...
        val duplicateKeysEi =
          DataTransaction.create(sender, duplicateKeys, fee, timestamp, proofs)
        duplicateKeysEi shouldBe Left(
          ValidationError.GenericError("Duplicate keys found"))

        val noFeeEi = DataTransaction.create(sender, data, 0, timestamp, proofs)
        noFeeEi shouldBe Left(ValidationError.InsufficientFee())

        val negativeFeeEi =
          DataTransaction.create(sender, data, -100, timestamp, proofs)
