...
class UserFunctionComplexityTest
    extends PropSpec
    with PropertyChecks
    with Matchers
    with TypedScriptGen {
  ...
  private def estimate(
      expr: EXPR,
      ctx: CTX,
      funcCosts: Map[FunctionHeader, Coeval[Long]]): Either[String, Long] = {
  ...
  property("estimate script for stdLib V1 with UserFunctions") {
    ...
    val exprNe =
      FUNCTION_CALL(PureContext.ne, List(CONST_LONG(1), CONST_LONG(2)))
    ...
    val exprDropRightBytes =
      FUNCTION_CALL(PureContext.dropRightBytes,
                    List(CONST_BYTESTR(ByteStr.fromLong(2)), CONST_LONG(1)))
    est(exprDropRightBytes).explicitGet() shouldBe 21

    val exprTakeRightBytes =
      FUNCTION_CALL(PureContext.takeRightBytes,
                    List(CONST_BYTESTR(ByteStr.fromLong(2)), CONST_LONG(1)))
    est(exprTakeRightBytes).explicitGet() shouldBe 21

    val exprDropRightString =
      FUNCTION_CALL(PureContext.dropRightString,
                    List(CONST_STRING("str"), CONST_LONG(1)))
    est(exprDropRightString).explicitGet() shouldBe 21

    val exprTakeRightString =
      FUNCTION_CALL(PureContext.takeRightString,
                    List(CONST_STRING("str"), CONST_LONG(1)))
    ...
    val exprAddressFromPublicKey =
      FUNCTION_CALL(User("addressFromPublicKey"),
                    List(CONST_BYTESTR(ByteStr.fromLong(2))))
    est(exprAddressFromPublicKey).explicitGet() shouldBe 83

    val exprAddressFromString =
      FUNCTION_CALL(User("addressFromString"), List(CONST_STRING("address")))
    est(exprAddressFromString).explicitGet() shouldBe 125

    val exprWavesBalance =
      FUNCTION_CALL(User("wavesBalance"), List(CONST_STRING("alias")))
  ...
  property("estimate script for stdLib V3 with UserFunctions") {
    ...
    val exprDataByIndex = LET_BLOCK(
      LET("arr",
          FUNCTION_CALL(PureContext.listConstructor,
                        List(CONST_STRING("str_1"), REF("nil")))),
