...
class ScriptCompilerV1Test extends PropSpec with PropertyChecks with Matchers {
  ...
  property("compile script with specified version") {
    ...
    ScriptCompiler(script, isAssetScript = false) shouldBe Right(
      (ExprScript(StdLibVersion.V1, expectedExpr).explicitGet(), 13))
  }

  property("use version 2 if not specified") {
    val script = scriptWithVersion(none)
    ScriptCompiler(script, isAssetScript = false) shouldBe Right(
      (ExprScript(StdLibVersion.V2, expectedExpr).explicitGet(), 13))
  }

  property("fails on unsupported version") {
    val script = scriptWithVersion("8".some)
    ScriptCompiler(script, isAssetScript = false) shouldBe Left(
  ...
  property("fails with right error position") {
    ...
    ScriptCompiler.compile(script) shouldBe Left(
      "Compilation failed: A definition of 'b' is not found in 72-73")
  }
  ...
  property("fails with contract for asset") {
    val script =
      """
        ...
        | {-# SCRIPT_TYPE ASSET #-}
      """.stripMargin
    ScriptCompiler.compile(script) should produce(
      "Inconsistent set of directives")
  }
