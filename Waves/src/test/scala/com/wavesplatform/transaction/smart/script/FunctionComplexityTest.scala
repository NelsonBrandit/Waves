...
class FunctionComplexityTest
  ...
  property("estimate script with with all functions") {
    val exprV1 = ExpressionCompiler(ctxV1.compilerContext,
                                    getAllFuncExpression()).explicitGet()._1
    estimate(exprV1, ctxV1, utils.functionCosts(StdLibVersion.V1)) shouldBe Right(
      2317)

    val exprV2 = ExpressionCompiler(ctxV2.compilerContext,
                                    getAllFuncExpression()).explicitGet()._1
    estimate(exprV2, ctxV2, utils.functionCosts(StdLibVersion.V2)) shouldBe Right(
      2317)

    val exprV3 = ExpressionCompiler(ctxV3.compilerContext,
                                    getAllFuncExpression()).explicitGet()._1
    estimate(exprV3, ctxV3, utils.functionCosts(StdLibVersion.V3)) shouldBe Right(
      2282)
  }
}
