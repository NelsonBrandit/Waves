...
class LeaseTransactionSpecification
  ...
  property("Lease transaction serialization roundtrip") {
    forAll(leaseGen) { tx: LeaseTransaction =>
      val recovered =
        tx.builder.parseBytes(tx.bytes()).get.asInstanceOf[LeaseTransaction]
  ...
  private def assertTxs(first: LeaseTransaction,
                        second: LeaseTransaction): Unit = {
  ...
  property("JSON format validation for LeaseTransactionV1") {
    ...
    val tx = LeaseTransactionV1
      .create(
        ...
        ByteStr
          .decodeBase58(
            "iy3TmfbFds7pc9cDDqfjEJhfhVyNtm3GcxoVz8L3kJFvgRPUmiqqKLMeJGYyN12AhaQ6HvE7aF1tFgaAoCCgNJJ")
          .get
      )
      .right
      .get
  ...
  property("forbid assetId in LeaseTransactionV2") {
    ...
    forAll(leaseV2Gen, assetIdBytesGen) { (tx, assetId) =>
      ...
      parsed.failed.get.getMessage
        .contains("Leasing assets is not supported yet") shouldBe true
    }
  }
}
