...
class ExchangeTransactionSpecification
  ...
  val preconditions: Gen[(PrivateKeyAccount,
    ...
    for {
      ...
      buyerMatcherFeeAssetId <- Gen.oneOf(pair.amountAsset,
                                          pair.priceAsset,
                                          buyerAnotherAsset)
      sellerMatcherFeeAssetId <- Gen.oneOf(pair.amountAsset,
                                           pair.priceAsset,
                                           sellerAnotherAsset)
      ...
      (sender1,
       ...
       matcher,
       pair,
       buyerMatcherFeeAssetId,
       sellerMatcherFeeAssetId,
       versions)
  ...
  property("ExchangeTransaction balance changes") {
    ...
    forAll(preconditions) {
      case (sender1,
        ...
        create(buyOrder = buy.updateType(OrderType.SELL)) shouldBe an[Left[_,
                                                                           _]]
        create(buyOrder = buy.updateAmount(0)) shouldBe an[Left[_, _]]
        create(buyOrder = buy.updateAmount(-1)) shouldBe an[Left[_, _]]
        create(buyOrder = buy.updateAmount(Order.MaxAmount + 1)) shouldBe an[
          Left[_, _]]
        create(
          buyOrder = buy.updatePair(buy.assetPair.copy(
            amountAsset = sell.assetPair.priceAsset))) shouldBe an[Left[_, _]]
        create(buyOrder = buy.updateExpiration(1L)) shouldBe an[Left[_, _]]
        create(buyOrder = buy.updateExpiration(buy.expiration + 1)) shouldBe an[
          Left[_, _]]
        create(buyOrder = buy.updatePrice(-1)) shouldBe an[Left[_, _]]
        create(buyOrder = buy.updatePrice(price = sellPrice - 1)) shouldBe an[
          Left[_, _]]
        create(buyOrder = buy.updateMatcher(sender2)) shouldBe an[Left[_, _]]

        create(sellOrder = sell.updateType(OrderType.BUY)) shouldBe an[Left[_,
                                                                            _]]
        create(sellOrder = sell.updateAmount(0)) shouldBe an[Left[_, _]]
        create(sellOrder = sell.updateAmount(-1)) shouldBe an[Left[_, _]]
        create(sellOrder = sell.updateAmount(Order.MaxAmount + 1)) shouldBe an[
          Left[_, _]]
        create(
          sellOrder = sell.updatePair(sell.assetPair.copy(
            priceAsset = buy.assetPair.amountAsset))) shouldBe an[Left[_, _]]
        create(sellOrder = sell.updateExpiration(1L)) shouldBe an[Left[_, _]]
        create(sellOrder = sell.updateExpiration(sell.expiration + 1)) shouldBe an[
          Left[_, _]]
        create(sellOrder = sell.updatePrice(-1)) shouldBe an[Left[_, _]]
        create(sellOrder = sell.updatePrice(price = buyPrice + 1)) shouldBe an[
          Left[_, _]]
        ...
        create(
          ...
          sellOrder = sell.updatePair(
            sell.assetPair.copy(
              priceAsset = IssuedAsset(ByteStr(Array(1: Byte)))))
  ...
  def createExTx(
      ...
      sell: Order,
      price: Long,
      matcher: PrivateKeyAccount,
      version: Byte): Either[ValidationError, ExchangeTransaction] = {
    val mf = 300000L
  ...
  property("Test transaction with small amount and expired order") {
    ...
    forAll(preconditions) {
      case (sender1,
        ...
        createExTx(buy, sell1, buyPrice, matcher, tver) shouldBe Left(
          OrderValidationError(sell1, "expiration should be > currentTime"))
    }
  }
  ...
  property("JSON format validation") {
    ...
    val buy = OrderV1(
      PublicKeyAccount
        ...
        .explicitGet(),
      PublicKeyAccount
        .fromBase58String("Fvk5DXmfyWVZqQVBowUBMwYtRAHDtdyZNNeRrwSjt6KP")
        .explicitGet(),
      AssetPair
        .createAssetPair("WAVES",
    ...
    val sell = OrderV1(
      PublicKeyAccount
        .fromBase58String("7E9Za8v8aT6EyU1sX91CVK7tWUeAetnNYDxzKZsyjyKV")
        .explicitGet(),
      PublicKeyAccount
        .fromBase58String("Fvk5DXmfyWVZqQVBowUBMwYtRAHDtdyZNNeRrwSjt6KP")
        .explicitGet(),
