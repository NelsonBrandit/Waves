...
class ReissueTransactionV2Specification
    extends GenericTransactionSpecification[ReissueTransactionV2] {

  def transactionParser
    : com.wavesplatform.transaction.TransactionParserFor[ReissueTransactionV2] =
    ReissueTransactionV2

  def updateProofs(tx: ReissueTransactionV2,
                   p: Proofs): ReissueTransactionV2 = {
    tx.copy(proofs = p)
  }

  def assertTxs(first: ReissueTransactionV2,
                second: ReissueTransactionV2): Unit = {
  ...
  def generator: Gen[
    ((Seq[com.wavesplatform.transaction.Transaction], ReissueTransactionV2))] =
    for {
    ...
    """),
       ReissueTransactionV2
         .create(
           ...
           IssuedAsset(
             ByteStr
               .decodeBase58("9ekQuYn92natMnMq8KqeGK3Nn7cpKd3BvPEGgD6fFyyz")
               .get),
           100000000L,
           true,
           100000000L,
           1526287561757L,
           Proofs(Seq(ByteStr
             .decodeBase58("4DFEtUwJ9gjMQMuEXipv2qK7rnhhWEBqzpC3ZQesW1Kh8D822t62e3cRGWNU3N21r7huWnaty95wj2tZxYSvCfro")
             .get))
         )
         .right
         .get))
