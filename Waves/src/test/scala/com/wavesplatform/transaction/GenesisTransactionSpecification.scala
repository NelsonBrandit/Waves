...
class GenesisTransactionSpecification
    extends PropSpec
    with PropertyChecks
    with Matchers {
  ...
  property("GenesisTransaction Signature should be the same") {
    ...
    val signature =
      GenesisTransaction.generateSignature(defaultRecipient, balance, timestamp)

    val expected =
      "3L4zhpN1o6TysvM8FZFv1NmSEjpGSgV4V71e2iJwseFrrt65aZJiyXwqj5WpigLAn296sUrFb9yN8fdsY7GSdwwR"
  ...
  property("GenesisTransaction parse from Bytes should work fine") {
    val bytes = Base58
      .decode("5GoidXWjBfzuS9tZm4Fp6GAXUYFunVMsoWAew3eBnDbmaDi7WiP9yVpBD2")
      .get
    ...
    val expectedTransaction = GenesisTransaction
      .create(defaultRecipient, balance, timestamp)
      .explicitGet()
  ...
  property("GenesisTransaction serialize/deserialize roundtrip") {
    forAll(Gen.listOfN(32, Arbitrary.arbitrary[Byte]).map(_.toArray),
           Gen.posNum[Long],
           Gen.posNum[Long]) {
      (recipientSeed: Array[Byte], time: Long, amount: Long) =>
        val recipient = PrivateKeyAccount(recipientSeed)
        val source =
          GenesisTransaction.create(recipient, amount, time).explicitGet()
