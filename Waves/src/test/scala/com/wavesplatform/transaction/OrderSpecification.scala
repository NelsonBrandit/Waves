...
class OrderSpecification
  ...
  private def checkFieldsEquality(left: Order, right: Order): Assertion = {
    ...
    (left, right) match {
      case (l: OrderV3, r: OrderV3) =>
        defaultChecks; l.matcherFeeAssetId shouldBe r.matcherFeeAssetId
  ...
  property("Order expiration validation") {
    forAll(arbitraryOrderGen) { order =>
      ...
      val time = ntpTime.correctedTime()
      whenever(
        order.expiration < time || order.expiration > time + Order.MaxLiveTime) {
  ...
  property("Order signature validation") {
    ...
    forAll(orderGen, accountGen) {
      case (order, pka) =>
        ...
        Verifier.verifyAsEllipticCurveSignature(order.updateSender(pka)) should produce(
          err)
        Verifier.verifyAsEllipticCurveSignature(order.updateMatcher(pka)) should produce(
          err)
        val assetPair = order.assetPair
        Verifier.verifyAsEllipticCurveSignature(
          order
            .updatePair(assetPair.copy(
              amountAsset = IssuedAsset(ByteStr(rndAsset))))) should produce(
          err)
        Verifier.verifyAsEllipticCurveSignature(
          order
            .updatePair(assetPair.copy(
              priceAsset = IssuedAsset(ByteStr(rndAsset))))) should produce(err)
        Verifier.verifyAsEllipticCurveSignature(
          order.updateType(OrderType.reverse(order.orderType))) should produce(
          err)
        Verifier.verifyAsEllipticCurveSignature(
          order.updatePrice(order.price + 1)) should produce(err)
        Verifier.verifyAsEllipticCurveSignature(
          order.updateAmount(order.amount + 1)) should produce(err)
        Verifier.verifyAsEllipticCurveSignature(
          order.updateExpiration(order.expiration + 1)) should produce(err)
        Verifier.verifyAsEllipticCurveSignature(
          order.updateFee(order.matcherFee + 1)) should produce(err)
        Verifier.verifyAsEllipticCurveSignature(order.updateProofs(
          Proofs(Seq(ByteStr(pka.publicKey ++ pka.publicKey))))) should produce(
          err)
    }
  }
