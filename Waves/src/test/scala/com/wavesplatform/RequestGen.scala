...
trait RequestGen extends TransactionGen { _: Suite =>
  val nonPositiveLong: G[Long] =
    choose(Long.MinValue, 0).label("non-positive value")
  ...
  val longAttachment: G[String] =
    genBoundedBytes(TransferTransaction.MaxAttachmentSize + 10,
                    TransferTransaction.MaxAttachmentSize + 50)
      .map(Base58.encode)
  val invalidBase58: G[String] =
    listOfN(50, oneOf(alphaNumChar, oneOf('O', '0', 'l')))
      .map(_.mkString)
      .label("invalid base58")
  val invalidName: G[String] = oneOf(
    genBoundedString(0, IssueTransaction.MinAssetNameLength - 1),
    genBoundedString(IssueTransaction.MaxAssetNameLength + 1,
                     IssueTransaction.MaxAssetNameLength + 50)
  ).map(new String(_))
  ...
  val longDescription: G[String] =
    genBoundedBytes(IssueTransaction.MaxDescriptionLength + 1,
                    IssueTransaction.MaxDescriptionLength + 50)
      .map(Base58.encode)

  val addressGen: G[String] =
    listOfN(32, Arbitrary.arbByte.arbitrary).map(b => Base58.encode(b.toArray))
  val signatureGen: G[String] =
    listOfN(SignatureLength, Arbitrary.arbByte.arbitrary)
  ...
  val issueReq: G[IssueV1Request] = for {
    ...
    name <- genBoundedString(IssueTransaction.MinAssetNameLength,
  ...
  val broadcastIssueReq: G[SignedIssueV1Request] = for {
    ...
    SignedIssueV1Request(_ir.sender,
                         _ir.name,
                         _ir.description,
                         _ir.quantity,
                         _ir.decimals,
  ...
  val broadcastReissueReq: G[SignedReissueV1Request] = for {
    ...
    SignedReissueV1Request(_rr.sender,
                           _rr.assetId,
                           _rr.quantity,
                           _rr.reissuable,
                           _rr.fee,
  ...
  val broadcastBurnReq: G[SignedBurnV1Request] = for {
    ...
    SignedBurnV1Request(_br.sender,
                        _br.assetId,
                        _br.quantity,
                        _br.fee,
                        _timestamp,
  ...
  val broadcastTransferReq: G[SignedTransferV1Request] = for {
    ...
    SignedTransferV1Request(_tr.sender,
                            _tr.assetId,
                            _tr.recipient,
                            _tr.amount,
                            _tr.fee,
  ...
  val createAliasReq: G[SignedCreateAliasV1Request] = for {
    ...
    SignedCreateAliasV1Request(_alias.sender.toString,
                               _alias.fee,
                               _alias.alias.name,
                               _timestamp,
                               _signature)
  ...
  val leaseReq: G[SignedLeaseV1Request] = for {
    ...
    SignedLeaseV1Request(_alias.sender.toString,
                         _alias.amount,
                         _alias.fee,
                         _alias.recipient.toString,
                         _timestamp,
                         _signature)
  ...
  val leaseCancelReq: G[SignedLeaseCancelV1Request] = for {
    ...
    SignedLeaseCancelV1Request(_cancel.sender.toString,
                               _cancel.leaseId.base58,
                               _cancel.timestamp,
                               _signature,
                               _cancel.fee)
}
