...
class ObservedLoadingCacheSpecification
  ...
  "notifies" - {
    "on refresh" in test { (loadingCache, changes, _) =>
      (changes.onNext _)
        .expects("foo")
        .returning(Future.successful(Ack.Continue))
        .once()
  ...
  private def test(
      f: (LoadingCache[String, Integer], Observer[String], FakeTicker) => Unit)
    : Unit = {
...
private object ObservedLoadingCacheSpecification {
  ...
  class FakeTicker extends Ticker {
    ...
    def advance(time: Long, timeUnit: TimeUnit): FakeTicker =
      advance(timeUnit.toNanos(time))
    def advance(nanoseconds: Long): FakeTicker = {
      nanos.addAndGet(nanoseconds)
      this
    }

    def setAutoIncrementStep(autoIncrementStep: Long,
                             timeUnit: TimeUnit): FakeTicker = {
      require(autoIncrementStep >= 0,
