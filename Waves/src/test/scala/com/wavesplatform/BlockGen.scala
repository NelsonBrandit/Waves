...
trait BlockGen extends TransactionGen { _: Suite =>
  ...
  val blockParamGen: Gen[(Seq[ProvenTransaction], PrivateKeyAccount)] = for {
    count <- Gen.choose(minTransactionsInBlockCount,
                        maxTransactionsInBlockCount)
    transactions <- randomTransactionsGen(count)
    signer <- accountGen
  } yield (transactions, signer)

  def versionedBlockGen(txs: Seq[Transaction],
                        signer: PrivateKeyAccount,
                        version: Byte): Gen[Block] =
    byteArrayGen(Block.BlockIdLength).flatMap(ref =>
      versionedBlockGen(ByteStr(ref), txs, signer, version))

  def versionedBlockGen(reference: ByteStr,
                        txs: Seq[Transaction],
                        signer: PrivateKeyAccount,
                        version: Byte): Gen[Block] =
    for {
  ...
  def blockGen(txs: Seq[Transaction], signer: PrivateKeyAccount): Gen[Block] =
    versionedBlockGen(txs, signer, 1)
  ...
  val randomBlocksSeqGen: Gen[(Int, Int, Seq[Block])] = blocksSeqGen(
    randomSignerBlockGen)

  val mixedBlocksSeqGen: Gen[(Int, Int, Seq[Block])] = blocksSeqGen(
    mixedBlockGen)
...
object BlockGen {
  ...
  val predefinedSignerPrivateKey: PrivateKeyAccount = PrivateKeyAccount(
