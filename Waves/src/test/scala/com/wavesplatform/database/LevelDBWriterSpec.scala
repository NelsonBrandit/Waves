...
class LevelDBWriterSpec
    ...
    "correctly joins height ranges" in {
      val fs = Enabled.copy(
        preActivatedFeatures =
          Map(BlockchainFeatures.SmartAccountTrading.id -> 0))
      val writer = new LevelDBWriter(db,
                                     ...
                                     2000,
                                     120 * 60 * 1000)
      writer.merge(Seq(15, 12, 3), Seq(12, 5)) shouldEqual Seq((15, 12),
                                                               (12, 12),
                                                               (3, 5))
      writer.merge(Seq(12, 5), Seq(15, 12, 3)) shouldEqual Seq((12, 15),
                                                               (12, 12),
                                                               (5, 3))
  ...
  "hasScript" - {
    ...
    def resetGen(ts: Long): Gen[(PrivateKeyAccount, Seq[Block])] =
      baseGen(ts).map {
        case (master, blocks) =>
          ...
          val block1 =
            TestBlock.create(ts + 1, blocks.last.uniqueId, Seq(unsetScriptTx))
          val block2 = TestBlock.create(ts + 2, block1.uniqueId, Seq())
          (master, blocks ++ List(block1, block2))
      }

    def baseGen(ts: Long): Gen[(PrivateKeyAccount, Seq[Block])] =
      accountGen.map { master =>
        val genesisTx =
          GenesisTransaction.create(master, ENOUGH_AMT, ts).explicitGet()
        val setScriptTx = SetScriptTransaction
          .selfSigned(master,
                      Some(ExprScript(Terms.TRUE).explicitGet()),
                      5000000,
                      ts)
          .explicitGet()
    ...
    def test(f: (LevelDBWriter, PrivateKeyAccount) => Unit): Unit =
      baseTest(t => gen(t.correctedTime()))(f)

    def resetTest(f: (LevelDBWriter, PrivateKeyAccount) => Unit): Unit =
      baseTest(t => resetGen(t.correctedTime()))(f)

  }

  def baseTest(gen: Time => Gen[(PrivateKeyAccount, Seq[Block])])(
      f: (LevelDBWriter, PrivateKeyAccount) => Unit): Unit = {
    val defaultWriter = new LevelDBWriter(db,
                                          ignoreSpendableBalanceChanged,
                                          TestFunctionalitySettings.Stub,
                                          maxCacheSize,
                                          2000,
    ...
    val settings = settings0.copy(
      featuresSettings = settings0.featuresSettings.copy(
  ...
  def testWithBlocks(gen: Time => Gen[(PrivateKeyAccount, Seq[Block])])(
      f: (LevelDBWriter, Seq[Block], PrivateKeyAccount) => Unit): Unit = {
    val defaultWriter = new LevelDBWriter(db,
                                          ignoreSpendableBalanceChanged,
                                          TestFunctionalitySettings.Stub,
                                          100000,
  ...
  def createTransfer(master: PrivateKeyAccount,
                     recipient: Address,
                     ts: Long): TransferTransaction = {
    TransferTransactionV1
      .selfSigned(Waves,
                  master,
                  recipient,
  ...
  def preconditions(ts: Long): Gen[(PrivateKeyAccount, List[Block])] = {
    for {
      ...
      genesisBlock = TestBlock
        .create(
          ts,
          Seq(GenesisTransaction.create(master, ENOUGH_AMT, ts).explicitGet()))
      block1 = TestBlock
        .create(
          ts + 3,
  ...
  "correctly reassemble block from header and transactions" in {
    val rw = new LevelDBWriter(db,
                               ...
                               120 * 60 * 1000)
    val settings0 = WavesSettings.fromConfig(loadConfig(ConfigFactory.load()))
    val settings = settings0.copy(featuresSettings =
      settings0.featuresSettings.copy(autoShutdownOnUnsupportedFeature = false))
    val bcu = new BlockchainUpdaterImpl(rw,
                                        ignoreSpendableBalanceChanged,
                                        settings,
                                        ntpTime)
    try {
        ...
        for {
          ...
          genesisBlock = TestBlock
            .create(ts,
                    Seq(
                      GenesisTransaction
                        .create(leaser, ENOUGH_AMT, ts)
                        .explicitGet()))
          leases <- Gen.listOfN(
            100,
            for {
              rec <- accountGen
              lease <- createLease(leaser,
                                   ENOUGH_AMT / 1000,
                                   1 * 10 ^ 8,
                                   ts,
                                   rec.toAddress)
  ...
  "addressTransactions" - {
    ...
    "return txs in correct ordering without fromId" in {
      baseTest(time => preconditions(time.correctedTime())) {
        (writer, account) =>
          val txs = writer
            .addressTransactions(account.toAddress,
                                 Set(TransferTransactionV1.typeId),
                                 3,
                                 None)
            .explicitGet()

          val ordering = Ordering
            .by[(Int, Transaction), (Int, Long)]({
              case (h, t) => (-h, -t.timestamp)
            })
    ...
    "return Left if fromId argument is a non-existent transaction" in {
      baseTest(time => preconditions(time.correctedTime())) {
        (writer, account) =>
          val nonExistentTxId =
            GenesisTransaction.create(account, ENOUGH_AMT, 1).explicitGet().id()

          val txs = writer
            .addressTransactions(account.toAddress,
                                 Set(TransferTransactionV1.typeId),
                                 3,
                                 Some(nonExistentTxId))
