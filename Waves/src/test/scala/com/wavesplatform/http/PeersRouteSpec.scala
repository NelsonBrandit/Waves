...
class PeersRouteSpec
  ...
  private val inetAddressGen = Gen
    .listOfN(4, Arbitrary.arbitrary[Byte])
    .map(_.toArray)
    .map(InetAddress.getByAddress)
  ...
  private def genListOf[A](maxLength: Int, src: Gen[A]) =
    Gen.chooseNum(0, maxLength).flatMap(n => Gen.listOfN(n, src))
  ...
  routePath("/connected") in {
    ...
    forAll(genListOf(TestsCount, gen)) { l: List[PeerInfo] =>
      ...
      val route = PeersApiRoute(restAPISettings,
                                connectToPeer,
                                peerDatabase,
                                connections).route
  ...
  routePath("/all") in {
    ...
    forAll(genListOf(TestsCount, gen)) { m =>
      (peerDatabase.knownPeers _)
        .expects()
        .returning(m.toMap[InetSocketAddress, Long])
      val route =
        PeersApiRoute(restAPISettings,
                      connectToPeer,
                      peerDatabase,
                      new ConcurrentHashMap[Channel, PeerInfo]()).route
  ...
  routePath("/connect") in {
    ...
    Post(connectUri, ConnectReq("example.com", 1)) ~> route should produce(
      ApiKeyNotValid)
    Post(connectUri, "") ~> api_key(apiKey) ~> route ~> check(
      handled shouldEqual false)
    Post(connectUri, Json.obj()) ~> api_key(apiKey) ~> route ~> check {
      (responseAs[JsValue] \ "validationErrors")
        .as[JsObject]
    ...
    val result = Post(connectUri,
                      ConnectReq(address.getHostName, address.getPort)) ~> api_key(
      apiKey) ~> route ~> runRoute
    check {
