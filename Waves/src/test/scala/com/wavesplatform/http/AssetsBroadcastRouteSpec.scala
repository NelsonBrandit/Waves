...
class AssetsBroadcastRouteSpec
  ...
  (utx.putIfNew _)
    .when(*)
    .onCall((t: Transaction) =>
      Left(TransactionValidationError(GenericError("foo"), t)))
    .anyNumberOfTimes()
    ...
    val vt = Table[String, G[_ <: Transaction], JsValue => JsValue](
      ...
      ("transfer", transferV1Gen, {
        case o: JsObject if o.value.contains("feeAsset") =>
          o ++ Json.obj("feeAssetId" -> o.value("feeAsset"),
                        "quantity" -> o.value("amount"))
        case other => other
      })
    )

    def posting(url: String, v: JsValue): RouteTestResult =
      Post(routePath(url), v) ~> route

    "when state validation fails" in {
      forAll(vt) { (url, gen, transform) =>
        forAll(gen) { t: Transaction =>
          posting(url, transform(t.json())) should produce(
            StateCheckFailed(t, "foo"))
        }
      }
    }
  }
  ...
  "returns appropriate error code when validation fails for" - {
    ...
    "reissue transaction" in {
      ...
      forAll(broadcastReissueReq) { rr =>
        def posting[A: Writes](v: A): RouteTestResult =
          Post(routePath("reissue"), v) ~> route

        // todo: invalid sender
        forAll(nonPositiveLong) { q =>
          posting(rr.copy(quantity = q)) should produce(
            NegativeAmount(s"$q of assets"))
        }
        forAll(nonPositiveLong) { fee =>
    ...
    "transfer transaction" in {
      ...
      forAll(broadcastTransferReq) { tr =>
        def posting[A: Writes](v: A): RouteTestResult =
          Post(routePath("transfer"), v) ~> route

        forAll(nonPositiveLong) { q =>
          posting(tr.copy(amount = q)) should produce(
            NegativeAmount(s"$q of ${tr.assetId.getOrElse("waves")}"))
        }
        forAll(invalidBase58) { pk =>
  ...
  "compatibility" - {
    ...
    (alwaysApproveUtx.putIfNew _)
      .when(*)
      .onCall((_: Transaction) => Right((true, Diff.empty)))
      .anyNumberOfTimes()
    ...
    val route = AssetsBroadcastApiRoute(settings,
                                        alwaysApproveUtx,
                                        alwaysSendAllChannels).route
    ...
    "/transfer" - {
      ...
      "accepts TransferRequest" in posting(transferRequest) ~> check {
        ...
        responseAs[TransferTransactions]
          .select[TransferTransactionV1] shouldBe defined
      }

      "accepts VersionedTransferRequest" in posting(versionedTransferRequest) ~> check {
        status shouldBe StatusCodes.OK
        responseAs[TransferTransactions]
          .select[TransferTransactionV2] shouldBe defined
      }

      "returns a error if it is not a transfer request" in posting(
        issueReq.sample.get) ~> check {
    ...
    "/batch-transfer" - {
      def posting[A: Writes](v: A): RouteTestResult =
        Post(routePath("batch-transfer"), v).addHeader(ApiKeyHeader) ~> route
      ...
      "accepts VersionedTransferRequest" in posting(
        List(versionedTransferRequest)) ~> check {
        status shouldBe StatusCodes.OK
        val xs = responseAs[Seq[TransferTransactions]]
        xs.size shouldBe 1
      ...
      "returns a error if it is not a transfer request" in posting(
        List(issueReq.sample.get)) ~> check {
        status shouldBe StatusCodes.BadRequest
      }
    }
  ...
  protected def createSignedTransferRequest(
      tx: TransferTransactionV1): SignedTransferV1Request = {
    import tx._
  ...
  protected def createSignedVersionedTransferRequest(
      tx: TransferTransactionV2): SignedTransferV2Request = {
    import tx._
