...
class AliasBroadcastRouteSpec
  ...
  "returns appropriate error code when validation fails for" - {
    ...
    "create alias transaction" in forAll(createAliasReq) { req =>
      ...
      def posting(v: JsValue): RouteTestResult =
        Post(routePath("create"), v) ~> route

      forAll(invalidBase58) { s =>
        posting(toJson(req.copy(senderPublicKey = s))) should produce(
          InvalidAddress)
      }
      ...
      forAll(invalidAliasStringByLength) { q =>
        ...
        posting(obj) should produce(
          CustomValidationError(
            s"Alias '$q' length should be between 4 and 30"))
      }
    }
  }
}
