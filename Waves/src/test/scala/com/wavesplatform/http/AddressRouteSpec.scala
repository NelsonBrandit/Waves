...
class AddressRouteSpec
  ...
  private val generatedMessages = for {
    ...
    message <- Gen
      .listOfN(length, Gen.alphaNumChar)
      .map(_.mkString)
      .label("message")
    ...
    forAll(generatedMessages) {
      case (account, message) =>
        ...
        Post(uri, message) ~> api_key(apiKey) ~> route ~> check {
          ...
          (resp \ "message")
            .as[String] shouldEqual (if (encode) Base58.encode(message.getBytes)
                                     else message)
          (resp \ "publicKey").as[String] shouldEqual Base58.encode(
            account.publicKey)
  ...
  private def testVerify(path: String, encode: Boolean): Unit = {
    ...
    forAll(
      generatedMessages.flatMap(m => Gen.oneOf(true, false).map(b => (m, b)))) {
      case ((account, message), b58) =>
        ...
        val validBody = Json.obj(
          "message" -> JsString(
            if (encode)
              if (b58) Base58.encode(messageBytes)
              else ("base64:" ++ Base64.encode(messageBytes))
            else message),
  ...
  routePath("") in {
    ...
    Post(routePath("")) ~> api_key(apiKey) ~> route ~> check {
      allAddresses should not contain (responseAs[JsObject] \ "address")
        .as[String]
    }
  }
  ...
  routePath(s"/scriptInfo/${allAddresses(1)}") in {
    (blockchain.accountScript _)
      ...
      .onCall((_: Address) => Some(ExprScript(TRUE).explicitGet()))
    ...
    val testContract =
      Contract(List(),
               List(),
               Some(
                 VerifierFunction(VerifierAnnotation("t"),
                                  FUNC("verify", List(), TRUE))))
    (blockchain.accountScript _)
      .when(allAccounts(3).toAddress)
      .onCall((_: Address) =>
        Some(ContractScript(StdLibVersion.V3, testContract).explicitGet()))
    Get(routePath(s"/scriptInfo/${allAddresses(3)}")) ~> route ~> check {
      ...
      (response \ "script")
        .as[String] shouldBe "base64:AAIDAAAAAAAAAAAAAAAAAAAAAQAAAAF0AQAAAAZ2ZXJpZnkAAAAABiDCPeI="
      (response \ "scriptText")
        .as[String] shouldBe "Contract(List(),List(),Some(VerifierFunction(VerifierAnnotation(t),FUNC(verify,List(),TRUE))))"
// [WAIT]                                           Decompiler(
//      testContract,
