...
class PaymentRouteSpec
  ...
  (utx.putIfNew _)
    .when(*)
    .onCall((t: Transaction) => Right((true, Diff.empty)))
    .anyNumberOfTimes()
  (allChannels
  ...
  "accepts payments" in {
    forAll(accountOrAliasGen.label("recipient"),
           positiveLongGen.label("amount"),
           smallFeeGen.label("fee")) {
      case (recipient, amount, fee) =>
        ...
        val tx = TransferTransactionV1.selfSigned(Asset.Waves,
                                                  sender,
                                                  recipient,
                                                  amount,
                                                  timestamp,
        ...
        val route = PaymentApiRoute(restAPISettings,
                                    testWallet,
                                    utx,
                                    allChannels,
                                    time).route

        val req = Json.obj("sender" -> sender.address,
