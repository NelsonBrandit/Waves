...
package object http {
  ...
  def sameSignature(target: Array[Byte])(actual: Array[Byte]): Boolean =
    target sameElements actual

  implicit class JsFieldTypeChecker(val s: String) extends AnyVal {
    def ofType[A <: JsValue](implicit r: Reads[A], t: ClassTag[A]) =
      HavePropertyMatcher[JsValue, Any] { json =>
        val actualValue = (json \ s).validate[A]
        HavePropertyMatchResult(actualValue.isSuccess,
                                s,
  ...
  implicit def tuple2ToHPM(
      v: (String, JsValue)): HavePropertyMatcher[JsValue, JsValue] =
    HavePropertyMatcher[JsValue, JsValue] { json =>
      val actualFieldValue = (json \ v._1).as[JsValue]
      HavePropertyMatchResult(actualFieldValue == v._2,
                              v._1,
                              v._2,
                              actualFieldValue)
    }
  ...
  implicit val publicKeyAccountFormat: Format[PublicKeyAccount] =
    byteStrFormat.inmap[PublicKeyAccount](
  ...
  implicit val addressOrAliasFormat: Format[AddressOrAlias] =
    Format[AddressOrAlias](
      Reads {
        case JsString(str) =>
          Base58
            .decode(str)
  ...
  implicit val versionedTransferTransactionFormat
    : Format[TransferTransactionV2] = (
    (JsPath \ "sender").format[PublicKeyAccount] and
      (JsPath \ "recipient").format[AddressOrAlias] and
      (JsPath \ "assetId").format[Asset] and
      (JsPath \ "amount").format[Long] and
  ...
  type TransferTransactions =
    TransferTransactionV1 :+: TransferTransactionV2 :+: CNil
  implicit val autoTransferTransactionsReads: Reads[TransferTransactions] =
    Reads { json =>
      (json \ "version").asOpt[Int] match {
        case None | Some(1) =>
          transferTransactionFormat
            .reads(json)
            .map(Coproduct[TransferTransactions](_))
        case _ =>
          versionedTransferTransactionFormat
            .reads(json)
            .map(Coproduct[TransferTransactions](_))
      }
    }
